...
function renderScoreDisplay(state) {
  ...
  const couragePoints = state.getCouragePoints();
  ...
  return `
    <div class="score-widget">
      ...
      ${couragePoints > 0 ? `
        <span class="courage-points">
          <span class="courage-label">💪</span>
          <span class="courage-value" id="courage-value">${couragePoints}</span>
        </span>
      ` : ''}
    </div>
  `;
...
function renderNavigationButtons(state) {
  ...
  const canReAnswer = state.canReAnswer();
  const hasBeenReAnswered = state.hasBeenReAnswered();
  ...
  // Add re-answer button if user can re-answer and hasn't re-answered this question yet
  if (canReAnswer && !hasBeenReAnswered) {
    html += `
      <button class="re-answer-btn" id="re-answer-btn" title="Try again for courage points!">
        💪 Re-answer
      </button>
    `;
  }
    ...
    <div class="${CSS_CLASSES.RESULTS_CONTAINER}">
      ...
      <div class="${CSS_CLASSES.SCORE_DISPLAY}">
        <div class="results-main-score">
          ...
          ${couragePoints > 0 ? `
            <div class="score-item courage-item">
              <div class="score-label">💪 Courage Points</div>
              <div class="score-big courage-big">${couragePoints}</div>
              <div class="score-sublabel">bonus points</div>
            </div>
          ` : ''}
          ${maxStreak > 0 ? `
            <div class="score-item">
      ...
      ${couragePoints > 0 ? renderCouragePointsBreakdown(state.getCouragePointHistory()) : ''}
...
/**
 * Renders courage points breakdown
 * @param {Array<Object>} couragePointHistory - Array of courage point entries
 * @returns {string} HTML string
 */
function renderCouragePointsBreakdown(couragePointHistory) {
  if (!couragePointHistory || couragePointHistory.length === 0) {
    return '';
  }

  const rows = couragePointHistory.map(entry => `
    <tr>
      <td>Question ${entry.questionNumber}</td>
      <td>${entry.userAnswer}</td>
      <td class="courage-points-cell">💪 +${entry.couragePointsEarned}</td>
    </tr>
  `).join('');

  return `
    <div class="courage-points-breakdown">
      <h3>💪 Courage Points Earned</h3>
      <table class="score-table">
        <thead>
          <tr>
            <th>Question</th>
            <th>Your Answer</th>
            <th>Courage Points</th>
          </tr>
        </thead>
        <tbody>
          ${rows}
...
/**
 * Handles re-answer logic with courage points
 * @param {QuizState} state - Quiz state
 * @param {HTMLElement} container - Container element
 */
function handleReAnswer(state, container) {
  playClickSound();

  const isCorrect = state.isCurrentAnswerCorrect();
  const scoreInfo = state.updateScore(isCorrect, true); // Pass true for re-answer

  // Mark question as re-answered
  state.markAsReAnswered();
  ...
  // Animate courage points change
  if (scoreInfo.couragePointsEarned > 0) {
    setTimeout(() => {
      animateCouragePointsChange(scoreInfo.couragePointsEarned, container);
    }, 100);
  }
...
/**
 * Animates courage points change
 * @param {number} points - Courage points earned
 * @param {HTMLElement} container - Container element
 */
function animateCouragePointsChange(points, container) {
  if (points === 0) return;

  const questionContainer = container.querySelector('#question-container');
  if (!questionContainer) return;

  const courageAnimation = document.createElement('div');
  courageAnimation.className = 'courage-animation';
  courageAnimation.textContent = `💪 +${points}`;
  questionContainer.appendChild(courageAnimation);

  setTimeout(() => courageAnimation.remove(), 1000);

  // Update courage points number with animation
  const courageValue = container.querySelector('#courage-value');
  if (courageValue) {
    courageValue.classList.add('courage-bump');
    setTimeout(() => courageValue.classList.remove('courage-bump'), 300);
  }
}
...
function attachEventListeners(state, container) {
  ...
  // Re-answer button
  const reAnswerBtn = container.querySelector('#re-answer-btn');
  if (reAnswerBtn) {
    reAnswerBtn.addEventListener('click', () => {
      handleReAnswer(state, container);
    });
  }
 ...
 * @param {Function} onShowSummary - Callback when summary button is clicked (optional)
 */
export function renderTestSetMenu(testSets, container, onSelect, onShowSummary) {
  const setsHtml = testSets.map(set => `
    <div class="test-set-card" data-set-number="${set.setNumber}">
      <button class="test-set-btn" data-set-number="${set.setNumber}">
        Set ${set.setNumber}
        <span class="question-count">${set.questionCount || set.questions?.length || 0} câu hỏi</span>
      </button>
      <button class="summary-btn" data-set-number="${set.setNumber}" title="Xem tóm tắt từ vựng và ngữ pháp">
        📖 Tóm tắt
      </button>
    </div>
  `).join('');
  ...
  container.innerHTML = `
    ...
    <div class="summary-modal-overlay" id="summary-modal" style="display: none;"></div>
  `;
  ...
  // Attach summary button listeners if callback provided
  if (onShowSummary) {
    container.querySelectorAll('.summary-btn').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation();
        const setNumber = parseInt(e.currentTarget.dataset.setNumber);
        onShowSummary(setNumber);
      });
    });
  }
}
...
/**
 * Renders the summary modal with vocabulary and grammar content
 * @param {Object} summary - Summary object from test set (may be null/undefined)
 * @param {number} setNumber - Set number for display
 * @param {HTMLElement} container - Container element
 */
export function renderSummaryModal(summary, setNumber, container) {
  const modalOverlay = container.querySelector('#summary-modal');
  if (!modalOverlay) return;

  // Defensive: check if summary exists
  if (!summary) {
    modalOverlay.innerHTML = `
      <div class="summary-modal">
        <div class="summary-header">
          <h3>📖 Tóm tắt Set ${setNumber}</h3>
          <button class="close-summary-btn" id="close-summary">&times;</button>
        </div>
        <div class="summary-content">
          <div class="no-summary-message">
            <p>⚠️ Set này chưa có tóm tắt từ vựng và ngữ pháp.</p>
    ...
    `;
    modalOverlay.style.display = 'flex';
    attachSummaryCloseListener(modalOverlay);
    return;
  }

  // Build vocabulary section (defensive)
  let vocabHtml = '';
  if (summary.vocabulary && Array.isArray(summary.vocabulary) && summary.vocabulary.length > 0) {
    const vocabRows = summary.vocabulary.map(v => `
      <tr>
    ...
    vocabHtml = `
      <div class="summary-section">
        <h4>📚 Từ vựng (${summary.vocabulary.length} từ)</h4>
  ...
  // Build grammar section (defensive)
  let grammarHtml = '';
  if (summary.grammarPoints && Array.isArray(summary.grammarPoints) && summary.grammarPoints.length > 0) {
    const grammarCards = summary.grammarPoints.map(g => `
      <div class="grammar-card">
        <div class="grammar-topic">${g.topic || ''}</div>
        ${g.structure ? `<div class="grammar-structure"><strong>Cấu trúc:</strong> ${g.structure}</div>` : ''}
        ${g.usage ? `<div class="grammar-usage"><strong>Cách dùng:</strong> ${g.usage}</div>` : ''}
        ${g.examples && Array.isArray(g.examples) && g.examples.length > 0 ? `
          <div class="grammar-examples">
            <strong>Ví dụ:</strong>
            <ul>${g.examples.map(ex => `<li>${ex}</li>`).join('')}</ul>
          </div>
        ` : ''}
      </div>
    `).join('');
    ...
    grammarHtml = `
      <div class="summary-section">
        <h4>📝 Ngữ pháp (${summary.grammarPoints.length} điểm)</h4>
        <div class="grammar-cards">${grammarCards}</div>
      </div>
    `;
  }

  // Build pronunciation section (defensive)
  let pronunciationHtml = '';
  if (summary.pronunciation && Array.isArray(summary.pronunciation) && summary.pronunciation.length > 0) {
    const pronCards = summary.pronunciation.map(p => `
      <div class="pronunciation-card">
        <div class="pron-topic">${p.topic || ''}</div>
        ${p.explanation ? `<div class="pron-explanation">${p.explanation}</div>` : ''}
        ${p.examples && Array.isArray(p.examples) && p.examples.length > 0 ? `
          <div class="pron-examples">
            <ul>${p.examples.map(ex => `<li>${ex}</li>`).join('')}</ul>
          </div>
        ` : ''}
      </div>
    `).join('');
    ...
    pronunciationHtml = `
      <div class="summary-section">
        <h4>🎤 Phát âm (${summary.pronunciation.length} quy tắc)</h4>
        <div class="pronunciation-cards">${pronCards}</div>
      </div>
    `;
  }

  // Check if we have any content
  const hasContent = vocabHtml || grammarHtml || pronunciationHtml;
  ...
  modalOverlay.innerHTML = `
    <div class="summary-modal">
      ...
      <div class="summary-content">
        ...
        ` : `
          <div class="no-summary-message">
            <p>⚠️ Set này chưa có nội dung tóm tắt.</p>
          </div>
        `}
      </div>
    </div>
...
/**
 * Attaches close listener to summary modal
 * @param {HTMLElement} modalOverlay - Modal overlay element
 */
function attachSummaryCloseListener(modalOverlay) {
  const closeBtn = modalOverlay.querySelector('#close-summary');
  if (closeBtn) {
