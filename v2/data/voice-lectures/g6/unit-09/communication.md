# G6 UNIT-09 COMMUNICATION

<!-- chunk: intro -->
<teacher_script pause="0">
Ok lớp 6, hôm nay học phần <eng>Communication</eng> nha. Mình sẽ học về câu cảm thán - <eng>Exclamatory Sentences</eng>. Đây là cách nói khi em muốn bày tỏ cảm xúc mạnh như ngạc nhiên, vui, hay thán phục. Mở sách trang 30 đi.
</teacher_script>

<!-- chunk: vocabulary -->
<teacher_script pause="0">
Học từ vựng trước nha. Bài này có nhiều từ về địa danh nổi tiếng thế giới.
</teacher_script>

<vocabulary>
**Vocabulary**
1. **tower** : (n) tháp /ˈtaʊə(r)/
2. **bank** : (n) bờ sông /bæŋk/
3. **bell** : (n) cái chuông /bel/
4. **shell** : (n) vỏ sò /ʃel/
5. **the whole view** : (n.p) toàn cảnh /ðə həʊl vjuː/
6. **theatre** : (n) nhà hát /ˈθɪətə/
7. **harbour** : (n) cảng /ˈhɑːbə/
8. **roof** : (n) mái nhà /ruːf/
9. **million** : (n) triệu /ˈmɪljən/
10. **climb** : (v) leo /klaɪm/
11. **ancient** : (adj) cổ xưa /ˈeɪnʃənt/
12. **capital city** : thủ đô /ˈkæpɪtl ˈsɪti/
13. **national park** : công viên quốc gia /ˈnæʃnəl pɑːk/
</vocabulary>

<!-- chunk: grammar_exclamatory -->
<teacher_script pause="0">
Giờ học ngữ pháp về câu cảm thán nha. Câu cảm thán bắt đầu bằng <eng>What</eng> và kết thúc bằng dấu chấm than (!). Mục đích là để thể hiện cảm xúc mạnh - như "Đẹp quá!", "Hay quá!".
</teacher_script>

**UNIT 9: CITIES OF THE WORLD**

**COMMUNICATION**

**Câu cảm thán (Exclamatory Sentences)**

<teacher_script pause="0">
Công thức: <eng>What + (a/an) + tính từ + danh từ + !</eng>. Quan trọng là biết khi nào dùng <eng>a/an</eng>, khi nào không.
</teacher_script>

<grammar>
**1. Danh từ số ít đếm được:**
What + a/an + tính từ + danh từ số ít!
Ví dụ: What a handsome boy! (Cậu bé đẹp trai quá!)

**2. Danh từ số nhiều đếm được:**
What + tính từ + danh từ số nhiều! (KHÔNG có a/an)
Ví dụ: What noisy boys! (Mấy cậu bé ồn ào quá!)

**3. Danh từ không đếm được:**
What + tính từ + danh từ không đếm được! (KHÔNG có a/an)
Ví dụ: What fresh air! (Không khí trong lành quá!)
</grammar>

<teacher_script pause="0">
Mẹo nhớ nè: Chỉ danh từ số ít đếm được mới có <eng>a/an</eng>. Nếu là danh từ số nhiều (có s) hoặc không đếm được (như <eng>air, weather, water</eng>) thì KHÔNG có <eng>a/an</eng>.
</teacher_script>

---

<!-- chunk: exercise_1 -->
### Bài 1 trang 30 - Listen and read

<teacher_script pause="0">
Bài 1. Nhìn bức tranh về thành phố và nghe các câu cảm thán. Em để ý cách dùng <eng>What</eng> để thể hiện cảm xúc nha.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2021/1105/gs-u9-communication-ex1.mp3">
**Audio:** Communication
</audio>

<task>
**Đề:** Look at the picture. Listen and read the exclamations.
**Dịch đề:** Nhìn vào bức tranh. Nghe và đọc các câu cảm thán.
</task>

<reading>
**Exclamations (Các câu cảm thán):**
1. What a nice city! *(Thật là một thành phố tốt đẹp!)*
2. What a clear sky! *(Thật là bầu trời trong sáng!)*
3. What tall buildings! *(Những tòa nhà cao làm sao!)*
</reading>

<teacher_script pause="0">
Em thấy không: câu 1 và 2 có <eng>a</eng> vì <eng>city</eng> và <eng>sky</eng> là danh từ số ít đếm được. Câu 3 không có <eng>a</eng> vì <eng>buildings</eng> là số nhiều.
</teacher_script>

---

<!-- chunk: exercise_2 -->
### Bài 2 trang 30 - Make exclamations

<teacher_script pause="60">
Bài 2. Nhìn hình và viết câu cảm thán với <eng>What</eng>. Nhớ quy tắc: số ít có <eng>a/an</eng>, số nhiều và không đếm được thì không. 1 phút.
</teacher_script>

<task>
**Đề:** Work in pairs. Look at the picture below and say what you think by using exclamations with What. You may use the suggestions below.
**Dịch đề:** Làm việc theo cặp. Nhìn vào hình bên dưới và nói những gì em nghĩ bằng cách sử dụng câu cảm thán với What. Em có thể sử dụng gợi ý bên dưới.
</task>

<questions type="fill_blanks">
**Gợi ý từ vựng:**
- happy / beautiful children
- colourful / nice picture
- sunny / beautiful weather
- small / cute boy
- beautiful / colourful apple trees

**Viết câu cảm thán:**
1. _______ children! (happy)
2. _______ picture! (colourful)
3. _______ weather! (sunny)
4. _______ boy! (cute)
5. _______ apple trees! (beautiful)
</questions>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. What happy and beautiful children! *(children - số nhiều → KHÔNG có a)*
2. What a colourful and nice picture! *(picture - số ít → có a)*
3. What sunny and beautiful weather! *(weather - không đếm được → KHÔNG có a)*
4. What a small and cute boy! *(boy - số ít → có a)*
5. What beautiful and colourful apple trees! *(trees - số nhiều → KHÔNG có a)*

**Giải thích:**
- <eng>children, trees</eng> là số nhiều nên KHÔNG có <eng>a</eng>
- <eng>weather</eng> là danh từ không đếm được nên KHÔNG có <eng>a</eng>
- <eng>picture, boy</eng> là số ít đếm được nên có <eng>a</eng>
</answer>

---

<!-- chunk: exercise_3 -->
### Bài 3 trang 31 - City landmarks

<teacher_script pause="45">
Bài 3. Viết tên các địa danh nổi tiếng dưới hình. Đây là những <eng>landmarks</eng> - địa danh nổi tiếng thế giới mà em nên biết. 45 giây.
</teacher_script>

<task>
**Đề:** Work in pairs. Write the names of the landmarks under the correct pictures.
**Dịch đề:** Làm việc theo cặp. Viết tên của các địa danh dưới các hình ảnh đúng.
</task>

<questions type="matching">
**Các địa danh để chọn:**
| Eiffel Tower | Big Ben | Sydney Opera House | Merlion |

*(Nhìn 4 hình ảnh và viết tên địa danh)*
1. _______ (hình con sư tử cá)
2. _______ (hình tháp đồng hồ)
3. _______ (hình nhà hát mái trắng như vỏ sò)
4. _______ (hình tháp sắt cao)
</questions>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. Merlion *(con sư tử biển - Singapore)*
2. Big Ben *(tháp đồng hồ - London)*
3. Sydney Opera House *(nhà hát con sò - Sydney)*
4. Eiffel Tower *(tháp Eiffel - Paris)*
</answer>

---

<!-- chunk: exercise_4 -->
### Bài 4 trang 31 - Match landmarks with descriptions

<teacher_script pause="60">
Bài 4. Đọc các đoạn mô tả và viết tên địa danh tương ứng. Chú ý các từ khóa như <eng>tower, clock, bell, theatre, shells, lion, fish</eng>. 1 phút.
</teacher_script>

<task>
**Đề:** Write the names of the landmarks in 3 for the descriptions.
**Dịch đề:** Viết tên của các địa danh trong bài 3 cho phần mô tả.
</task>

<questions type="fill_blanks">
**1. _______**
This tower is on the bank of the River Thames in London. In the tower, there's a big clock and a bell.
*(Tòa tháp này nằm trên bờ sông Thames ở London. Trong tháp có một cái đồng hồ lớn và một cái chuông.)*

**2. _______**
It's a theatre on Sydney Harbour. Its roofs are white and look like shells. Visitors from all over the world come to see it.
*(Đó là một nhà hát trên Cảng Sydney. Mái nhà có màu trắng và trông giống như vỏ sò. Du khách từ khắp nơi trên thế giới đến để xem nó.)*

**3. _______**
This famous tower is in Paris. Every year, millions of visitors climb to its top to see the whole view of Paris.
*(Tòa tháp nổi tiếng này ở Paris. Mỗi năm, hàng triệu du khách leo lên đỉnh để ngắm toàn cảnh Paris.)*

**4. _______**
It has the head of a lion and the body of a fish. It's a famous landmark in Singapore.
*(Nó có đầu sư tử và thân cá. Đó là địa danh nổi tiếng ở Singapore.)*
</questions>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. Big Ben *(từ khóa: tower, River Thames, London, clock, bell)*
2. Sydney Opera House *(từ khóa: theatre, Sydney Harbour, white roofs, shells)*
3. Eiffel Tower *(từ khóa: famous tower, Paris, climb to top)*
4. Merlion *(từ khóa: head of lion, body of fish, Singapore)*
</answer>

---

<!-- chunk: exercise_5 -->
### Bài 5 trang 31 - Quiz

<teacher_script pause="60">
Bài 5. Trả lời các câu hỏi quiz về các thành phố và địa danh. Đây là kiến thức về địa lý và du lịch nha. 1 phút.
</teacher_script>

<task>
**Đề:** Work in pairs. Read the questions and circle the correct answers.
**Dịch đề:** Làm việc theo cặp. Đọc các câu hỏi và khoanh tròn câu trả lời đúng.
</task>

<questions type="multiple_choice">
**1.** Which city is nearer to Ha Noi than the others?
*(Thành phố nào gần Hà Nội hơn những thành phố còn lại?)*
- A. Berlin *(Béc-lin - Đức)*
- B. Bangkok *(Băng Cốc - Thái Lan)*
- C. Tokyo *(Tokyo - Nhật)*

**2.** The people of Quang Nam are proud of their ancient town, _______.
*(Người dân Quảng Nam tự hào về phố cổ của họ, _______)*
- A. Hoi An *(Hội An)*
- B. SaPa
- C. Bach Ma *(Bạch Mã)*

**3.** Phnom Penh is the capital city of _______.
*(Phnom Penh là thủ đô của _______)*
- A. Indonesia *(In-đô-nê-xi-a)*
- B. the Philippines *(Phi-lip-pin)*
- C. Cambodia *(Cam-pu-chia)*

**4.** For children, California is famous for its _______.
*(Đối với trẻ em, California nổi tiếng với _______)*
- A. Disneyland Park *(Công viên Disneyland)*
- B. Golden Gate Bridge *(Cầu Cổng Vàng)*
- C. national parks *(các công viên quốc gia)*
</questions>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** 1.B | 2.A | 3.C | 4.A

**Giải thích:**
- **Câu 1: B. Bangkok** - Băng Cốc (Thái Lan) gần Hà Nội nhất vì cùng ở Đông Nam Á. Berlin ở Đức (châu Âu), Tokyo ở Nhật (Đông Á).
- **Câu 2: A. Hoi An** - Phố cổ Hội An thuộc tỉnh Quảng Nam, được UNESCO công nhận là di sản văn hóa thế giới.
- **Câu 3: C. Cambodia** - Phnom Penh là thủ đô của Campuchia (Cambodia).
- **Câu 4: A. Disneyland Park** - Trẻ em yêu thích Disneyland vì đó là công viên giải trí nổi tiếng với chuột Mickey và các nhân vật hoạt hình.
</answer>

<!-- chunk: end -->
<teacher_script pause="0">
Ok hết bài <eng>Communication</eng>. Hôm nay em học được câu cảm thán với <eng>What</eng>. Nhớ: số ít có <eng>a/an</eng> (<eng>What a beautiful city!</eng>), số nhiều và không đếm được thì không (<eng>What nice people! What great weather!</eng>). Bài sau học Skills nha. Bye!
</teacher_script>
