# UNIT 10: COMMUNICATION IN THE FUTURE
## SKILLS 2 - Listening and Writing

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_bab9d840.mp3">
Ok lớp 8, Unit 10 bài <eng>Skills 2</eng> nha. Bài này mình sẽ luyện nghe và viết về chủ đề giao tiếp trong tương lai. Các em mở sách trang 110 đi.
</teacher_script>

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_54686200.mp3">
Trước khi nghe, mình học từ vựng trước nha. Bấm nút "Bắt Đầu" bên dưới để học từ vựng. Máy sẽ đọc và các em đọc theo. Sau đó làm bài trắc nghiệm từ vựng rồi ghi từ vựng vô tập.
</teacher_script>

<vocabulary>
1. **carrier pigeon** : bồ câu đưa thư /ˈkæriə ˈpɪdʒɪn/
2. **smartwatch** : đồng hồ thông minh /smɑːtˈwɒtʃ/
3. **translation machine** : máy dịch /trænsˈleɪʃn məˈʃiːn/
4. **hologram** : (n) hình chiếu lập thể /ˈhɒləɡræm/
5. **agenda** : (n) lịch trình /əˈdʒendə/
6. **announcement** : (n) thông báo /əˈnaʊnsmənt/
7. **ceremony** : (n) nghi thức /ˈserɪməni/
8. **introductory talk** : bài nói giới thiệu
9. **exhibition** : (n) triển lãm /ˌeksɪˈbɪʃn/
</vocabulary>

<!-- chunk: bai1 -->
### Bai 1 trang 110 - Picture Vocabulary

<teacher_script pause="0" href="audio/tts_894064c1.mp3">
Bài 1 <eng>Look at the pictures. Complete the word or phrase for each picture</eng> nha. Nhìn tranh và hoàn thành từ hoặc cụm từ cho mỗi tranh.
</teacher_script>

<task>
**De:** Look at the pictures. Complete the word/phrase for each picture.
**Dich de:** Nhìn vào những bức tranh. Hoàn thành từ/cụm từ cho mỗi bức tranh.
</task>

<questions type="fill_blanks">
**1.** c _ _ _ _ _ r   p _ _ _ _ n
*bồ câu đưa thư*

**2.** s _ _ _ t   w _ _ _ h
*đồng hồ thông minh*

**3.** t _ _ _ _ _ _ _ _ _ n   m _ _ _ _ _ e
*máy dịch*

**4.** h _ _ _ _ _ _ _ y
*ảnh toàn ký/hình chiếu lập thể*
</questions>

<teacher_script pause="30" href="audio/tts_fe659949.mp3">
Các em làm bài 1 đi, 30 giây nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. carrier pigeon (bồ câu đưa thư)
2. smartwatch (đồng hồ thông minh)
3. translation machine (máy dịch)
4. holography (ảnh toàn ký)
</answer>

<explanation>
- **carrier pigeon**: Ngày xưa người ta dùng bồ câu đưa thư để gửi tin nhắn đi xa á.
- **smartwatch**: Đồng hồ thông minh có thể kết nối internet, nhận tin nhắn, gọi điện nè.
- **translation machine**: Máy dịch tự động chuyển ngôn ngữ này sang ngôn ngữ khác.
- **holography**: Công nghệ tạo hình ảnh 3D lập thể, trong tương lai có thể dùng để giao tiếp luôn.
</explanation>

<!-- chunk: tapescript -->
### Tapescript - Bai nghe

<teacher_script pause="0" href="audio/tts_cd2bc2ae.mp3">
Giờ mình đọc bài nghe trước nha. Đây là thông báo về một buổi triển lãm.
</teacher_script>

<reading>
Hi students! Here's our plan for the exhibition tomorrow morning.

First, the opening ceremony is at the Main Hall. We'll listen to Dr. Lam introduce the exhibition. The ceremony starts at 8:30 am, so remember to meet me at the Main Hall by 8:20.

Next, we'll visit the "History" section. It's on the first floor. You'll see pictures of different ways people communicated in the past, such as smoke signals and carrier pigeons. We'll be there from 9:15 to 10:00.

Then, we'll move to the "Modern Time" section. It's in the Tech Room. You will see devices with internet connections, like smartphones and smartwatches. I'm sure you'll like it very much but we can only spend an hour there from 10:15 to 11:15.

Finally, we'll watch an animated film in the Cinema Room. The film is about communication devices in the future, like personal translation machines and holography. We must be at the Cinema Room by 11:30 am.

Now...
</reading>

<translation>
Chào các em học sinh! Đây là kế hoạch của chúng ta cho buổi triển lãm vào sáng mai.

Đầu tiên là lễ khai mạc tại Sảnh Chính. Chúng ta sẽ nghe Tiến sĩ Lâm giới thiệu về cuộc triển lãm. Buổi lễ bắt đầu lúc 8:30 sáng, vì vậy hãy nhớ gặp cô tại Hội trường chính trước 8:20.

Tiếp theo, chúng ta sẽ ghé thăm gian "Lịch sử". Nó ở tầng một. Các em sẽ thấy hình ảnh về những cách giao tiếp khác nhau mà con người sử dụng trong quá khứ, chẳng hạn như tín hiệu khói và chim bồ câu đưa thư. Chúng ta sẽ ở đó từ 9:15 đến 10:00.

Sau đó, chúng ta sẽ chuyển sang gian "Thời hiện đại". Nó nằm trong Phòng Công nghệ. Các em sẽ thấy các thiết bị có kết nối internet, như điện thoại thông minh và đồng hồ thông minh. Cô chắc rằng các em sẽ rất thích nhưng chúng ta chỉ có thể dành một giờ ở đó, từ 10:15 đến 11:15.

Cuối cùng, chúng ta sẽ xem một bộ phim hoạt hình trong Phòng chiếu phim. Bộ phim nói về các thiết bị liên lạc trong tương lai, như máy phiên dịch cá nhân và ảnh toàn ký. Chúng ta phải có mặt tại Phòng chiếu phim trước 11:30 sáng.

Bây giờ...
</translation>

<teacher_script pause="120" href="audio/tts_a1b87899.mp3">
Các em đọc và dịch bài nghe vô tập nha. 2 phút.
</teacher_script>

<!-- chunk: bai2 -->
### Bai 2 trang 110 - Fill in the Blanks

<teacher_script pause="0" href="audio/tts_5091bb08.mp3">
Bài 2 <eng>Listen to an announcement about an exhibition. Complete each gap in the agenda with one word or a number</eng>. Nghe thông báo về một cuộc triển lãm và điền vào chỗ trống trong lịch trình.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2023/0804/track-66.mp3">
**Audio:** Skills 2 - Bai 2
</audio>

<task>
**De:** Listen to an announcement about an exhibition. Complete each gap in the agenda with one word or a number.
**Dich de:** Nghe một thông báo về một cuộc triển lãm. Hoàn thành mỗi khoảng trống trong lịch trình với một từ hoặc một số.
</task>

<questions type="fill_blanks">
**EXHIBITION AGENDA**

| **Event** | **Where to go** | **When to meet** |
|-----------|-----------------|------------------|
| Opening ceremony | Main Hall | by (1) _______ a.m. |
| History | (2) _______ floor | 9:15 a.m. |
| (3) _______ Time | Tech Room | (4) _______ a.m. |
| animated film | (5) _______ Room | 11:30 a.m. |
</questions>

<teacher_script pause="60" href="audio/tts_d95408f7.mp3">
Các em nghe và điền vào chỗ trống nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** 1. 8:20 | 2. first | 3. Modern | 4. 10:15 | 5. Cinema
</answer>

<explanation>
- **Câu 1: 8:20** - Cô giáo nói "remember to meet me at the Main Hall by 8:20" (nhớ gặp cô tại Sảnh Chính trước 8:20).
- **Câu 2: first** - "It's on the first floor" (Nó ở tầng một).
- **Câu 3: Modern** - "we'll move to the Modern Time section" (chúng ta sẽ chuyển sang gian Thời hiện đại).
- **Câu 4: 10:15** - "from 10:15 to 11:15" (từ 10:15 đến 11:15).
- **Câu 5: Cinema** - "watch an animated film in the Cinema Room" (xem phim hoạt hình trong Phòng chiếu phim).
</explanation>

<!-- chunk: bai3 -->
### Bai 3 trang 111 - Multiple Choice

<teacher_script pause="0" href="audio/tts_be7dba33.mp3">
Bài 3 <eng>Listen again. Circle the correct answer A, B, or C</eng>. Nghe lại và khoanh tròn đáp án đúng A, B hoặc C nha.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2023/0804/track-67.mp3">
**Audio:** Skills 2 - Bai 3
</audio>

<task>
**De:** Listen again. Circle the correct answer A, B, or C.
**Dich de:** Nghe lại. Khoanh tròn câu trả lời đúng A, B hoặc C.
</task>

<questions type="multiple_choice">
**1.** Who is making the announcement?
- A. A teacher.
- B. A student.
- C. Dr Lam.
*Ai là người đưa ra thông báo?*

**2.** What is in the History section?
- A. An introductory talk.
- B. A documentary.
- C. Many pictures.
*Có gì trong phần Lịch sử?*

**3.** What is NOT true about the Modern Time section?
- A. Students won't like this section.
- B. It shows smart devices.
- C. They will be there for one hour.
*Điều gì KHÔNG đúng về phần Thời hiện đại?*

**4.** What will the film in the Cinema Room be about?
- A. Super smartwatches.
- B. Translation devices for personal use.
- C. Telepathy machines.
*Bộ phim trong phòng chiếu phim nói về điều gì?*

**5.** In general, what does the exhibition show?
- A. Problems of modern technology.
- B. History of smartphones.
- C. Means of communication.
*Nhìn chung, buổi triển lãm cho thấy điều gì?*
</questions>

<teacher_script pause="60" href="audio/tts_670adb7a.mp3">
Các em nghe và chọn đáp án nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_fb9ab770.mp3">
Ok đáp án nè, mình giải thích từng câu luôn.
</teacher_script>

<answer>
**Đáp án:** 1.A | 2.C | 3.A | 4.B | 5.C
</answer>

<explanation>
- **Câu 1: A. A teacher** - Người nói bắt đầu bằng "Hi students!" và nói "remember to meet me" (nhớ gặp cô), cho thấy đây là giáo viên đang nói với học sinh.

- **Câu 2: C. Many pictures** - Bài nghe nói "You'll see pictures of different ways people communicated in the past" (Các em sẽ thấy hình ảnh về những cách giao tiếp khác nhau trong quá khứ).

- **Câu 3: A. Students won't like this section** - Câu này SAI vì cô giáo nói "I'm sure you'll like it very much" (Cô chắc chắn các em sẽ rất thích nó). Còn B và C đều đúng.

- **Câu 4: B. Translation devices for personal use** - Bài nghe nói "The film is about communication devices in the future, like personal translation machines" (Bộ phim nói về các thiết bị liên lạc trong tương lai, như máy phiên dịch cá nhân).

- **Câu 5: C. Means of communication** - Buổi triển lãm trình bày về các phương tiện giao tiếp qua các thời kỳ: quá khứ (bồ câu, tín hiệu khói), hiện đại (smartphone, smartwatch), và tương lai (máy dịch, holography).
</explanation>

<!-- chunk: bai4 -->
### Bai 4 trang 111 - Matching

<teacher_script pause="0" href="audio/tts_259fe9de.mp3">
Qua phần <eng>Writing</eng> nha. Bài 4 <eng>Work in pairs. Match the ideas with parts of an outline for a paragraph about a video call</eng>. Làm việc theo cặp và nối các ý với phần dàn bài cho đoạn văn về cuộc gọi video.
</teacher_script>

<task>
**De:** Work in pairs. Match the ideas (a - h) with parts of an outline (1 - 4) for a paragraph about a video call.
**Dich de:** Làm việc theo cặp. Nối các ý (a - h) với các phần của dàn ý (1 - 4) cho một đoạn văn về một cuộc gọi video.
</task>

<questions type="matching">
**Outline (Dan y):**
1. What is it? *(Đó là gì?)*
2. What are its advantages? *(Lợi ích của nó là gì?)*
3. What are its disadvantages? *(Nhược điểm của nó là gì?)*
4. Will people use it in the future? *(Liệu mọi người có sử dụng nó trong tương lai không?)*

**Ideas (Y tuong):**
a. a phone call using Internet connection *(cuộc gọi điện thoại sử dụng kết nối Internet)*
b. needs a high-speed Internet access *(cần truy cập Internet tốc độ cao)*
c. saves time because people needn't travel to meet *(tiết kiệm thời gian vì mọi người không cần đi lại để gặp nhau)*
d. transmits live images of the speakers with a webcam or camera on smart devices *(truyền hình ảnh trực tiếp của người nói qua webcam hoặc camera trên thiết bị thông minh)*
e. may spend too much time chatting with each other *(có thể dành quá nhiều thời gian nói chuyện với nhau)*
f. can be useful for family members and business partners *(có thể hữu ích cho các thành viên gia đình và đối tác kinh doanh)*
g. will still be a common tool of communication *(vẫn sẽ là công cụ giao tiếp phổ biến)*
h. can see and hear each other in real time *(có thể nhìn và nghe nhau trong thời gian thực)*
</questions>

<teacher_script pause="60" href="audio/tts_e5b9b977.mp3">
Các em nối các ý với phần dàn bài nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** 1.a | 2.c,d | 3.b,e | 4.f
</answer>

<explanation>
- **1 - a**: "What is it?" nối với "a phone call using Internet connection" - Video call là cuộc gọi điện thoại qua kết nối Internet.
- **2 - c, d**: "What are its advantages?" nối với "saves time because people needn't travel to meet" (tiết kiệm thời gian vì không cần di chuyển) và "transmits live images of the speakers with a webcam or camera on smart devices" (truyền hình ảnh trực tiếp qua webcam).
- **3 - b, e**: "What are its disadvantages?" nối với "needs a high-speed Internet access" (cần mạng tốc độ cao) và "may spend too much time chatting with each other" (có thể tốn quá nhiều thời gian chat).
- **4 - f**: "Will people use it in the future?" nối với "can be useful for family members and business partners" - Video call vẫn sẽ hữu ích cho gia đình và đối tác kinh doanh trong tương lai.
</explanation>

<!-- chunk: bai5 -->
### Bai 5 trang 111 - Writing

<teacher_script pause="0" href="audio/tts_5f079cff.mp3">
Bài 5 <eng>Write a paragraph of 80-100 words to describe a way of modern communication. You can use the ideas in 4 or your own ideas</eng>. Viết một đoạn văn 80-100 từ miêu tả một cách giao tiếp hiện đại. Các em có thể dùng ý ở bài 4 hoặc ý của mình.
</teacher_script>

<task>
**De:** Write a paragraph (80-100 words) to describe a way of modern communication. You can use the ideas in 4 or your own ideas.
**Dich de:** Viết một đoạn văn (80-100 từ) miêu tả một cách thức giao tiếp hiện đại. Em có thể dùng các ý ở bài 4 hoặc dùng ý tưởng của riêng mình.
</task>

<teacher_script pause="0" href="audio/tts_7b989f95.mp3">
Mình có thể viết về video call theo dàn bài ở bài 4, hoặc viết về mạng xã hội, tin nhắn, email... đều được nha. Nhớ theo cấu trúc:
1. Đó là gì?
2. Lợi ích của nó?
3. Nhược điểm của nó?
4. Tương lai sẽ như thế nào?
</teacher_script>

<teacher_script pause="300" action="photo" href="audio/tts_8c292e8d.mp3">
Các em viết bài vô tập rồi chụp hình gửi nha. 5 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_9fa96849.mp3">
Đây là bài mẫu về mạng xã hội nè.
</teacher_script>

<answer>
**Bai mau - Social Networks:**

One of the most popular ways of modern communication is using social media platforms, such as Facebook, Instagram, Zalo etc. Social media allows people to connect and communicate with each other instantly, regardless of their geographical location. With just a few clicks, people can share pictures, videos, and messages with their friends and family. Furthermore, social media has become a powerful tool for spreading news and information quickly and efficiently. Overall, social media has revolutionized the way we communicate and interact with each other in the modern age.
</answer>

<translation>
Một trong những cách thức thông dụng nhất để giao tiếp hiện đại đó là sử dụng các nền tảng mạng xã hội, như Facebook, Instagram, Zalo, vân vân. Mạng xã hội cho phép mọi người kết nối và giao tiếp với nhau ngay tức thì, bất kể vị trí địa lý của họ. Với chỉ vài cái nhấp chuột, người ta có thể chia sẻ hình ảnh, video và những tin nhắn với bạn bè và người thân. Hơn nữa, mạng xã hội còn là một công cụ vô cùng mạnh mẽ để lan truyền tin tức và thông tin một cách nhanh chóng và hiệu quả. Nhìn chung, mạng xã hội đã cách mạng hóa phương thức mà chúng ta giao tiếp hay tương tác với nhau trong xã hội hiện đại.
</translation>

<teacher_script pause="0" href="audio/tts_d351f38a.mp3">
Còn đây là bài mẫu về video call nha.
</teacher_script>

<answer>
**Bai mau - Video Call:**

Making a video call is a modern way of communication. A video call is a phone call via the Internet. It transmits live images of the speakers with a webcam or camera on smart devices. Video calls are time-saving because people do not need to travel to meet and still see and hear each other in real time. They help both family members and business partners keep contact with each other regardless of geographic distances. However, video calls require high-speed Internet access, so not all the time people can make one. I believe that when the Internet becomes better in the future, video calls will be an even more common tool of communication.
</answer>

<translation>
Gọi điện video là một cách giao tiếp hiện đại. Gọi điện video là một cuộc gọi điện thoại qua Internet. Nó truyền hình ảnh trực tiếp của người nói bằng webcam hoặc camera trên các thiết bị thông minh. Gọi điện video tiết kiệm thời gian vì mọi người không cần phải di chuyển để gặp nhau và vẫn có thể nhìn thấy và nghe thấy nhau theo thời gian thực. Chúng giúp cả các thành viên gia đình và đối tác kinh doanh giữ liên lạc với nhau bất kể khoảng cách địa lý. Tuy nhiên, gọi điện video yêu cầu truy cập Internet tốc độ cao, vì vậy không phải lúc nào mọi người cũng có thể gọi điện. Tôi tin rằng khi Internet trở nên tốt hơn trong tương lai, gọi điện video sẽ trở thành một công cụ giao tiếp phổ biến hơn nữa.
</translation>

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_b706853f.mp3">
Ok vậy là xong bài <eng>Skills 2</eng> Unit 10 rồi nha. Bài này các em đã học về các phương tiện giao tiếp qua các thời kỳ: từ bồ câu đưa thư ngày xưa, đến smartphone, smartwatch ngày nay, và máy dịch, hologram trong tương lai. Các em nhớ ôn lại từ vựng và tập viết đoạn văn về một cách giao tiếp hiện đại nha. Hẹn gặp lại các em ở bài sau!
</teacher_script>
