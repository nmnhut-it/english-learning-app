# UNIT 4: ETHNIC GROUPS OF VIET NAM
## GETTING STARTED - Viet Nam's 54 Ethnic Groups

<!-- chunk: vocabulary -->
<teacher_script pause="0">
Ok lớp 8, Unit 4 nha - <eng>Ethnic Groups of Viet Nam</eng>. Mở sách trang 36 đi. Ghi tựa bài vô tập: <eng>Getting Started - Viet Nam's 54 Ethnic Groups</eng>. Xong rồi bấm "Bắt đầu" để học từ vựng. Máy đọc thì đọc theo, sau đó làm quiz rồi ghi từ vựng vô tập nha.
</teacher_script>

<vocabulary>
1. **look** : (v) trông có vẻ, nhìn /lʊk/
2. **ethnic group** : (n) nhóm dân tộc /ˈeθnɪk ɡruːp/
3. **bamboo house** : (n) nhà tre /bæmˈbuː haʊs/
4. **terraced fields** : (n) ruộng bậc thang /ˈterəst fiːldz/
5. **awesome** : (adj) tuyệt vời, ấn tượng /ˈɔːsəm/
6. **culture** : (n) văn hóa /ˈkʌltʃər/
7. **folk dance** : (n) múa dân gian /fəʊk dɑːns/
8. **musical instrument** : (n) nhạc cụ /ˈmjuːzɪkəl ˈɪnstrəmənt/
9. **five-colour sticky rice** : (n) xôi ngũ sắc /faɪv ˈkʌlər ˈstɪki raɪs/
10. **minority group** : (n) nhóm dân tộc thiểu số /maɪˈnɒrɪti ɡruːp/
11. **multi-storey home** : (n) nhà nhiều tầng /ˈmʌlti ˈstɔːri həʊm/
12. **costume** : (n) trang phục truyền thống /ˈkɒstjuːm/
13. **stilt house** : (n) nhà sàn /stɪlt haʊs/
14. **overlook** : (v) nhìn ra, nhìn xuống /ˌəʊvəˈlʊk/
15. **traditional** : (adj) truyền thống /trəˈdɪʃənəl/
16. **mention** : (v) đề cập, nhắc đến /ˈmenʃən/
17. **post** : (n) cây cột /pəʊst/
</vocabulary>

<!-- chunk: dialogue -->
<teacher_script pause="0">
Ok, giờ nghe hội thoại bài 1 nha. Tom gặp Lai - một bạn học sinh người dân tộc Tày từ Hà Giang. Nghe audio rồi đọc theo, vừa nghe vừa nhìn bảng dịch luôn.
</teacher_script>

<reading>
🎧 [Track 20](https://img.loigiaihay.com/picture/2023/0803/track-20.mp3)
</reading>

<dialogue>
**Tom:** Hi, I'm Tom. You look new here.
**Lai:** I am. I'm Lai from Ha Giang.
**Tom:** Oh, I've heard about beautiful Ha Giang. Do you live in the mountains?
**Lai:** Yes, I do. I'm from the Tay ethnic group. We are the second largest ethnic group in Viet Nam, only after the Kinh.
**Tom:** Oh... I once saw a bamboo house on high posts in a travel brochure. Do you live in a home like that?
**Lai:** Yes, we call it a "stilt house". Our house overlooks terraced fields.
**Tom:** Awesome. What is life in your village like?
**Lai:** It's peaceful. There are 16 houses in my village. We live very close to nature.
**Tom:** I love it. Can you tell me something about your culture?
**Lai:** Certainly. We have our own culture. You can see it in our folk dances, musical instruments like the dan tinh, and our special five-colour sticky rice.
**Tom:** It sounds interesting. I hope to visit Ha Giang one day.
</dialogue>

<translation>
**Tom:** Xin chào, mình là Tom. Trông cậu giống người mới đến đây.
**Lai:** Đúng rồi. Mình là Lai đến từ Hà Giang.
**Tom:** Ồ, mình đã nghe về tỉnh Hà Giang tươi đẹp rồi. Cậu có sống trên những ngọn núi không?
**Lai:** Có chứ. Mình là người Tày. Dân tộc mình lớn thứ hai ở Việt Nam, chỉ sau người Kinh.
**Tom:** Ồ… mình từng thấy một ngôi nhà tre trên một tờ gấp quảng cáo du lịch. Cậu có sống ở một căn nhà như thế không?
**Lai:** Có chứ, người ta gọi đó là nhà sàn. Nhà mình thì nhìn ra được ruộng bậc thang.
**Tom:** Tuyệt vời. Cuộc sống ở làng bạn như thế nào?
**Lai:** Nó rất là yên bình. Có 16 ngôi nhà trong làng mình. Làng mình sống rất gần gũi với thiên nhiên.
**Tom:** Thích quá. Cậu có thể nói cho mình biết một vài thứ về văn hóa làng cậu không?
**Lai:** Chắc chắn rồi. Tụi mình có nền văn hóa riêng. Cậu có thể thấy nó trong các điệu dân vũ, hay các nhạc cụ như đàn tính và món xôi ngũ sắc đặc biệt.
**Tom:** Nghe thú vị quá. Mong là ngày nào đó mình sẽ được đến thăm Hà Giang.
</translation>

<teacher_script pause="180">
Ok, giờ dịch hội thoại vô vở nha. Nhìn cột tiếng Việt để tham khảo. Cho 3 phút.
</teacher_script>

<!-- chunk: exercise-2 -->
<teacher_script pause="0">
Xong rồi làm bài 2 nha. Bài <eng>multiple choice</eng> — chọn đáp án đúng dựa vào hội thoại vừa đọc. Cho 1 phút.
</teacher_script>

<task>
**Đề:** Choose the best answer A, B, or C based on the dialogue.
**Dịch đề:** Chọn đáp án đúng nhất A, B, hoặc C dựa vào hội thoại.
</task>

<questions type="multiple_choice">
**1.** What are Tom and Lai talking about?
- A. Minority groups in Viet Nam.
- B. Ha Giang.
- C. Life of the Tay people.
*Tom và Lai đang nói về điều gì?*

**2.** A "stilt house" _______.
- A. is a multi-storey home
- B. is on high posts
- C. always faces a field
*Một "nhà sàn" _______.*

**3.** Lai mentions _______ of the Tay people.
- A. folk dances
- B. popular festivals
- C. costumes
*Lai đề cập đến _______ của người Tày.*
</questions>

<teacher_script pause="0">
Ok đáp án nha.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** 1.C | 2.C | 3.A

**1.C** — Hội thoại chủ yếu nói về cuộc sống của người Tày ở làng Lai: nhà sàn, ruộng bậc thang, văn hóa, đàn tính, xôi ngũ sắc.

**2.C** — Lai nói: *"Our house overlooks terraced fields"* = nhà sàn **luôn nhìn ra một cánh đồng** (*always faces a field*). KHÔNG phải B vì Tom dùng "on high posts" để hỏi, không phải Lai định nghĩa nhà sàn.

**3.A** — Lai nói: *"You can see it in our folk dances, musical instruments like the dan tinh..."* → múa dân gian.
</answer>

<!-- chunk: exercise-3 -->
<teacher_script pause="0">
Bài 3 nè — nối từ với định nghĩa. Đọc định nghĩa rồi nối với từ bên trái. Cho 2 phút nha.
</teacher_script>

<task>
**Đề:** Match the words (1–4) with their definitions (a–d).
**Dịch đề:** Nối từ (1–4) với định nghĩa đúng (a–d).
</task>

<questions type="matching">
**Từ:**
1. stilt house
2. terraced fields
3. five-colour sticky rice
4. folk dance

**Định nghĩa:**
- a. stepped fields cut into hillsides, used for farming
- b. a traditional dance of a specific group of people
- c. a house built on posts above the ground
- d. a traditional food made of sticky rice dyed in five different colours
</questions>

<teacher_script pause="0">
Đáp án nè.
</teacher_script>

<answer>
**Bài 3 - Đáp án:** 1-c | 2-a | 3-d | 4-b

**1-c** — *stilt house* = nhà sàn → xây trên cột gỗ cao hơn mặt đất (*built on posts above the ground*).

**2-a** — *terraced fields* = ruộng bậc thang → những thửa ruộng bậc thang cắt vào sườn đồi để trồng lúa (*stepped fields cut into hillsides*).

**3-d** — *five-colour sticky rice* = xôi ngũ sắc → món ăn truyền thống làm từ gạo nếp nhuộm 5 màu (*sticky rice dyed in five different colours*).

**4-b** — *folk dance* = múa dân gian → điệu múa truyền thống của một nhóm dân tộc cụ thể (*traditional dance of a specific group*).
</answer>

<!-- chunk: exercise-4 -->
<teacher_script pause="0">
Bài 4 — điền từ vô chỗ trống. Có 5 từ cho sẵn, điền vô 5 câu. Đọc kỹ từng câu rồi chọn từ phù hợp nha. Cho 2 phút.
</teacher_script>

<task>
**Đề:** Fill in the blanks with the correct words from the box.
**Dịch đề:** Điền từ thích hợp vào chỗ trống.

**Word box:** *folk dance / overlooks / musical instrument / traditional / ethnic groups*
</task>

<questions type="fill_blanks">
**1.** I love mua sap, a popular _______ of the Thai people.
*Mình thích múa sạp, một điệu _______ phổ biến của người Thái.*

**2.** Their beautiful stilt house _______ a large rice field.
*Ngôi nhà sàn đẹp của họ _______ một cánh đồng lúa rộng lớn.*

**3.** A: What is the name of a popular Tay's _______?
B: It's dan tinh.
*Tên của một _______ phổ biến của người Tày là gì? — Đó là đàn tính.*

**4.** Every ethnic group has their own _______ culture.
*Mỗi dân tộc có nền văn hóa _______ riêng của mình.*

**5.** Of the 54 _______ in Viet Nam, the Kinh is the largest.
*Trong số 54 _______ ở Việt Nam, người Kinh là nhóm đông nhất.*
</questions>

<teacher_script pause="0">
Ok đáp án nè. Giải thích từng câu luôn.
</teacher_script>

<answer>
**Bài 4 - Đáp án:** 1. folk dance | 2. overlooks | 3. musical instrument | 4. traditional | 5. ethnic groups

**1. folk dance** — *mua sap* (múa sạp) là một điệu *múa dân gian* (*folk dance*) nổi tiếng của người Thái. Động từ *love* + danh từ chỉ loại hình nghệ thuật.

**2. overlooks** — chủ ngữ là *stilt house* (số ít) nên động từ thêm *-s* → *overlooks*. Nghĩa: *nhìn ra*, *nhìn xuống* một cánh đồng phía dưới.

**3. musical instrument** — *nhạc cụ* = *musical instrument*. Đàn tính là một nhạc cụ (*musical instrument*) truyền thống của người Tày.

**4. traditional** — *truyền thống* = *traditional* (tính từ). Đặt trước danh từ *culture* → *traditional culture*.

**5. ethnic groups** — *các nhóm dân tộc* = *ethnic groups* (số nhiều). Câu nói đến 54 nhóm → dùng số nhiều.
</answer>

<!-- chunk: end -->
<teacher_script pause="0">
Xong bài <eng>Getting Started</eng> rồi nha! Hôm nay mình biết thêm về 54 dân tộc Việt Nam. Ngoài người Kinh, còn có nhiều dân tộc thiểu số đặc sắc lắm: Nùng, Hmông, Chăm, Ê Đê, Ba Na, Khmer — mỗi nhóm có trang phục, nhà ở và truyền thống riêng độc đáo. Tiết sau mình vào <eng>A Closer Look 1</eng> nha. Cất tập lại.
</teacher_script>
