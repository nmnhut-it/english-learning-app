# UNIT 7: ENVIRONMENTAL PROTECTION

## GETTING STARTED - What can we do to protect the environment?

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_fd00461e.mp3">
Chào các em. Chào mừng các em đến với bài học hôm nay. Mời các em mở sách giáo khoa.
</teacher_script>

<teacher_script pause="0" href="audio/tts_c0841176.mp3">
Chúng ta cùng học từ vựng mới. Các em hãy nghe và lặp lại theo mẫu nhé.
</teacher_script>

---

<!-- chunk: vocabulary -->
<vocabulary>
1. **environmental protection** : (n) bảo vệ môi trường /ɪnˌvaɪrənˈmentl prəˈtekʃn/
2. **discuss** : (v) thảo luận /dɪˈskʌs/
3. **environmental problems** : (n) các vấn đề môi trường /ɪnˌvaɪrənˈmentl ˈprɒbləmz/
4. **serious** : (adj) nghiêm trọng /ˈsɪəriəs/
5. **pollution** : (n) ô nhiễm /pəˈluːʃn/
6. **habitat** : (n) môi trường sống /ˈhæbɪtæt/
7. **habitat loss** : (n) mất môi trường sống /ˈhæbɪtæt lɒs/
8. **worse and worse** : (adv) ngày càng tệ hơn /wɜːs ənd wɜːs/
9. **quality** : (n) chất lượng /ˈkwɒlɪti/
10. **global warming** : (n) sự nóng lên toàn cầu /ˌɡləʊbl ˈwɔːmɪŋ/
11. **endangered species** : (n) các loài có nguy cơ tuyệt chủng /ɪnˈdeɪndʒəd ˈspiːʃiːz/
12. **protect** : (v) bảo vệ /prəˈtekt/
13. **reduce** : (v) giảm /rɪˈdjuːs/
14. **carbon footprint** : (n) lượng khí thải carbon /ˈkɑːbən ˈfʊtprɪnt/
15. **carbon dioxide** : (n) khí carbon dioxide /ˌkɑːbən daɪˈɒksaɪd/
16. **release** : (v) thải ra /rɪˈliːs/
17. **amount** : (n) lượng /əˈmaʊnt/
18. **turn off** : (v) tắt /tɜːn ɒf/
19. **device** : (n) thiết bị /dɪˈvaɪs/
20. **practise** : (v) thực hành /ˈpræktɪs/
21. **3Rs** : reduce, reuse, recycle (giảm thiểu, tái sử dụng, tái chế)
22. **avoid** : (v) tránh /əˈvɔɪd/
23. **litter** : (n) rác, (v) xả rác /ˈlɪtə/
24. **volunteer** : (n/v) tình nguyện viên, tình nguyện /ˌvɒlənˈtɪə/
25. **species** : (n) loài /ˈspiːʃiːz/
26. **reuse** : (v) tái sử dụng /ˌriːˈjuːz/
27. **recycle** : (v) tái chế /ˌriːˈsaɪkl/
28. **single-use** : (adj) dùng một lần /ˌsɪŋɡlˈjuːs/
29. **product** : (n) sản phẩm /ˈprɒdʌkt/
30. **dump** : (v) thả, xả /dʌmp/
31. **waste** : (n) chất thải /weɪst/
32. **rubbish/trash** : (n) rác /ˈrʌbɪʃ/ /træʃ/
</vocabulary>


<teacher_script pause="0" action="record" href="audio/tts_9546697f.mp3">
Các em nghe và thực hành theo nhé.
</teacher_script>

---

### Bài 1 trang 72 - Listen and read

<teacher_script pause="0" href="audio/tts_c08c1d7b.mp3">
Bây giờ các em sẽ nghe một đoạn băng. Hãy chú ý lắng nghe để tìm thông tin trả lời câu hỏi.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2023/0613/track-40.mp3">
**Audio:** Listen and read
</audio>

<task>
**Đề:** Listen and read.
**Dịch đề:** Nghe và đọc.
**Yêu cầu:** Nghe audio và đọc theo đoạn hội thoại trong câu lạc bộ môi trường.
**Hướng dẫn:**
- Bước 1: Nghe audio 1 lần để hiểu nội dung chung
- Bước 2: Đọc theo từng câu, chú ý phát âm
- Bước 3: Đọc lại và gạch chân từ mới
</task>

<dialogue>
**Club leader:** Hello. Welcome back. Today we're discussing environmental problems and environmental protection. What are our serious environmental problems now?
**Nam:** Pollution and habitat loss, I think.
**Ann:** I agree. Air and water quality is getting worse and worse.
**Club leader:** Any others?
**Ann:** Yes, I can think of some like global warming, the loss of endangered species...
**Nam:** So what should we do to help protect the environment?
**Club leader:** We can reduce our carbon footprint even in our homes.
**Ann:** What do you mean by "carbon footprint"?
**Club leader:** It's the amount of carbon dioxide that we release into the environment.
**Ann:** I see. So we can do things like turning off devices when we're not using them.
**Club leader:** Exactly... And there are more things we can do, like practising the 3Rs.
**Nam:** We can also plant more trees in our neighbourhood.
**Ann:** And try to avoid using single-use products, like plastic bags, and stop littering.
**Club leader:** Yeah. And we can volunteer for some local environment programmes to save endangered species.
</dialogue>

<translation>
**Trưởng CLB:** Xin chào. Chào mừng trở lại. Hôm nay chúng ta đang thảo luận về vấn đề môi trường và bảo vệ môi trường. Các vấn đề môi trường nghiêm trọng của chúng ta hiện nay là gì?
**Nam:** Ô nhiễm và mất môi trường sống, tôi nghĩ vậy.
**Ann:** Tôi đồng ý. Chất lượng không khí và nước ngày càng trở nên tồi tệ.
**Trưởng CLB:** Còn gì nữa không?
**Ann:** Có, tôi có thể nghĩ đến một số vấn đề như sự nóng lên toàn cầu, sự biến mất của các loài có nguy cơ tuyệt chủng...
**Nam:** Vậy chúng ta nên làm gì để góp phần bảo vệ môi trường?
**Trưởng CLB:** Chúng ta có thể giảm lượng khí thải carbon ngay cả trong nhà của mình.
**Ann:** "Dấu chân carbon" nghĩa là gì?
**Trưởng CLB:** Đó là lượng khí carbon dioxide mà chúng ta thải ra môi trường.
**Ann:** Tôi hiểu rồi. Vậy chúng ta có thể làm những việc như tắt thiết bị khi không sử dụng.
**Trưởng CLB:** Đúng vậy... Và còn nhiều điều chúng ta có thể làm, như thực hành 3R.
**Nam:** Chúng ta cũng có thể trồng nhiều cây hơn trong khu phố của mình.
**Ann:** Và cố gắng tránh sử dụng các sản phẩm dùng một lần, như túi nhựa, và ngừng xả rác.
**Trưởng CLB:** Đúng. Và chúng ta có thể tình nguyện tham gia một số chương trình môi trường địa phương để cứu các loài có nguy cơ tuyệt chủng.
</translation>

<teacher_script pause="240" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<teacher_script pause="60" href="audio/tts_45612be0.mp3">
Ok sửa bài dịch nha. Đối chiếu với bản dịch trên màn hình.
</teacher_script>

---

### Bài 2 trang 73 - Match

<teacher_script pause="60" href="audio/tts_8a5bec91.mp3">
Bài 2 <eng>Match the phrases in A with their meanings in B</eng>. Nối cụm từ với định nghĩa. 1 phút nha. Dạng bài này hay ra thi, cần nhớ kỹ từ vựng.
</teacher_script>

<task>
**Đề:** Match the phrases in A with their meanings in B.
**Dịch đề:** Nối các cụm từ ở cột A với nghĩa ở cột B.
**Yêu cầu:** Đọc lại hội thoại, nối cụm từ với định nghĩa tương ứng.
**Hướng dẫn:**
- Bước 1: Đọc các cụm từ ở cột A
- Bước 2: Đọc các định nghĩa ở cột B
- Bước 3: Nối số với chữ cái tương ứng
</task>

<questions type="matching">
| | Column A | | Column B |
|---|---------|---|----------|
| 1 | reduce our carbon footprint | A | not use items that can only be used once |
| 2 | turn off devices when not using | B | not throw rubbish in public places |
| 3 | plant more trees | C | produce less CO2 |
| 4 | avoid using single-use products | D | save electricity |
| 5 | stop littering | E | make the neighbourhood greener |
</questions>

<translation>
| | Cột A | | Cột B |
|---|-------|---|-------|
| 1 | giảm lượng khí thải carbon | A | không sử dụng đồ dùng một lần |
| 2 | tắt thiết bị khi không dùng | B | không vứt rác nơi công cộng |
| 3 | trồng thêm cây | C | thải ra ít CO2 hơn |
| 4 | tránh sử dụng sản phẩm dùng một lần | D | tiết kiệm điện |
| 5 | ngừng xả rác | E | làm khu phố xanh hơn |
</translation>

<teacher_script pause="0" type="answer" href="audio/tts_3060bec5.mp3">
Đáp án nha.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** 1-C | 2-D | 3-E | 4-A | 5-B
</answer>

<teacher_script pause="0" href="audio/tts_71750885.mp3">
1-C: <eng>Reduce carbon footprint</eng> = <eng>produce less CO2</eng>, giảm lượng CO2 thải ra.
</teacher_script>

<teacher_script pause="0" href="audio/tts_cda971c3.mp3">
2-D: <eng>Turn off devices</eng> = <eng>save electricity</eng>, tắt thiết bị = tiết kiệm điện.
</teacher_script>

<teacher_script pause="0" href="audio/tts_22ec6c9f.mp3">
3-E: <eng>Plant trees</eng> = <eng>make neighbourhood greener</eng>, trồng cây = làm xanh khu phố.
</teacher_script>

<teacher_script pause="0" href="audio/tts_8554deb5.mp3">
4-A: <eng>Avoid single-use</eng> = <eng>not use items used once</eng>, tránh dùng một lần.
</teacher_script>

<teacher_script pause="0" href="audio/tts_afc80412.mp3">
5-B: <eng>Stop littering</eng> = <eng>not throw rubbish</eng>, ngừng xả rác.
</teacher_script>

<explanation>
**Giải thích:**
1. C - Reduce carbon footprint = produce less CO2 (giảm lượng CO2 thải ra)
2. D - Turn off devices = save electricity (tắt thiết bị = tiết kiệm điện)
3. E - Plant trees = make neighbourhood greener (trồng cây = làm xanh khu phố)
4. A - Avoid single-use = not use items used once (tránh dùng một lần)
5. B - Stop littering = not throw rubbish (ngừng xả rác)
</explanation>

---

### Bài 3 trang 73 - Fill in the blanks

<teacher_script pause="60" href="audio/tts_8a40d1b4.mp3">
Bài 3 <eng>Fill in the blanks with words from the conversation</eng>. Điền từ vào chỗ trống. 1 phút nha. Nhớ đọc lại hội thoại để tìm từ chính xác.
</teacher_script>

<task>
**Đề:** Fill in the blanks with words from the conversation.
**Dịch đề:** Điền từ vào chỗ trống bằng từ trong hội thoại.
**Yêu cầu:** Đọc lại hội thoại, tìm từ phù hợp để điền vào chỗ trống.
**Hướng dẫn:**
- Bước 1: Đọc câu cần điền
- Bước 2: Tìm từ trong hội thoại
- Bước 3: Điền từ đúng ngữ pháp
</task>

<questions type="fill_blanks">
**1.** Air and water _______ causes many health problems.
**2.** We can _______ our carbon footprint by using less energy.
**3.** Plastic bags are _______ products. We should avoid using them.
**4.** Your _______ is the amount of CO2 you produce.
**5.** We can volunteer for _______ to protect nature.
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_a43b7c53.mp3">
Ok đáp án nha.
</teacher_script>

<answer>
**Bài 3 - Đáp án:** 1. pollution | 2. reduce | 3. single-use | 4. carbon footprint | 5. environment programmes
</answer>

<teacher_script pause="0" href="audio/tts_684dde8c.mp3">
Câu 1 là <eng>pollution</eng> - ô nhiễm. <eng>Air and water pollution causes health problems</eng>.
</teacher_script>

<teacher_script pause="0" href="audio/tts_61736752.mp3">
Câu 2 là <eng>reduce</eng> - giảm. <eng>We can reduce our carbon footprint</eng>.
</teacher_script>

<teacher_script pause="0" href="audio/tts_fb8781c4.mp3">
Câu 3 là <eng>single-use</eng> - dùng một lần. <eng>Plastic bags are single-use products</eng>.
</teacher_script>

<teacher_script pause="0" href="audio/tts_0991a8c1.mp3">
Câu 4 là <eng>carbon footprint</eng> - lượng khí thải carbon. Cụm từ quan trọng nha, nhớ kỹ.
</teacher_script>

<teacher_script pause="0" href="audio/tts_1d662904.mp3">
Câu 5 là <eng>environment programmes</eng> - chương trình môi trường.
</teacher_script>

<explanation>
**Giải thích:**
1. pollution - "air and water quality" → pollution causes health problems
2. reduce - "We can reduce our carbon footprint"
3. single-use - "avoid using single-use products, like plastic bags"
4. carbon footprint - "the amount of carbon dioxide that we release"
5. environment programmes - "volunteer for some local environment programmes"
</explanation>

---

### Bài 4 trang 73 - Complete the phrases

<teacher_script pause="45" href="audio/tts_6ce8593e.mp3">
Bài 4 <eng>Complete the phrases with the words given</eng>. Hoàn thành các cụm từ. 45 giây thôi, bài này nhanh.
</teacher_script>

<task>
**Đề:** Complete the phrases with the words given.
**Dịch đề:** Hoàn thành các cụm từ với từ cho sẵn.
**Yêu cầu:** Ghép từ để tạo thành cụm từ hoàn chỉnh về môi trường.
</task>

<questions type="fill_blanks">
**Word bank:** *3Rs / water / endangered / plastic / single-use*

**1.** practise the _______
**2.** _______ pollution
**3.** _______ species
**4.** _______ rubbish
**5.** _______ products
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_3060bec5.mp3">
Đáp án nha.
</teacher_script>

<answer>
**Bài 4 - Đáp án:** 1. 3Rs | 2. water | 3. endangered | 4. plastic | 5. single-use
</answer>

<teacher_script pause="0" href="audio/tts_2185048d.mp3">
Câu 1: <eng>practise the 3Rs</eng> - thực hành 3R. <eng>Reduce, reuse, recycle</eng> - giảm thiểu, tái sử dụng, tái chế.
</teacher_script>

<teacher_script pause="0" href="audio/tts_89c2a9ba.mp3">
Câu 2: <eng>water pollution</eng> - ô nhiễm nước.
</teacher_script>

<teacher_script pause="0" href="audio/tts_87b91700.mp3">
Câu 3: <eng>endangered species</eng> - các loài có nguy cơ tuyệt chủng. Cụm từ này hay ra thi lắm nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_b21b42f2.webm">
Câu 4: <eng>plastic rubbish</eng> - rác nhựa.
</teacher_script>

<teacher_script pause="0" href="audio/tts_f607801d.mp3">
Câu 5: <eng>single-use products</eng> - sản phẩm dùng một lần.
</teacher_script>

<explanation>
**Giải thích:**
1. practise the 3Rs - thực hành 3R (reduce, reuse, recycle)
2. water pollution - ô nhiễm nước
3. endangered species - các loài có nguy cơ tuyệt chủng
4. plastic rubbish - rác nhựa
5. single-use products - sản phẩm dùng một lần
</explanation>

---

### Bài 5 trang 73 - Quiz

<teacher_script pause="60" href="audio/tts_916731d8.mp3">
Bài 5 <eng>Quiz: Are you eco-friendly?</eng> Trắc nghiệm: Bạn có thân thiện với môi trường không? Chọn câu trả lời phù hợp với bản thân. 1 phút nha.
</teacher_script>

<task>
**Đề:** Quiz: Are you eco-friendly?
**Dịch đề:** Trắc nghiệm: Bạn có thân thiện với môi trường không?
**Yêu cầu:** Chọn câu trả lời phù hợp với bản thân.
</task>

<questions type="multiple_choice">
**1.** Do you turn off the lights when you leave a room?
- A. Always
- B. Sometimes
- C. Never

*Bạn có tắt đèn khi rời khỏi phòng không?*

**2.** Do you use reusable bags when shopping?
- A. Never
- B. Sometimes
- C. Always

*Bạn có dùng túi tái sử dụng khi mua sắm không?*

**3.** Do you recycle paper and plastic?
- A. Always
- B. Sometimes
- C. Never

*Bạn có tái chế giấy và nhựa không?*

**4.** Do you walk or cycle to school?
- A. Never
- B. Sometimes
- C. Always

*Bạn có đi bộ hoặc đạp xe đến trường không?*

**5.** Do you take short showers to save water?
- A. Never
- B. Always
- C. Sometimes

*Bạn có tắm nhanh để tiết kiệm nước không?*
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_dde6f5e7.mp3">
Nếu chọn nhiều <eng>Always</eng>, bạn rất <eng>eco-friendly</eng>. Nếu chọn nhiều <eng>Sometimes</eng> hoặc <eng>Never</eng>, cần cố gắng thêm nha. Bảo vệ môi trường bắt đầu từ những việc nhỏ.
</teacher_script>

---

<teacher_script pause="0" href="audio/tts_ba688992.mp3">
Bài học hôm nay kết thúc tại đây. Các em nhớ ôn bài và chuẩn bị cho bài học tiếp theo. Tạm biệt các em.
</teacher_script>
