# UNIT 7: ENVIRONMENTAL PROTECTION

## A CLOSER LOOK 1 - Vocabulary & Pronunciation

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_fd00461e.mp3">
Chào các em. Chào mừng các em đến với bài học hôm nay. Mời các em mở sách giáo khoa.
</teacher_script>

---

<!-- chunk: vocabulary -->
<vocabulary>
**Vocabulary:**
1. **ecosystem** : (n) hệ sinh thái /ˈiːkəʊsɪstəm/
2. **pollution** : (n) sự ô nhiễm /pəˈluːʃn/
3. **habitat** : (n) môi trường sống /ˈhæbɪtæt/
4. **environmental protection** : (n) bảo vệ môi trường /ɪnˌvaɪrənˈmentl prəˈtekʃn/
5. **endangered species** : (n) loài có nguy cơ tuyệt chủng /ɪnˈdeɪndʒəd ˈspiːʃiːz/
6. **campfire** : (n) lửa trại /ˈkæmpfaɪə/
7. **cut down** : (v) chặt, đốn (cây) /kʌt daʊn/
</vocabulary>

<teacher_script pause="0" href="audio/tts_c0841176.mp3">
Chúng ta cùng học từ vựng mới. Các em hãy nghe và lặp lại theo mẫu nhé.
</teacher_script>

---

<!-- chunk: exercise_1 -->
### Bài 1 trang 74 - Label pictures

<teacher_script pause="60" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<task>
**Đề:** Label each picture. Tick (✓) the actions that protect the environment.
</task>

<questions type="matching">
**Phrases:**
a. building a campfire
b. cutting down trees
c. picking up rubbish
d. protecting endangered species
e. saving water
</questions>

---

<!-- chunk: answer_1 -->
<teacher_script pause="0" href="audio/tts_aca3c861.mp3">
Đã hết thời gian. Sau đây là đáp án gợi ý. Các em hãy đối chiếu với bài làm của mình nhé.
</teacher_script>

<answer>
**Bài 1 - Đáp án:**
1. **c. picking up rubbish** (nhặt rác) - ✓
2. **d. protecting endangered species** (bảo vệ loài nguy cấp) - ✓
3. **b. cutting down trees** (chặt cây)
4. **e. saving water** (tiết kiệm nước) - ✓
5. **a. building a campfire** (đốt lửa trại)
</answer>

<explanation>
**Giải thích:**
- Hành động bảo vệ môi trường là: c, d, e.
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: exercise_2 -->
### Bài 2 trang 74 - Match words with meanings

<teacher_script pause="60" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<task>
**A:**
1. ecosystem
2. pollution
3. habitat
4. environmental protection
5. endangered species

**B:**
a. The process of making water, air, soil... dirty.
b. The practice of protecting the natural environment.
c. The way living and non-living things in an area are related.
d. Animals facing a high risk of extinction.
e. The place where a type of plant or animal lives.
</task>

---

<!-- chunk: answer_2 -->
<teacher_script pause="0" href="audio/tts_aca3c861.mp3">
Đã hết thời gian. Sau đây là đáp án gợi ý. Các em hãy đối chiếu với bài làm của mình nhé.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** 1.e | 2.a | 3.c | 4.b | 5.d
</answer>

<explanation>
**Giải thích:**
1. **ecosystem** - e (place where plant/animal lives - nhưng sách giáo khoa mới định nghĩa ecosystem rộng hơn, thường là c - "related to each other", còn habitat là e. *Kiểm tra lại kỹ:*
*Theo sách Global Success 8:*
- **Habitat**: The place where a type of plant or animal lives (e).
- **Ecosystem**: The way living and non-living things... are related (c).
*Chỉnh sửa theo đúng đáp án chuẩn:*
**1 - c (Ecosystem)**
**2 - a (Pollution)**
**3 - e (Habitat)**
**4 - b (Environmental protection)**
**5 - d (Endangered species)**
</explanation>

<teacher_script pause="0" href="audio/tts_0dc4eb0d.webm">
Lưu ý: **Hospital** là nơi ở, **Ecosystem** là hệ sinh thái (mối quan hệ giữa các sinh vật).
Đáp án chính xác: 1-c, 2-a, 3-e, 4-b, 5-d.
</teacher_script>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: exercise_3 -->
### Bài 3 trang 74 - Complete sentences

<teacher_script pause="60" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<questions type="fill_blanks">
**Word bank:** *habitat | cutting down trees | ecosystem | endangered species | carbon dioxide*

**1.** People in my neighbourhood are doing a lot to save _______.
**2.** Con Dao National Park provides a rich _______ for marine life.
**3.** Forests help release oxygen and absorb _______; they also provide homes for many species.
**4.** _______ is a serious environmental concern as it harms natural habitats.
**5.** An _______ may be a whole forest, or a small pond, and it can be of any size.
</questions>

<answer>
**Bài 3 - Đáp án:** 1. endangered species | 2. habitat | 3. carbon dioxide | 4. Cutting down trees | 5. ecosystem
</answer>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: pronunciation -->
### Bài 4 trang 74 - Pronunciation /bl/ & /kl/

<teacher_script pause="0" href="audio/tts_9546697f.mp3">
Các em nghe và thực hành theo nhé.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2023/0613/track-41.mp3">
**Audio:** Pronunciation /bl/ and /kl/
</audio>

<task>
**/bl/**: block, blast, blanket, black, problem
**/kl/**: clean, clown, club, class, clear
</task>

---

<!-- chunk: exercise_5 -->
### Bài 5 trang 74 - Listen and practise sentences

<teacher_script pause="0" href="audio/tts_9546697f.mp3">
Các em nghe và thực hành theo nhé.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2023/0613/track-42.mp3">
**Audio:** Practice sentences
</audio>

<reading>
1. Look! There are **black** clouds all over!
2. A truck **blocked** the way to the **club**.
3. The students painted the **classroom** **blue**.
4. The wind **blew** the **clock** down.
5. We **cleaned** up the environment after the **blast**.
</reading>

<teacher_script pause="0" action="record" href="audio/tts_0ac6d7df.webm">
Em hãy ghi âm 5 câu này gửi cho thầy nhé. Cố gắng phát âm rõ /bl/ và /kl/.
</teacher_script>

---

<!-- chunk: additional_practice -->
### Bài tập thực hành bổ sung

<teacher_script pause="0" href="audio/tts_cc297d45.mp3">
Các em làm thêm bài tập thực hành để củng cố từ vựng nha. Bài tập này rất quan trọng đó!
</teacher_script>

---

<!-- chunk: practice_1 -->
### Bài tập 1 - Environmental vocabulary practice

<teacher_script pause="120" href="audio/tts_01a32f98.mp3">
Chúng ta cùng làm Bài tập 1. Các em hãy hoàn thành các câu về bảo vệ môi trường. Các em có 2 phút.
</teacher_script>

<questions type="fill_blanks">
**Word bank:** *pollution | ecosystem | habitat | endangered species | cut down*

**1.** Air _______ is a serious problem in big cities.
*Ô nhiễm không khí là vấn đề nghiêm trọng ở các thành phố lớn.*

**2.** We should protect _______ like pandas and tigers.
*Chúng ta nên bảo vệ các loài nguy cấp như gấu trúc và hổ.*

**3.** Don't _______ trees in the forest.
*Đừng chặt cây trong rừng.*

**4.** The forest is a natural _______ for many animals.
*Khu rừng là môi trường sống tự nhiên cho nhiều động vật.*

**5.** A healthy _______ has many different plants and animals.
*Một hệ sinh thái khỏe mạnh có nhiều loài động thực vật khác nhau.*
</questions>

---

<!-- chunk: answer_practice_1 -->
<teacher_script pause="0" href="audio/tts_120809e0.mp3">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Bài tập 1 - Đáp án:**
1. pollution
2. endangered species
3. cut down
4. habitat
5. ecosystem
</answer>

<teacher_script pause="0" href="audio/tts_4ae98f10.mp3">
Câu 1: <eng>pollution</eng> - ô nhiễm. Air pollution là ô nhiễm không khí, water pollution là ô nhiễm nước.
</teacher_script>

<teacher_script pause="0" href="audio/tts_2f852bf9.mp3">
Câu 2: <eng>endangered species</eng> - loài nguy cấp. Đây là những loài có nguy cơ tuyệt chủng, cần được bảo vệ.
</teacher_script>

<teacher_script pause="0" href="audio/tts_8134381a.mp3">
Câu 3: <eng>cut down</eng> - chặt cây. Chặt cây phá rừng rất có hại cho môi trường nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_259e6796.mp3">
Câu 4: <eng>habitat</eng> - môi trường sống. Mỗi loài động vật có môi trường sống riêng của mình.
</teacher_script>

<teacher_script pause="0" href="audio/tts_894cdb71.mp3">
Câu 5: <eng>ecosystem</eng> - hệ sinh thái. Đây là mối quan hệ giữa các sinh vật trong một khu vực.
</teacher_script>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: practice_2 -->
### Bài tập 2 - Environmental actions

<teacher_script pause="120" href="audio/tts_f45359a3.mp3">
Chúng ta cùng làm Bài tập 2. Các em hãy viết câu về các hành động bảo vệ môi trường. Các em có 2 phút.
</teacher_script>

<questions type="complete_sentences">
**Hoàn thành câu:**

**1.** (pick up rubbish / beach)
→ We should _______________________________________

**2.** (protect / endangered species)
→ It's important to _______________________________________

**3.** (save water / daily life)
→ People should _______________________________________

**4.** (not build campfire / forest)
→ We shouldn't _______________________________________

**5.** (not cut down / trees)
→ We must _______________________________________

**6.** (reduce / pollution)
→ Everyone should _______________________________________
</questions>

---

<!-- chunk: answer_practice_2 -->
<teacher_script pause="0" href="audio/tts_120809e0.mp3">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Bài tập 2 - Đáp án:**
1. We should pick up rubbish on the beach.
2. It's important to protect endangered species.
3. People should save water in daily life.
4. We shouldn't build campfires in the forest.
5. We must not cut down trees.
6. Everyone should reduce pollution.
</answer>

<teacher_script pause="0" href="audio/tts_9a343468.mp3">
Các câu này đều nói về bảo vệ môi trường. Nhặt rác, tiết kiệm nước, bảo vệ động vật quý hiếm - đây là những việc chúng ta cần làm.
</teacher_script>

<teacher_script pause="0" href="audio/tts_b0484e2b.mp3">
Lưu ý: <eng>endangered species</eng> là loài có nguy cơ tuyệt chủng, <eng>pollution</eng> là ô nhiễm, <eng>habitat</eng> là môi trường sống.
</teacher_script>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: practice_3 -->
### Bài tập 3 - True or False

<teacher_script pause="120" href="audio/tts_141cfd15.mp3">
Bài tập cuối cùng. Các em hãy đọc các câu và chọn True (T) hoặc False (F). Các em có 2 phút.
</teacher_script>

<questions type="true_false">
**Đúng (T) hay Sai (F):**

**1.** Cutting down trees is good for the environment. (T / F)
*Chặt cây tốt cho môi trường.*

**2.** We should protect endangered species. (T / F)
*Chúng ta nên bảo vệ các loài nguy cấp.*

**3.** Picking up rubbish helps protect the environment. (T / F)
*Nhặt rác giúp bảo vệ môi trường.*

**4.** Building campfires in the forest is safe. (T / F)
*Đốt lửa trại trong rừng là an toàn.*

**5.** An ecosystem includes only animals. (T / F)
*Hệ sinh thái chỉ bao gồm động vật.*

**6.** Saving water is a way to protect the environment. (T / F)
*Tiết kiệm nước là cách bảo vệ môi trường.*
</questions>

---

<!-- chunk: answer_practice_3 -->
<teacher_script pause="0" href="audio/tts_120809e0.mp3">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Bài tập 3 - Đáp án:**
1. F (False)
2. T (True)
3. T (True)
4. F (False)
5. F (False)
6. T (True)
</answer>

<teacher_script pause="0" href="audio/tts_4183a89c.mp3">
Câu 1: <eng>False</eng>. Chặt cây phá rừng rất có hại cho môi trường, không phải tốt.
</teacher_script>

<teacher_script pause="0" href="audio/tts_71d2cb1c.mp3">
Câu 2: <eng>True</eng>. Chúng ta phải bảo vệ các loài nguy cấp để chúng không bị tuyệt chủng.
</teacher_script>

<teacher_script pause="0" href="audio/tts_64d13d93.mp3">
Câu 3: <eng>True</eng>. Nhặt rác giúp môi trường sạch đẹp hơn.
</teacher_script>

<teacher_script pause="0" href="audio/tts_c8b24025.mp3">
Câu 4: <eng>False</eng>. Đốt lửa trại trong rừng rất nguy hiểm, có thể gây cháy rừng.
</teacher_script>

<teacher_script pause="0" href="audio/tts_70053c5e.mp3">
Câu 5: <eng>False</eng>. Hệ sinh thái bao gồm cả động vật, thực vật và môi trường xung quanh.
</teacher_script>

<teacher_script pause="0" href="audio/tts_adab5a0e.mp3">
Câu 6: <eng>True</eng>. Tiết kiệm nước là một trong những cách bảo vệ tài nguyên thiên nhiên.
</teacher_script>

<teacher_script pause="30" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_ba688992.mp3">
Bài học hôm nay kết thúc tại đây. Các em nhớ ôn bài và chuẩn bị cho bài học tiếp theo. Tạm biệt các em.
</teacher_script>
