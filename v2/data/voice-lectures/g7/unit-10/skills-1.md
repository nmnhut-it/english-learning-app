# UNIT 10: ENERGY SOURCES
## SKILLS 1 - Reading and Speaking

<!-- chunk: vocabulary -->
<teacher_script pause="0">
Ok lớp 7, Unit 10 bài <eng>Skills 1</eng> nha. Bài này mình học <eng>Reading</eng> và <eng>Speaking</eng> về nguồn năng lượng. Bấm nút "Bắt đầu" bên dưới để học từ vựng. Máy sẽ đọc và các em đọc theo. Sau đó làm bài trắc nghiệm từ vựng rồi ghi vô tập nha.
</teacher_script>

<vocabulary>
1. **'d like = would like** : muốn /wʊd laɪk/
2. **energy source** : nguồn năng lượng
3. **non-renewable source** : nguồn không tái tạo
4. **renewable source** : nguồn tái tạo
5. **produce energy** : sản xuất năng lượng
6. **cheap** : (adj) rẻ /tʃiːp/
7. **easy to use** : dễ sử dụng
8. **limited** : (adj) hạn chế /ˈlɪmɪtɪd/
9. **run out** : (v) cạn kiệt
10. **available** : (adj) sẵn có /əˈveɪləbl/
11. **rely on** : (v) phụ thuộc vào
</vocabulary>

<!-- chunk: reading-intro -->
<teacher_script pause="0">
Giờ mình đọc bài đọc nha. Ông Lâm đang giảng bài về các nguồn năng lượng. Đọc và hiểu nội dung nè.
</teacher_script>

### Reading - Mr Lam's Lecture

<reading>
Hello, class. Today I'd like to tell you about two energy sources. They are non-renewable sources and renewable sources.
Non-renewable sources are coal, oil and natural gas. We can use these sources to produce energy. They are cheap and easy to use. People use them a lot. But they are very limited and will run out soon.
Renewable sources come from the sun, wind or water. When energy comes from the sun, we call it solar energy. Wind energy comes from the wind, and hydro energy comes from water. Renewable sources are available, clean and safe to use. But they are expensive to produce.
In the future we will rely more on renewable energy sources. They are better for the environment and they will not run out.
</reading>

<translation>
Xin chào cả lớp. Hôm nay tôi muốn nói với các bạn về hai nguồn năng lượng. Chúng là những nguồn không thể tái tạo và những nguồn có thể tái tạo.
Các nguồn không thể tái tạo là than đá, dầu mỏ và khí đốt tự nhiên. Chúng ta có thể sử dụng những nguồn này để sản xuất năng lượng. Chúng rẻ và dễ sử dụng. Mọi người sử dụng chúng rất nhiều. Nhưng chúng rất hạn chế và sẽ hết sớm.
Các nguồn tái tạo đến từ mặt trời, gió hoặc nước. Khi năng lượng đến từ mặt trời, chúng ta gọi nó là năng lượng mặt trời. Năng lượng gió đến từ gió, và năng lượng thủy điện đến từ nước. Nguồn tái tạo có sẵn, sạch sẽ và an toàn để sử dụng. Nhưng chúng đắt tiền để sản xuất.
Trong tương lai, chúng ta sẽ dựa nhiều hơn vào các nguồn năng lượng tái tạo. Chúng tốt hơn cho môi trường và chúng sẽ không cạn kiệt.
</translation>

<teacher_script pause="180">
Các em đọc bài và dịch vô vở nha. 3 phút.
</teacher_script>

<!-- chunk: exercise-1 -->
### Bài 1 trang 110 - Discussion

<teacher_script pause="0">
Bài 1 <eng>Work in pairs. Discuss the following questions</eng>. Làm việc theo cặp và thảo luận các câu hỏi sau nha.
</teacher_script>

<task>
**Instruction:** Work in pairs. Discuss the following questions.

**Dịch:** Làm việc theo cặp. Thảo luận các câu hỏi sau.
</task>

<questions type="speaking">
**1.** What are the main energy sources in Viet Nam?

*Các nguồn năng lượng chính ở Việt Nam là gì?*

**2.** What type(s) of energy sources will we use in the future?

*Chúng ta sẽ sử dụng (những) loại nguồn năng lượng nào trong tương lai?*
</questions>

<teacher_script pause="60">
Các em suy nghĩ và trả lời nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Bài 1 - Đáp án:**

**1.** They are hydro energy and energy from coal.

*(Chúng là năng lượng thủy điện và năng lượng từ than đá.)*

**2.** We will use solar energy and wind energy.

*(Chúng tôi sẽ sử dụng năng lượng mặt trời và năng lượng gió.)*
</answer>

<explanation>
Câu 1: Nguồn năng lượng chính ở Việt Nam là năng lượng thủy điện (<eng>hydro energy</eng>) và năng lượng từ than đá (<eng>energy from coal</eng>).

Câu 2: Trong tương lai chúng ta sẽ sử dụng năng lượng mặt trời (<eng>solar energy</eng>) và năng lượng gió (<eng>wind energy</eng>) vì đây là nguồn năng lượng tái tạo, tốt cho môi trường.
</explanation>

<!-- chunk: exercise-2 -->
### Bài 2 trang 110 - Multiple Choice

<teacher_script pause="0">
Bài 2 nè. <eng>Mr Lam is giving a lecture on energy sources. Read the text and choose the best option (A, B, or C) to complete the sentences</eng>. Ông Lâm đang giảng bài về các nguồn năng lượng. Đọc văn bản và chọn phương án tốt nhất A, B hoặc C để hoàn thành các câu nha.
</teacher_script>

<task>
**Instruction:** Mr Lam is giving a lecture on energy sources. Read the text and choose the best option (A, B, or C) to complete the sentences.

**Dịch:** Ông Lâm đang giảng bài về các nguồn năng lượng. Đọc văn bản và chọn phương án tốt nhất (A, B hoặc C) để hoàn thành các câu.
</task>

<questions type="multiple_choice">
**1.** Non-renewable sources are cheap and _______.

- A. available
- B. easy to use
- C. expensive

*Nguồn không tái tạo được rẻ và _______.*

**2.** _______ come from the sun, wind or water.

- A. Renewable sources
- B. All energy sources
- C. Non-renewable sources

*_______ nhận được từ mặt trời, gió hoặc nước.*

**3.** When energy comes from water, we call it _______.

- A. wind energy
- B. solar energy
- C. hydro energy

*Khi năng lượng đến từ nước, chúng ta gọi nó là _______.*

**4.** Renewable energy sources are better for _______.

- A. the environment
- B. our cars
- C. hydro energy

*Các nguồn năng lượng tái tạo tốt hơn cho _______.*
</questions>

<teacher_script pause="60">
Các em làm bài nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** 1.B | 2.A | 3.C | 4.A
</answer>

<explanation>
**Câu 1 - Đáp án B:** <eng>Non-renewable sources are cheap and easy to use</eng>. Nguồn không tái tạo được rẻ và dễ sử dụng. Trong bài đọc có câu: "They are cheap and easy to use."

**Câu 2 - Đáp án A:** <eng>Renewable sources come from the sun, wind or water</eng>. Các nguồn năng lượng có thể tái tạo đến từ mặt trời, gió và nước. Trong bài đọc: "Renewable sources come from the sun, wind or water."

**Câu 3 - Đáp án C:** <eng>When energy comes from water, we call it hydro energy</eng>. Khi năng lượng đến từ nước, chúng ta gọi nó là năng lượng thủy điện. Trong bài đọc: "hydro energy comes from water."

**Câu 4 - Đáp án A:** <eng>Renewable energy sources are better for the environment</eng>. Các nguồn năng lượng có thể tái tạo thì tốt hơn cho môi trường. Trong bài đọc: "They are better for the environment and they will not run out."
</explanation>

<!-- chunk: exercise-3 -->
### Bài 3 trang 110 - Answer Questions

<teacher_script pause="0">
Bài 3 nè. <eng>Read the text again and answer the questions</eng>. Đọc lại văn bản và trả lời các câu hỏi nha.
</teacher_script>

<task>
**Instruction:** Read the text again and answer the questions.

**Dịch:** Đọc lại văn bản và trả lời các câu hỏi.
</task>

<questions type="complete_sentences">
**1.** How many energy sources are there? What are they?

*Có bao nhiêu nguồn năng lượng? Chúng là gì?*

**2.** What do non-renewable sources include?

*Các nguồn không tái tạo bao gồm những gì?*

**3.** What are the advantages of renewable energy?

*Ưu điểm của năng lượng tái tạo là gì?*

**4.** What will we rely more on in the future?

*Chúng ta sẽ dựa vào điều gì nhiều hơn trong tương lai?*
</questions>

<teacher_script pause="90">
Các em đọc bài và trả lời các câu hỏi nha. 1 phút rưỡi.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Bài 3 - Đáp án:**

**1.** There are two energy sources. They are non-renewable sources and renewable sources.

*(Có hai nguồn năng lượng. Chúng là những nguồn không thể tái tạo và những nguồn có thể tái tạo.)*

**2.** Non-renewable sources are coal, oil and natural gas.

*(Các nguồn không thể tái tạo là than đá, dầu mỏ và khí đốt tự nhiên.)*

**3.** Renewable sources are available, clean and safe to use. But they are expensive to produce.

*(Nguồn tái tạo có sẵn, sạch sẽ và an toàn để sử dụng. Nhưng chúng đắt tiền để sản xuất.)*

**4.** In the future we will rely more on renewable energy sources.

*(Trong tương lai, chúng tôi sẽ dựa nhiều hơn vào các nguồn năng lượng tái tạo.)*
</answer>

<explanation>
Tất cả các câu trả lời đều lấy trực tiếp từ bài đọc nha các em.

Câu 1 tìm ở đầu bài: "Today I'd like to tell you about two energy sources. They are non-renewable sources and renewable sources."

Câu 2 tìm ở đoạn 2: "Non-renewable sources are coal, oil and natural gas."

Câu 3 tìm ở đoạn 3: "Renewable sources are available, clean and safe to use."

Câu 4 tìm ở đoạn cuối: "In the future we will rely more on renewable energy sources."
</explanation>

<!-- chunk: exercise-4 -->
### Bài 4 trang 110 - Speaking: Advantages and Disadvantages

<teacher_script pause="0">
Giờ qua phần <eng>Speaking</eng> nha. Bài 4, <eng>Work in groups. Discuss and put the following words or phrases in the appropriate columns</eng>. Làm việc theo nhóm. Thảo luận và đặt các từ hoặc cụm từ sau vào các cột thích hợp nha.
</teacher_script>

<task>
**Instruction:** Work in groups. Discuss and put the following words or phrases in the appropriate columns.

**Dịch:** Làm việc theo nhóm. Thảo luận và đặt các từ hoặc cụm từ sau vào các cột thích hợp.

**Word bank:** run out | limited | easy to use | cheap | expensive | available | safe to use | good for the environment
</task>

<questions type="matching">
| **Advantages** *(Thuận lợi)* | **Disadvantages** *(Bất lợi)* |
|------------------------------|-------------------------------|
| ? | ? |
</questions>

<teacher_script pause="60">
Các em phân loại các từ nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Bài 4 - Đáp án:**

| **Advantages** *(Thuận lợi)* | **Disadvantages** *(Bất lợi)* |
|------------------------------|-------------------------------|
| easy to use *(dễ dàng sử dụng)* | run out *(cạn kiệt)* |
| cheap *(rẻ)* | limited *(có giới hạn)* |
| available *(sẵn có)* | expensive *(đắt tiền)* |
| safe to use *(an toàn sử dụng)* | |
| good for the environment *(tốt cho môi trường)* | |
</answer>

<explanation>
**Advantages** (Thuận lợi) là những đặc điểm tốt, có lợi: rẻ, dễ dùng, sẵn có, an toàn, tốt cho môi trường.

**Disadvantages** (Bất lợi) là những đặc điểm xấu, có hại: cạn kiệt, hạn chế, đắt tiền.

Nhớ nha: <eng>run out</eng> là cạn kiệt - đó là bất lợi vì năng lượng sẽ hết!
</explanation>

<!-- chunk: exercise-5 -->
### Bài 5 trang 110 - Speaking: Ask and Answer

<teacher_script pause="0">
Bài 5 nè. <eng>Work in pairs. Ask and answer questions about the advantages and disadvantages of different energy sources</eng>. Làm việc theo cặp. Hỏi và trả lời các câu hỏi về ưu điểm và nhược điểm của các nguồn năng lượng khác nhau nha.
</teacher_script>

<task>
**Instruction:** Work in pairs. Ask and answer questions about the advantages and disadvantages of different energy sources.

**Dịch:** Làm việc theo cặp. Hỏi và trả lời các câu hỏi về ưu điểm và nhược điểm của các nguồn năng lượng khác nhau.
</task>

<questions type="speaking">
**Example dialogue:**

| English | Vietnamese |
|---------|------------|
| **A:** What are the advantages of hydro energy? | **A:** Những lợi thế của năng lượng thủy điện là gì? |
| **B:** It's available, clean and safe to use. | **B:** Nó có sẵn, sạch sẽ và an toàn để sử dụng. |
| **A:** What are its disadvantages? | **A:** Nhược điểm của nó là gì? |
| **B:** It's expensive to produce. | **B:** Nó đắt tiền để sản xuất. |
</questions>

<teacher_script pause="0" action="record">
Các em thực hành hỏi và trả lời theo mẫu nha. Ghi âm lại bài nói của mình.
</teacher_script>

<teacher_script pause="0">
Đây là một mẫu hội thoại khác về năng lượng từ than đá nè.
</teacher_script>

<answer>
**Bài 5 - Sample dialogue - Energy from coal:**

| English | Vietnamese |
|---------|------------|
| **A:** What are the advantages of energy from coal? | **A:** Ưu điểm của năng lượng từ than đá là gì? |
| **B:** It's cheap and easy to use. | **B:** Nó rẻ và dễ sử dụng. |
| **A:** What are its disadvantages? | **A:** Nhược điểm của nó là gì? |
| **B:** It's limited and not good for the environment. | **B:** Nó hạn chế và không tốt cho môi trường. |
</answer>

<explanation>
Khi nói về ưu điểm và nhược điểm, các em dùng cấu trúc:

- <eng>What are the advantages of...?</eng> = Ưu điểm của... là gì?
- <eng>What are its disadvantages?</eng> = Nhược điểm của nó là gì?

Nhớ phân biệt:
- <eng>Renewable sources</eng> (năng lượng tái tạo): available, clean, safe to use, good for environment, nhưng expensive.
- <eng>Non-renewable sources</eng> (năng lượng không tái tạo): cheap, easy to use, nhưng limited, will run out, not good for environment.
</explanation>

<!-- chunk: end -->
<teacher_script pause="0">
Ok vậy là xong bài <eng>Skills 1</eng> Unit 10 rồi nha. Các em đã học về các nguồn năng lượng tái tạo và không tái tạo, ưu điểm và nhược điểm của chúng. Nhớ ôn lại từ vựng và luyện nói hen. Bài sau mình học <eng>Skills 2</eng> về <eng>Listening</eng> và <eng>Writing</eng>. Tạm biệt các em!
</teacher_script>
