# UNIT 11: TRAVELLING IN THE FUTURE
## SKILLS 1 - Reading and Speaking

<!-- chunk: vocabulary -->
<teacher_script pause="0">
Ok lớp 7, Unit 11 bài <eng>Skills 1</eng> nha. Bài này mình học <eng>Reading</eng> và <eng>Speaking</eng> về xe điện trong tương lai. Bấm nút "Bắt đầu" bên dưới để học từ vựng. Máy sẽ đọc và các em đọc theo. Sau đó làm bài trắc nghiệm từ vựng rồi ghi vô tập nha.
</teacher_script>

<vocabulary>
1. **electric car** : (n.phr) ô tô điện /ɪˈlektrɪk kɑː/
2. **introduce** : (v) giới thiệu /ˌɪntrəˈdjuːs/
3. **comfortable** : (adj) thoải mái /ˈkʌmftəbl/
4. **charge** : (v) sạc /tʃɑːdʒ/
5. **battery** : (n) pin /ˈbætəri/
6. **gaming screen** : (n.phr) màn hình trò chơi /ˈɡeɪmɪŋ skriːn/
7. **economical** : (adj) tiết kiệm /ˌiːkəˈnɒmɪkl/
8. **autopilot function** : (n.phr) chức năng tự lái /ˈɔːtəʊˌpaɪlət ˈfʌŋkʃn/
9. **driverless** : (adj) không người lái /ˈdraɪvləs/
10. **passenger** : (n) hành khách /ˈpæsɪndʒə/
</vocabulary>

<!-- chunk: reading -->
<teacher_script pause="0">
Giờ mình đọc bài đọc nha. Bài đọc về công ty xe điện Roadrunner. Đọc và hiểu nội dung nè.
</teacher_script>

### Reading - Roadrunner

<reading>
| English | Vietnamese |
|---------|------------|
| Roadrunner is a car company that makes electric cars. The company introduced the first model in 2015 and they named it Speed. Speed soon became a success. Speed is safe, fast, comfortable, and not very expensive. It has four seats. The car can travel over 350 kilometres per hour. You only have to charge the battery every 700 kilometres. It even has a gaming screen inside. | Roadrunner là một công ty sản xuất ô tô điện. Công ty đã giới thiệu mô hình đầu tiên vào năm 2015 và họ đặt tên cho nó là Speed (Tốc độ). Speed sớm thành công. Speed an toàn, nhanh chóng, thoải mái và không tốn kém nhiều. Nó có bốn chỗ ngồi. Xe có thể di chuyển trên 350 km/h. Bạn chỉ phải sạc pin sau mỗi 700 km. Nó thậm chí còn có một màn hình chơi game bên trong xe. |
| The company will introduce a new model next year called Safety. This model will become the largest and safest electric car you can buy. It will be able to carry seven passengers. The new model will also be more economical. | Công ty sẽ giới thiệu một mô hình mới vào năm tới có tên là Safety (An toàn). Mẫu xe này sẽ trở thành chiếc xe điện lớn nhất và an toàn nhất mà bạn có thể mua. Nó sẽ có thể chở bảy hành khách. Mô hình mới cũng sẽ tiết kiệm hơn. |
| Both models have an autopilot function, so they are driverless. Passengers can read, play games, or even sleep while they travel. | Cả hai mô hình đều có chức năng lái tự động, vì vậy chúng không cần người lái. Hành khách có thể đọc, chơi trò chơi hoặc thậm chí ngủ trong khi đi du lịch. |
</reading>

<teacher_script pause="180">
Các em đọc bài và dịch vô vở nha. 3 phút.
</teacher_script>

<!-- chunk: exercise-1 -->
### Bài 1 trang 119 - Speaking

<teacher_script pause="0">
Bài 1 <eng>Work in pairs. Look at the picture and find the words or phrases to describe it</eng>. Nhìn hình trong sách và tìm các từ hoặc cụm từ để mô tả nó nha.
</teacher_script>

<task>
**Instruction:** Work in pairs. Look at the picture and find the words or phrases to describe it.

**Dịch:** Làm việc theo cặp. Nhìn vào bức tranh và tìm các từ hoặc cụm từ để mô tả nó.
</task>

<questions type="speaking">
Nhìn hình trong sách và mô tả bức tranh bằng tiếng Anh.

*Nhìn hình trong sách nha.*
</questions>

<teacher_script pause="60">
Các em suy nghĩ và trả lời nha. 1 phút.
</teacher_script>

<teacher_script pause="0">
Ok đáp án nè.
</teacher_script>

<answer>
I think she is using a driverless car with a smart navigation system.

*(Tôi nghĩ cô ấy đang sử dụng một chiếc xe không người lái với hệ thống định vị thông minh.)*
</answer>

<explanation>
Nhìn hình trong sách, mình thấy cô gái đang ngồi trong xe mà không cần lái. Xe tự chạy nên gọi là <eng>driverless car</eng> (xe không người lái), và có hệ thống định vị thông minh gọi là <eng>smart navigation system</eng>.
</explanation>

<!-- chunk: exercise-2 -->
### Bài 2 trang 119 - Multiple Choice

<teacher_script pause="0">
Bài 2 nè. <eng>Read the passage and answer the question: What is its main idea?</eng> Đọc đoạn văn và trả lời câu hỏi: Ý chính của bài là gì? Chọn A, B hoặc C nha.
</teacher_script>

<task>
**Instruction:** Read the passage and answer the question: What is its main idea?

**Dịch:** Đọc đoạn văn và trả lời câu hỏi: Ý chính của bài là gì?
</task>

<questions type="multiple_choice">
**What is its main idea?**

- A. Roadrunner is a success.
- B. Roadrunner is a car company.
- C. Roadrunner's cars have an autopilot function.

*Ý chính của bài là gì?*
*A. Roadrunner là một thành công.*
*B. Roadrunner là một công ty ô tô.*
*C. Ô tô của Roadrunner có chức năng lái tự động.*
</questions>

<teacher_script pause="45">
Các em đọc lại bài và chọn đáp án nha. 45 giây.
</teacher_script>

<teacher_script pause="0">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** A
</answer>

<explanation>
**Đáp án A:** <eng>Roadrunner is a success</eng>. Bài đọc giới thiệu về các sản phẩm thành công của Roadrunner - xe Speed đã thành công, xe Safety sắp ra mắt cũng rất tốt. Nên ý chính là Roadrunner là một thành công.

Đáp án B chỉ nói Roadrunner là công ty xe hơi - đúng nhưng quá chung chung, không phải ý chính.

Đáp án C nói về chức năng tự lái - đúng nhưng chỉ là một chi tiết nhỏ trong bài, không phải ý chính.
</explanation>

<!-- chunk: exercise-3 -->
### Bài 3 trang 119 - Fill in the Blanks

<teacher_script pause="0">
Bài 3 nè. <eng>Read the passage again and complete the fact file with no more than TWO words or a number</eng>. Đọc lại đoạn văn và hoàn thành bảng thông tin với không quá hai từ hoặc một số nha.
</teacher_script>

<task>
**Instruction:** Read the passage again and complete the fact file with no more than TWO words or a number.

**Dịch:** Đọc lại đoạn văn và hoàn thành tệp dữ kiện với không quá HAI từ hoặc một số.
</task>

<questions type="fill_blanks">
| **Fact File** | |
|---|---|
| **Name of the company** | Roadrunner |
| **Speed was introduced in** | (1) ________ |
| **Safety will be the** | (2) ________ and (3) ________ electric car |
| **Both models have** | a gaming screen and an (4) ________ |
| **Both models run on** | (5) ________ |
| **Passengers can** | read, sleep, play games |
</questions>

<teacher_script pause="60">
Các em đọc lại bài và điền vô bảng nha. 1 phút.
</teacher_script>

<teacher_script pause="0">
Ok đáp án nè.
</teacher_script>

<answer>
| **Fact File** | |
|---|---|
| **Name of the company** | Roadrunner |
| **Speed was introduced in** | (1) **2015** |
| **Safety will be the** | (2) **largest** and (3) **safest** electric car |
| **Both models have** | a gaming screen and an (4) **autopilot function** |
| **Both models run on** | (5) **electricity** |
| **Passengers can** | read, sleep, play games |
</answer>

<explanation>
**Câu 1:** Trong bài đọc có câu: "The company introduced the first model in 2015" nên đáp án là <eng>2015</eng>.

**Câu 2 và 3:** Trong bài có câu: "This model will become the largest and safest electric car you can buy" nên đáp án là <eng>largest</eng> và <eng>safest</eng>.

**Câu 4:** Trong bài có câu: "Both models have an autopilot function" nên đáp án là <eng>autopilot function</eng>.

**Câu 5:** Bài nói về <eng>electric cars</eng> (xe điện) nên cả hai mẫu xe đều chạy bằng <eng>electricity</eng> (điện). Trong bài có câu: "Roadrunner is a car company that makes electric cars" và "You only have to charge the battery every 700 kilometres" - sạc pin nghĩa là chạy bằng điện.
</explanation>

<!-- chunk: exercise-4 -->
### Bài 4 trang 119 - Speaking: Ask and Answer

<teacher_script pause="0">
Giờ qua phần <eng>Speaking</eng> nha. Bài 4, <eng>Work in pairs. Look at the fact file in 3 then ask and answer questions about it</eng>. Làm việc theo cặp. Nhìn vào bảng thông tin ở bài 3 rồi hỏi và trả lời các câu hỏi về nó nha.
</teacher_script>

<task>
**Instruction:** Work in pairs. Look at the fact file in 3 then ask and answer questions about it.

**Dịch:** Làm việc theo cặp. Nhìn vào tệp thông tin trong bài 3 rồi hỏi và trả lời các câu hỏi về nó.
</task>

<questions type="speaking">
**Example dialogue:**

| English | Vietnamese |
|---------|------------|
| **A:** What is the name of the company? | **A:** Tên của công ty là gì? |
| **B:** Roadrunner. | **B:** Roadrunner. |
| **A:** When did the company introduce its first model? | **A:** Khi nào công ty giới thiệu mô hình đầu tiên của mình? |
| **B:** In 2015. | **B:** Vào năm 2015. |
</questions>

<teacher_script pause="0" action="record">
Các em thực hành hỏi và trả lời theo mẫu nha. Ghi âm lại bài nói của mình.
</teacher_script>

<teacher_script pause="0">
Đây là thêm một số câu hỏi và trả lời mẫu nè.
</teacher_script>

<answer>
**Sample dialogue:**

| English | Vietnamese |
|---------|------------|
| **A:** What will Safety be? | **A:** Safety sẽ như thế nào? |
| **B:** It will be the largest and safest electric car. | **B:** Nó sẽ là chiếc xe điện lớn nhất và an toàn nhất. |
| **A:** What will both models have? | **A:** Cả hai mô hình sẽ có những gì? |
| **B:** They will have a gaming screen and an autopilot function. | **B:** Chúng sẽ có màn hình chơi game và chức năng lái tự động. |
| **A:** What will both models run on? | **A:** Cả hai mô hình sẽ chạy bằng cái gì? |
| **B:** Electricity. | **B:** Điện. |
| **A:** What can passengers do? | **A:** Hành khách có thể làm gì? |
| **B:** Read, sleep, play games. | **B:** Đọc, ngủ, chơi game. |
</answer>

<explanation>
Các em dựa vào bảng <eng>Fact File</eng> ở bài 3 để hỏi và trả lời nha. Mỗi dòng trong bảng là một câu hỏi. Nhớ dùng <eng>will</eng> khi nói về tương lai (Safety will be...) và dùng thì quá khứ khi nói về Speed (Speed was introduced in 2015).
</explanation>

<!-- chunk: exercise-5 -->
### Bài 5 trang 119 - Speaking: Discussion

<teacher_script pause="0">
Bài 5 nè. <eng>Work in groups. Discuss why Roadrunner's cars will or won't become popular in the near future. Report your reasons to the class</eng>. Làm việc nhóm. Thảo luận tại sao xe của Roadrunner sẽ hoặc sẽ không trở nên phổ biến trong tương lai gần nha.
</teacher_script>

<task>
**Instruction:** Work in groups. Discuss why Roadrunner's cars will or won't become popular in the near future. Report your reasons to the class.

**Dịch:** Làm việc nhóm. Thảo luận tại sao những chiếc xe của Roadrunner sẽ hoặc sẽ không trở nên phổ biến trong tương lai gần. Báo cáo lý do của bạn với lớp.
</task>

<questions type="speaking">
**Example:**

- We think Roadrunner's cars will become popular in the near future because they are fast...

*(Chúng tôi nghĩ rằng những chiếc xe của Roadrunner sẽ trở nên phổ biến trong tương lai gần vì chúng nhanh ...)*

- We think Roadrunner's cars won't become popular in the near future because they might be dangerous / expensive...

*(Chúng tôi nghĩ rằng những chiếc xe của Roadrunner sẽ không trở nên phổ biến trong tương lai gần vì chúng có thể nguy hiểm / đắt tiền ...)*
</questions>

<teacher_script pause="0" action="record">
Các em thảo luận nhóm rồi ghi âm lại ý kiến của mình nha.
</teacher_script>

<teacher_script pause="0">
Đây là một số câu trả lời mẫu nè.
</teacher_script>

<answer>
- We think Roadrunner's cars will become popular in the near future because they are eco-friendly.

*(Chúng tôi nghĩ rằng xe của Roadrunner sẽ trở nên phổ biến trong tương lai gần vì chúng thân thiện với môi trường.)*

- We think Roadrunner's cars won't become popular in the near future because they might be dependent on battery.

*(Chúng tôi nghĩ rằng ô tô của Roadrunner sẽ không trở nên phổ biến trong tương lai gần vì chúng có thể phụ thuộc vào pin.)*

- We think Roadrunner's cars will become popular in the near future because they allow passengers read, sleep, play games.

*(Chúng tôi nghĩ xe của Roadrunner sẽ trở nên phổ biến trong tương lai gần vì chúng cho phép hành khách đọc sách, ngủ, chơi trò chơi.)*
</answer>

<explanation>
Khi thảo luận, các em dùng cấu trúc:

- <eng>We think... will become popular because...</eng> = Chúng tôi nghĩ... sẽ trở nên phổ biến vì...
- <eng>We think... won't become popular because...</eng> = Chúng tôi nghĩ... sẽ không trở nên phổ biến vì...

Lý do xe sẽ phổ biến: thân thiện với môi trường (<eng>eco-friendly</eng>), nhanh, an toàn, không cần người lái, hành khách có thể nghỉ ngơi.

Lý do xe sẽ không phổ biến: phụ thuộc vào pin (<eng>dependent on battery</eng>), có thể đắt tiền, có thể nguy hiểm nếu hệ thống tự lái lỗi.
</explanation>

<!-- chunk: end -->
<teacher_script pause="0">
Ok vậy là xong bài <eng>Skills 1</eng> Unit 11 rồi nha. Các em đã học về công ty xe điện Roadrunner với hai mẫu xe Speed và Safety. Nhớ ôn lại từ vựng và luyện nói hen. Bài sau mình học <eng>Skills 2</eng> về <eng>Listening</eng> và <eng>Writing</eng>. Tạm biệt các em!
</teacher_script>
