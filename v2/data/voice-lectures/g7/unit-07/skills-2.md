# UNIT 7: TRAFFIC

## SKILLS 2 - Listening & Writing

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_be2b8ef5.webm">
Chào các em. Mời các em mở sách giáo khoa trang 15. Bài học hôm nay là <eng>Skills 2</eng>, chúng ta sẽ luyện tập hai kỹ năng Nghe và Viết về vấn đề giao thông.
</teacher_script>

---

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_89dec682.webm">
Trước khi bắt đầu bài nghe, chúng ta cùng học một số từ vựng mới. Các em hãy nghe và đọc theo, sau đó làm bài tập để ghi nhớ nhé.
</teacher_script>

<vocabulary>
**Từ vựng bài Listening:**
1. **traffic problem** : (n) vấn đề giao thông /ˈtræfɪk ˈprɒbləm/
2. **congested** : (adj) tắc nghẽn, đông đúc /kənˈdʒestɪd/
3. **one of the most congested cities** : một trong những thành phố tắc nghẽn nhất
4. **happen** : (v) xảy ra /ˈhæpən/
5. **several** : (adj) vài, một số /ˈsevrəl/
6. **reason** : (n) lý do /ˈriːzən/
7. **increase** : (n) sự tăng lên; (v) tăng lên /ˈɪnkriːs/
8. **population** : (n) dân số /ˌpɒpjuˈleɪʃən/
9. **narrow** : (adj) hẹp /ˈnærəʊ/
10. **bumpy** : (adj) gồ ghề /ˈbʌmpi/
11. **in good condition** : trong tình trạng tốt
12. **obey the traffic rules** : tuân thủ luật giao thông
13. **as a result** : kết quả là
14. **getting worse and worse** : ngày càng tệ hơn

**Từ vựng bài Writing:**
15. **wild animals** : (n) động vật hoang dã /waɪld ˈænɪməlz/
16. **public transportation** : (n) phương tiện giao thông công cộng /ˈpʌblɪk ˌtrænspɔːˈteɪʃn/
17. **private car** : (n) xe riêng /ˈpraɪvət kɑː/
18. **reduce** : (v) giảm /rɪˈdjuːs/
</vocabulary>

<teacher_script pause="90" href="audio/tts_03161635.webm">
Các em hãy dành 1 phút 30 giây để nghe phát âm và ghi chép từ vựng vào vở nhé.
</teacher_script>

<teacher_script pause="0" action="record" href="audio/tts_e4cf0019.webm">
Các em hãy bấm nút Ghi âm để đọc từ vựng và gửi cho thầy nhé.
</teacher_script>

---

<!-- chunk: listening_intro -->
### Listening

<teacher_script pause="0" href="audio/tts_036536d9.webm">
Chúng ta cùng bắt đầu với phần <eng>Listening</eng>. Các em sẽ nghe về vấn đề giao thông ở Mumbai, Ấn Độ.
</teacher_script>

---

<!-- chunk: exercise_1 -->
### Bài 1 trang 15 - Look at the picture

<teacher_script pause="30" href="audio/tts_c9daddc1.webm">
Chúng ta cùng làm Bài 1 <eng>Look at the picture</eng>. Các em hãy quan sát bức tranh và cho biết điều gì đặc biệt trong đó. Các em có 30 giây suy nghĩ.
</teacher_script>

<task>
**Đề:** What can you see in this picture? What is special about it?
**Dịch đề:** Bạn thấy gì trong hình này? Có gì đặc biệt?
</task>

<teacher_script pause="0" href="audio/tts_7c7d2388.webm">
Trong hình là đường phố ở Mumbai, Ấn Độ. Rất đông xe, kẹt cứng luôn!
Mumbai là một trong những thành phố tắc nghẽn nhất thế giới đó.
</teacher_script>

<answer>
**Trả lời:**
I can see a busy street with heavy traffic. There are many vehicles: cars, buses, motorbikes. The special thing is that the traffic is very congested - vehicles can hardly move.

*(Tôi thấy một con đường đông đúc với giao thông dày đặc. Có nhiều phương tiện: ô tô, xe buýt, xe máy. Điều đặc biệt là giao thông rất tắc nghẽn - xe cộ hầu như không di chuyển được.)*
</answer>

---

<!-- chunk: tapescript -->
<teacher_script pause="0" href="audio/tts_1eb71222.webm">
Đây là nội dung bài nghe. Các em hãy đọc trước để hiểu nội dung nhé.
</teacher_script>

<reading>
Big cities around the world have traffic problems. Mumbai in India is one of them.
Mumbai is one of the most congested cities in the world.
Traffic jams happen nearly every day, so drivers have to spend a lot of time on the road.
There are several reasons for traffic jams in this city.
One reason is its increase in population.
With nearly 20 million, Mumbai has too many people on the road.
Another reason is that the roads in Mumbai are narrow, and many are not in good condition.
Also, many road users do not obey the traffic rules.
As a result, this problem is getting worse and worse.
</reading>

<translation>
Các thành phố lớn trên thế giới đều có vấn đề giao thông. Mumbai ở Ấn Độ là một trong số đó.
Mumbai là một trong những thành phố tắc nghẽn nhất thế giới.
Tắc đường xảy ra gần như mỗi ngày, nên tài xế phải mất nhiều thời gian trên đường.
Có vài lý do cho tình trạng tắc đường ở thành phố này.
Một lý do là sự gia tăng dân số.
Với gần 20 triệu dân, Mumbai có quá nhiều người trên đường.
Một lý do khác là đường ở Mumbai hẹp, và nhiều con đường không trong tình trạng tốt.
Ngoài ra, nhiều người tham gia giao thông không tuân thủ luật giao thông.
Kết quả là, vấn đề này ngày càng tệ hơn.
</translation>

<teacher_script pause="60" href="audio/tts_a8150dfc.webm">
Các em hãy dành 1 phút để đọc bài nghe nhé.
</teacher_script>

---

<!-- chunk: exercise_2 -->
### Bài 2 trang 15 - Listen and choose

<audio src="https://img.loigiaihay.com/picture/2022/0815/052.mp3">
**Audio:** Listen to the passage about traffic problems in Mumbai
</audio>

<teacher_script pause="30" href="audio/tts_9b2a5da7.webm">
Chúng ta cùng làm Bài 2 <eng>Listen and choose</eng>. Các em hãy nghe đoạn văn và chọn đáp án đúng nhất về chủ đề của bài. Các em có 30 giây.
</teacher_script>

<questions type="multiple_choice">
**What is the passage mainly about?**
*(Bài nghe chủ yếu nói về gì?)*

- A. Travelling in Mumbai
- B. Reasons for traffic problems in Mumbai
- C. Traffic problems in Mumbai
</questions>

---

<!-- chunk: answer_2 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** C - Traffic problems in Mumbai
*(Vấn đề giao thông ở Mumbai)*
</answer>

<teacher_script pause="0" href="audio/tts_24e71d75.webm">
Đáp án đúng là C - Traffic problems in Mumbai nha. Tại sao? Vì bài nghe nói về vấn đề giao thông ở Mumbai á. Bài nói Mumbai là một trong những thành phố tắc nghẽn nhất thế giới. Sau đó bài giải thích các lý do gây tắc đường: dân số tăng lên gần 20 triệu người, đường hẹp và không tốt, người dân không tuân thủ luật. Kết quả là tình trạng ngày càng tệ hơn. Vậy chủ đề chính của bài là vấn đề giao thông ở Mumbai, không phải về du lịch ở Mumbai hay chỉ nói về nguyên nhân thôi hen.
</teacher_script>

<explanation>
**Giải thích:**
Bài nghe nói về:
- Mumbai là thành phố tắc nghẽn
- Các lý do gây tắc đường (dân số tăng, đường hẹp, không tuân thủ luật)
- Tình trạng ngày càng tệ

→ Chủ đề chính là "Traffic problems in Mumbai" (Vấn đề giao thông ở Mumbai)
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: exercise_3 -->
### Bài 3 trang 15 - Listen and fill

<teacher_script pause="60" href="audio/tts_d6dcb557.webm">
Chúng ta cùng làm Bài 3 <eng>Listen and fill</eng>. Các em hãy nghe lại và điền một từ hoặc một số vào mỗi chỗ trống. Các em có 1 phút.
</teacher_script>

<questions type="fill_blanks">
**1.** Traffic _______ happen every day in Mumbai.

*(Tắc _______ xảy ra mỗi ngày ở Mumbai.)*

**2.** There are several reasons for the traffic _______.

*(Có vài lý do cho vấn đề giao thông / tắc đường.)*

**3.** _______ million people live and work in Mumbai.

*(_______ triệu người sống và làm việc ở Mumbai.)*

**4.** Many _______ are narrow and not in good condition.

*(Nhiều con _______ hẹp và không trong tình trạng tốt.)*

**5.** Many people don't _______ the traffic rules.

*(Nhiều người không _______ luật giao thông.)*
</questions>

---

<!-- chunk: answer_3 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. jams
2. jams / problems
3. Twenty / 20
4. roads
5. obey
</answer>

<teacher_script pause="0" href="audio/tts_bc013f56.webm">
Câu 1 điền "jams" nha. Câu đầy đủ là "Traffic jams happen every day in Mumbai". Traffic jams nghĩa là tắc đường, ùn tắc giao thông á. Trong bài nghe có nói rõ là "Traffic jams happen nearly every day" - tắc đường xảy ra gần như mỗi ngày.
</teacher_script>

<teacher_script pause="0" href="audio/tts_f06b0919.webm">
Câu 2 điền "jams" hoặc "problems" đều được nha. Câu đầy đủ là "There are several reasons for the traffic jams" hoặc "traffic problems". Cả hai từ đều có thể chấp nhận được vì bài nói về lý do gây ra tắc đường hay vấn đề giao thông.
</teacher_script>

<teacher_script pause="0" href="audio/tts_8be6bc2d.webm">
Câu 3 điền "Twenty" hoặc viết số "20". Câu đầy đủ là "Twenty million people live and work in Mumbai". Bài nghe nói rõ là "With nearly 20 million" - với gần 20 triệu dân, Mumbai có quá nhiều người trên đường, đây là một lý do gây tắc nghẽn.
</teacher_script>

<teacher_script pause="0" href="audio/tts_60ed64dd.webm">
Câu 4 điền "roads" nha. Câu đầy đủ là "Many roads are narrow and not in good condition". Bài nghe nói "the roads in Mumbai are narrow, and many are not in good condition" - nhiều con đường hẹp và không trong tình trạng tốt, gồ ghề, đây là lý do thứ hai gây tắc đường á.
</teacher_script>

<teacher_script pause="0" href="audio/tts_f7c79a81.webm">
Câu 5 điền "obey". Câu đầy đủ là "Many people don't obey the traffic rules". Obey nghĩa là tuân thủ, tuân theo. Bài nghe nói "many road users do not obey the traffic rules" - nhiều người tham gia giao thông không tuân thủ luật giao thông, làm cho tình hình càng tệ hơn hen.
</teacher_script>

<explanation>
**Giải thích chi tiết:**

| Câu | Đáp án | Câu đầy đủ |
|-----|--------|------------|
| 1 | jams | Traffic **jams** happen every day in Mumbai. |
| 2 | jams/problems | There are several reasons for the traffic **jams/problems**. |
| 3 | Twenty/20 | **Twenty** million people live and work in Mumbai. |
| 4 | roads | Many **roads** are narrow and not in good condition. |
| 5 | obey | Many people don't **obey** the traffic rules. |
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: writing_intro -->
### Writing

<teacher_script pause="0" href="audio/tts_f837f6e9.webm">
Chúng ta cùng chuyển sang phần <eng>Writing</eng>.
</teacher_script>

---

<!-- chunk: exercise_4 -->
### Bài 4 trang 15 - Tick the traffic problems

<teacher_script pause="30" href="audio/tts_4deab741.webm">
Chúng ta cùng làm Bài 4 <eng>Tick the traffic problems</eng>. Các em hãy đánh dấu vào những vấn đề giao thông thường gặp ở các thành phố lớn tại Việt Nam. Các em có 30 giây.
</teacher_script>

<task>
**Đề:** Tick (✓) the traffic problems in big cities in Viet Nam.
**Dịch đề:** Đánh dấu (✓) các vấn đề giao thông ở các thành phố lớn Việt Nam.
</task>

<questions type="true_false">
**Đánh dấu (✓) nếu là vấn đề ở Việt Nam:**

| # | English | Vietnamese | ✓? |
|---|---------|------------|-----|
| 1 | Too many vehicles on the roads | Quá nhiều phương tiện trên đường | |
| 2 | Narrow and bumpy roads | Đường hẹp và gồ ghề | |
| 3 | Some people don't obey the traffic rules | Một số người không tuân thủ luật giao thông | |
| 4 | Children cycling dangerously | Trẻ em đạp xe nguy hiểm | |
| 5 | Wild animals running across the roads | Động vật hoang dã chạy qua đường | |
</questions>

---

<!-- chunk: answer_4 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án gợi ý:**

| # | Vấn đề | Việt Nam? |
|---|--------|-----------|
| 1 | Too many vehicles | ✓ Có - đặc biệt ở Sài Gòn, Hà Nội |
| 2 | Narrow and bumpy roads | ✓ Có - nhiều đường nhỏ, hẻm |
| 3 | People don't obey traffic rules | ✓ Có - vượt đèn đỏ, không đội mũ bảo hiểm |
| 4 | Children cycling dangerously | ✓ Có - một số nơi |
| 5 | Wild animals on roads | ✗ Không phổ biến ở thành phố lớn |
</answer>

<teacher_script pause="0" href="audio/tts_c8a3d9f2.webm">
Vấn đề số 1 - Too many vehicles - quá nhiều phương tiện. Tick nha, vì ở Việt Nam đặc biệt là Sài Gòn, Hà Nội có rất nhiều xe máy, ô tô trên đường, đông nghịt luôn.
</teacher_script>

<teacher_script pause="0" href="audio/tts_e7b2a4c1.webm">
Vấn đề số 2 - Narrow and bumpy roads - đường hẹp và gồ ghề. Tick. Ở Việt Nam có nhiều đường nhỏ, hẻm hẹp, và một số đường bị ổ gà, gồ ghề á.
</teacher_script>

<teacher_script pause="0" href="audio/tts_f9d5b3e8.webm">
Vấn đề số 3 - Some people don't obey traffic rules - một số người không tuân thủ luật giao thông. Tick. Đây là vấn đề khá phổ biến: vượt đèn đỏ, không đội mũ bảo hiểm, đi sai làn đường hen.
</teacher_script>

<teacher_script pause="0" href="audio/tts_a2c6e8f4.webm">
Vấn đề số 4 - Children cycling dangerously - trẻ em đạp xe nguy hiểm. Tick. Ở một số nơi vẫn có trẻ em đạp xe không an toàn, không đội mũ bảo hiểm, đi sai làn đường.
</teacher_script>

<teacher_script pause="0" href="audio/tts_d1f7b9a5.webm">
Vấn đề số 5 - Wild animals running across the roads - động vật hoang dã chạy qua đường. Không tick nha. Vấn đề này không phổ biến ở các thành phố lớn Việt Nam. Có thể gặp ở vùng núi, vùng nông thôn, nhưng không phải vấn đề chính của giao thông đô thị.
</teacher_script>

<explanation>
**Giải thích:**
Ở Việt Nam, 4 vấn đề đầu tiên khá phổ biến ở các thành phố lớn. Riêng vấn đề số 5 về động vật hoang dã thì ít gặp hơn tại các đô thị.
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: exercise_5 -->
### Bài 5 trang 15 - Write a paragraph

<teacher_script pause="0" href="audio/tts_7ce58049.webm">
Chúng ta cùng làm Bài 5 <eng>Write a paragraph</eng>. Các em hãy viết một đoạn văn khoảng 70 từ về các vấn đề giao thông ở thị trấn hoặc thành phố của mình.
</teacher_script>

<task>
**Đề:** Write a paragraph of about 70 words about the traffic problems in your town/city.
**Dịch đề:** Viết đoạn văn khoảng 70 từ về vấn đề giao thông ở thị trấn/thành phố của bạn.
</task>

<grammar>
📝 *Các em ghi vô tập bài học:*

**Dàn ý viết đoạn văn:**

1. **Mở bài:** Giới thiệu nơi bạn sống (đông đúc hay không?)
2. **Thân bài:** Nêu các vấn đề giao thông
   - Đường xá như thế nào? (narrow, bumpy, crowded...)
   - Người dân có tuân thủ luật không?
   - Có tai nạn nhiều không?
3. **Kết bài:** Đề xuất giải pháp

**Từ vựng hữu ích:**
- crowded (đông đúc), narrow (hẹp), bumpy (gồ ghề)
- traffic jam (tắc đường), accident (tai nạn)
- obey the traffic rules (tuân thủ luật giao thông)
- should be stricter (nên nghiêm ngặt hơn)
</grammar>

<teacher_script pause="300" href="audio/tts_20a53546.webm">
Các em hãy viết đoạn văn vào vở. Các em có 5 phút.
</teacher_script>

---

<!-- chunk: sample_writing -->
<teacher_script pause="0" href="audio/tts_77096c2e.webm">
Sau đây là một bài viết mẫu để các em tham khảo.
</teacher_script>

<reading>
My town is not very crowded, but there are some traffic problems.
The roads are narrow and bumpy.
Some people ride their motorbikes dangerously.
Some young people do not obey the traffic lights, and therefore accidents happen.
I think the traffic rules should be made stricter to reduce accidents.
</reading>

<translation>
Thị trấn của tôi không đông lắm, nhưng có một số vấn đề giao thông.
Đường thì hẹp và gồ ghề.
Một số người chạy xe máy nguy hiểm.
Một số người trẻ không tuân thủ đèn giao thông, và vì vậy tai nạn xảy ra.
Tôi nghĩ luật giao thông nên được làm nghiêm ngặt hơn để giảm tai nạn.
</translation>

<teacher_script pause="60" href="audio/tts_62f0f119.webm">
Các em hãy so sánh bài viết của mình với bài mẫu và chỉnh sửa nếu cần nhé. Các em có 1 phút.
</teacher_script>

<teacher_script pause="0" action="photo" href="audio/tts_c8db25e1.webm">
Các em hãy chụp hình bài viết của mình và gửi cho thầy nhé.
</teacher_script>

---

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_4e1ebf73.webm">
Ok hết Skills 2 rồi. Nhớ mấy từ quan trọng nha:
<eng>Traffic jam</eng> = tắc đường, <eng>congested</eng> = tắc nghẽn, <eng>obey the traffic rules</eng> = tuân thủ luật giao thông.
Bài sau qua Looking Back. Hẹn gặp lại!
</teacher_script>
