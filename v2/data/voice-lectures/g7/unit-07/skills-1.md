# UNIT 7: TRAFFIC

## SKILLS 1 - Reading & Speaking

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_09b02762.webm">
Chào các em. Mời các em mở sách giáo khoa trang 14. Bài học hôm nay là <eng>Skills 1</eng>, chúng ta sẽ luyện tập hai kỹ năng Đọc và Nói về chủ đề an toàn giao thông.
</teacher_script>

---

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_9676ed05.webm">
Trước khi vào bài đọc, chúng ta cùng học một số từ vựng mới. Các em hãy nghe và đọc theo, sau đó làm bài tập để ghi nhớ nhé.
</teacher_script>

<vocabulary>
**Từ vựng bài đọc:**
1. **road safety** : (n) an toàn giao thông /rəʊd ˈseɪfti/
2. **obey** : (v) tuân theo, tuân thủ /əˈbeɪ/
3. **road user** : (n) người tham gia giao thông /rəʊd ˈjuːzə/
4. **pedestrian** : (n) người đi bộ /pəˈdestriən/
5. **pavement** : (n) vỉa hè /ˈpeɪvmənt/
6. **footpath** : (n) đường dành cho người đi bộ /ˈfʊtpɑːθ/
7. **zebra crossing** : (n) vạch kẻ đường (vạch ngựa vằn) /ˈzebrə ˈkrɒsɪŋ/
8. **cyclist** : (n) người đi xe đạp /ˈsaɪklɪst/
9. **handlebars** : (n) ghi đông xe đạp /ˈhændlbɑːz/
10. **cycle lane** : (n) làn đường dành cho xe đạp /ˈsaɪkl leɪn/
11. **give a signal** : ra tín hiệu /ɡɪv ə ˈsɪɡnəl/
12. **passenger** : (n) hành khách /ˈpæsɪndʒə/
13. **fasten your seatbelt** : thắt dây an toàn /ˈfɑːsn jɔː ˈsiːtbelt/
14. **fully stop** : dừng hẳn /ˈfʊli stɒp/
15. **get on / get off** : lên xe / xuống xe /ɡet ɒn/ /ɡet ɒf/
16. **stick out** : đưa ra ngoài /stɪk aʊt/
17. **moving vehicle** : phương tiện đang di chuyển /ˈmuːvɪŋ ˈviːɪkl/

**Từ vựng bài Speaking:**
18. **stand in a line** : xếp hàng /stænd ɪn ə laɪn/
19. **shout** : (v) la hét, hét lớn /ʃaʊt/
20. **wave** : (v) vẫy tay /weɪv/
21. **careful** : (adj) cẩn thận /ˈkeəfl/
22. **carefully** : (adv) một cách cẩn thận /ˈkeəfli/
</vocabulary>

<teacher_script pause="120" href="audio/tts_b98f6548.webm">
Các em hãy dành 2 phút để nghe phát âm và ghi chép từ vựng vào vở nhé.
</teacher_script>

<teacher_script pause="0" action="record" href="audio/tts_e4cf0019.mp3">
Các em hãy bấm nút Ghi âm để đọc từ vựng và gửi cho thầy nhé.
</teacher_script>

---

<!-- chunk: exercise_1 -->
### Bài 1 trang 14 - Look at the picture

<teacher_script pause="30" href="audio/tts_3aa8ce53.webm">
Chúng ta cùng làm Bài 1 <eng>Look at the picture</eng>. Các em hãy quan sát bức tranh và cho biết điều gì nguy hiểm đang xảy ra. Các em có 30 giây.
</teacher_script>

<task>
**Đề:** Look at the picture. Can you see anything that is dangerous?
**Dịch đề:** Nhìn vào bức tranh. Bạn có thấy điều gì nguy hiểm không?
</task>

<teacher_script pause="0" href="audio/tts_a7be3fea.webm">
Trong hình có mấy bạn học sinh đang qua đường.
</teacher_script>

<answer>
**Bài 1 - Trả lời:**
Yes, we can see some potential danger as the students do not walk across the street at the zebra crossing and it seems like they are not paying attention to other vehicles on the road.

*(Có, chúng ta thấy một số nguy hiểm tiềm ẩn vì các học sinh không đi qua đường ở vạch kẻ đường và dường như họ không chú ý đến các phương tiện khác trên đường.)*
</answer>

<teacher_script pause="0" href="audio/tts_c3d17039.webm">
Các bạn học sinh đi qua đường không đúng chỗ, không đi trên vạch kẻ đường dành cho người đi bộ, và không chú ý đến xe cộ. Đây là hành vi rất nguy hiểm, dễ gây tai nạn. Các em tuyệt đối không được làm như vậy nhé!
</teacher_script>

---

<!-- chunk: exercise_2_intro -->
### Bài 2 trang 14 - Read and choose

<teacher_script pause="0" href="audio/tts_e5b560cf.webm">
Chúng ta cùng làm Bài 2 <eng>Read and choose</eng>. Các em hãy đọc đoạn văn về luật an toàn giao thông và chọn đáp án đúng nhất.
</teacher_script>

---

<!-- chunk: reading -->
<teacher_script pause="0" href="audio/tts_767c0247.webm">
Mời các em đọc kỹ bài đọc sau.
</teacher_script>

<reading>
These are some rules about road safety. It is important to obey these rules when you are a road user.
</reading>

<translation>
Đây là một số luật về an toàn đường bộ. Điều quan trọng là phải tuân thủ các luật này khi bạn là người tham gia giao thông.
</translation>

---

<!-- chunk: reading_pedestrians -->
<teacher_script pause="0" href="audio/tts_17730a02.webm">
Phần 1: <eng>Pedestrians</eng> - Luật giao thông dành cho người đi bộ.
</teacher_script>

<reading>
**Pedestrians**
1. Always look carefully when you cross the street.
2. Use the pavement or footpath.
3. Walk across the street at the zebra crossing.
4. Don't cross the road on a red light.
</reading>

<translation>
**Người đi bộ**
1. Luôn quan sát cẩn thận khi băng qua đường.
2. Đi trên vỉa hè hoặc đường dành cho người đi bộ.
3. Đi bộ qua đường ở vạch kẻ đường.
4. Không sang đường khi đèn đỏ.
</translation>

<teacher_script pause="0" href="audio/tts_eeb00d48.webm">
Có 4 quy tắc quan trọng cho người đi bộ: Luôn quan sát cẩn thận, đi trên vỉa hè hoặc lề đường, qua đường tại vạch kẻ đường, và không qua đường khi đèn đỏ.
</teacher_script>

---

<!-- chunk: reading_cyclists -->
<teacher_script pause="0" href="audio/tts_324fb2be.webm">
Phần 2: <eng>Cyclists</eng> - Luật giao thông dành cho người đi xe đạp.
</teacher_script>

<reading>
**Cyclists**
1. Always keep both hands on the handlebars.
2. Wear helmets, and always use the cycle lane.
3. Give a signal before you turn.
4. Don't carry more than one passenger.
</reading>

<translation>
**Người đi xe đạp**
1. Luôn giữ cả hai tay trên ghi đông.
2. Đội mũ bảo hiểm và luôn đi trên làn đường dành cho xe đạp.
3. Ra tín hiệu trước khi rẽ.
4. Không chở quá một hành khách.
</translation>

<teacher_script pause="0" href="audio/tts_d78e44c1.webm">
Có 4 quy tắc cho người đi xe đạp: Luôn giữ hai tay trên ghi đông, đội mũ bảo hiểm và đi đúng làn đường, ra tín hiệu trước khi rẽ, và không chở quá một người.
</teacher_script>

---

<!-- chunk: reading_passengers -->
<teacher_script pause="0" href="audio/tts_caa7c000.webm">
Phần 3: <eng>Passengers</eng> - Luật giao thông dành cho hành khách.
</teacher_script>

<reading>
**Passengers**
1. Fasten your seatbelt when you are in a car.
2. Wait for buses to fully stop before getting on or off.
3. Don't talk to the driver when he/she is driving.
4. Don't stick any body parts out of the window of a moving vehicle.
</reading>

<translation>
**Hành khách**
1. Thắt dây an toàn khi ngồi trên ô tô.
2. Đợi xe buýt dừng hẳn trước khi lên hoặc xuống.
3. Không nói chuyện với tài xế khi họ đang lái xe.
4. Không đưa bất kỳ bộ phận cơ thể nào ra ngoài cửa sổ của xe đang chạy.
</translation>

<teacher_script pause="0" href="audio/tts_4b3bfe53.webm">
Có 4 quy tắc cho hành khách: Thắt dây an toàn khi đi ô tô, đợi xe buýt dừng hẳn mới lên xuống, không nói chuyện với tài xế khi xe đang chạy, và không đưa tay hay đầu ra ngoài cửa sổ xe. Quy tắc cuối cùng rất quan trọng để đảm bảo an toàn.
</teacher_script>

---

<!-- chunk: exercise_2_question -->
<teacher_script pause="30" href="audio/tts_2afd2c9f.webm">
Bây giờ các em hãy trả lời câu hỏi: Bài đọc này nói về chủ đề gì? Các em có 30 giây.
</teacher_script>

<questions type="multiple_choice">
**This text is about _________.**

- A. rules for pedestrians *(luật cho người đi bộ)*
- B. traffic lights *(đèn giao thông)*
- C. traffic rules *(luật giao thông)*
</questions>

---

<!-- chunk: answer_2 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** C - traffic rules
</answer>

<teacher_script pause="0" href="audio/tts_9b6c76d1.webm">
Đáp án đúng là C - Traffic rules nha. Tại sao? Vì bài đọc này nói về luật giao thông cho 3 nhóm người tham gia giao thông á: Người đi bộ - pedestrians, người đi xe đạp - cyclists, và hành khách - passengers. Mỗi nhóm đều có những luật riêng phải tuân thủ. Vậy nên đáp án không phải A vì A chỉ nói về người đi bộ thôi, còn bài này nói đủ 3 nhóm. Cũng không phải B vì B chỉ nói về đèn giao thông, mà bài này nói về nhiều luật khác nhau hen.
</teacher_script>

<explanation>
**Giải thích:**
Bài đọc nói về luật giao thông (traffic rules) cho 3 đối tượng:
- Người đi bộ (pedestrians)
- Người đi xe đạp (cyclists)
- Hành khách (passengers)

Nên đáp án không phải A (chỉ nói về người đi bộ) hay B (đèn giao thông).
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: exercise_3 -->
### Bài 3 trang 14 - Read and answer

<teacher_script pause="90" href="audio/tts_ad757d93.webm">
Chúng ta cùng làm Bài 3 <eng>Read the text again</eng>. Các em hãy đọc lại bài và trả lời các câu hỏi sau. Các em có 1 phút 30 giây.
</teacher_script>

<questions type="read_answer">
**1.** Where should pedestrians cross the street?
→ ___________________________________

*(Người đi bộ nên qua đường ở đâu?)*

**2.** Which lane should you use when riding a bike?
→ ___________________________________

*(Bạn nên đi làn nào khi đi xe đạp?)*

**3.** What should you do before you turn while riding a bike?
→ ___________________________________

*(Bạn nên làm gì trước khi rẽ khi đang đi xe đạp?)*

**4.** What must you do when you get on or off a bus?
→ ___________________________________

*(Bạn phải làm gì khi lên hoặc xuống xe buýt?)*

**5.** What mustn't you do when you are in a moving vehicle?
→ ___________________________________

*(Bạn không được làm gì khi đang ở trên xe đang chạy?)*
</questions>

---

<!-- chunk: answer_3 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Bài 3 - Đáp án:**
1. At the zebra crossing.
2. The cycle lane.
3. Give a signal before you turn.
4. Wait for buses to fully stop.
5. Don't stick any body parts out of the window.
</answer>

<teacher_script pause="0" href="audio/tts_b7161c97.webm">
Câu 1 hỏi người đi bộ nên qua đường ở đâu. Đáp án là "At the zebra crossing" nha - tại vạch kẻ đường. Các em tìm trong phần Pedestrians, rule số 3 có viết là "Walk across the street at the zebra crossing". Đây là vạch trắng kẻ trên đường dành riêng cho người đi bộ, rất quan trọng để đảm bảo an toàn.
</teacher_script>

<teacher_script pause="0" href="audio/tts_64accd22.webm">
Câu 2 hỏi nên đi làn nào khi đi xe đạp. Đáp án là "The cycle lane" - làn đường dành cho xe đạp. Trong phần Cyclists, rule số 2 có viết "always use the cycle lane". Cycle lane là làn đường riêng biệt dành cho người đi xe đạp, đi đúng làn sẽ an toàn hơn hen.
</teacher_script>

<teacher_script pause="0" href="audio/tts_bc54f5a7.webm">
Câu 3 hỏi phải làm gì trước khi rẽ khi đang đi xe đạp. Đáp án là "Give a signal" - ra tín hiệu. Trong phần Cyclists, rule số 3 viết rõ "Give a signal before you turn". Nghĩa là phải giơ tay ra hiệu cho người khác biết mình sắp rẽ á, như vậy họ sẽ cẩn thận.
</teacher_script>

<teacher_script pause="0" href="audio/tts_eeab5cbe.webm">
Câu 4 hỏi phải làm gì khi lên xuống xe buýt. Đáp án là "Wait for buses to fully stop" - đợi xe buýt dừng hẳn. Trong phần Passengers, rule số 2 viết rõ "Wait for buses to fully stop before getting on or off". Phải đợi xe dừng hẳn mới lên xuống, chứ xe còn chạy mà nhảy lên xuống rất nguy hiểm nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_480adb0b.webm">
Câu 5 hỏi không được làm gì khi ở trên xe đang chạy. Đáp án là "Don't stick any body parts out of the window" - không đưa bất kỳ bộ phận cơ thể nào ra ngoài cửa sổ. Trong phần Passengers, rule số 4 viết rõ luôn. Đưa tay hay đầu ra ngoài cửa sổ khi xe đang chạy rất nguy hiểm, có thể va vào xe khác hoặc vật cản bên ngoài á.
</teacher_script>

<explanation>
**Giải thích chi tiết:**

| Câu | Đáp án | Nguồn trong bài |
|-----|--------|-----------------|
| 1 | At the zebra crossing | Pedestrians - rule 3 |
| 2 | The cycle lane | Cyclists - rule 2 |
| 3 | Give a signal | Cyclists - rule 3 |
| 4 | Wait for buses to fully stop | Passengers - rule 2 |
| 5 | Don't stick body parts out | Passengers - rule 4 |
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

---

<!-- chunk: speaking_intro -->
### Speaking

<teacher_script pause="0" href="audio/tts_9a96ff97.webm">
Chúng ta cùng chuyển sang phần <eng>Speaking</eng>.
</teacher_script>

---

<!-- chunk: exercise_4 -->
### Bài 4 trang 14 - Work in groups

<teacher_script pause="0" href="audio/tts_076bb28f.webm">
Chúng ta cùng làm Bài 4 <eng>Work in groups</eng>. Các em hãy thảo luận trong nhóm và trả lời câu hỏi: Những điều KHÔNG NÊN làm khi tham gia giao thông là gì? Hãy lập danh sách cùng nhau nhé.
</teacher_script>

<task>
**Đề:** Work in groups. Ask and answer the following question: When you are a road user, what should you NOT do? Make a list in groups.
**Dịch đề:** Làm việc nhóm. Hỏi và trả lời câu hỏi: Khi là người tham gia giao thông, bạn KHÔNG NÊN làm gì? Lập danh sách theo nhóm.
</task>

<questions type="speaking">
**Câu hỏi:** When you are a road user, what should you NOT do?
*(Khi là người tham gia giao thông, bạn không nên làm gì?)*

**Mẫu danh sách:**

| English | Vietnamese |
|---------|------------|
| Cross the road on a red light. | Sang đường khi đèn đỏ. |
| Cycle on the pavement or footpath. | Đạp xe trên vỉa hè. |
| Lose concentration when driving. | Mất tập trung khi lái xe. |
| Stick any body parts out of the window of a moving vehicle. | Đưa bộ phận cơ thể ra ngoài cửa sổ xe đang chạy. |
| Use your phone while driving. | Dùng điện thoại khi lái xe. |
| Drive too fast. | Lái xe quá nhanh. |
| Not wear a helmet when cycling. | Không đội mũ bảo hiểm khi đạp xe. |

**Cách nói:**
- You shouldn't cross the road on a red light.
- You shouldn't use your phone while driving.
- You shouldn't drive too fast.
</questions>

<teacher_script pause="0" action="record" href="audio/tts_ab701031.webm">
Các em hãy ghi âm 3 điều không nên làm khi tham gia giao thông và gửi cho thầy nhé.
</teacher_script>

---

<!-- chunk: exercise_5 -->
### Bài 5 trang 14 - Who is being safe?

<teacher_script pause="0" href="audio/tts_0cb7c2e8.webm">
Chúng ta cùng làm Bài 5 <eng>Who is being safe?</eng> Các em hãy đọc các tình huống và thảo luận xem ai đang tham gia giao thông an toàn và ai không nhé.
</teacher_script>

<task>
**Đề:** Read the following sentences about these people. Then work in groups. Discuss who is being safe, and who isn't.
**Dịch đề:** Đọc các câu sau về những người này. Làm việc nhóm, thảo luận ai đang an toàn và ai không.
</task>

<questions type="speaking">
**Đọc các tình huống:**

**1.** Hoang is riding a bike, and he is wearing a helmet.
*(Hoàng đang đi xe đạp và đang đội mũ bảo hiểm.)*

**2.** It is raining hard, but Mr Long is driving quickly.
*(Trời mưa to, nhưng chú Long vẫn lái xe rất nhanh.)*

**3.** The students are standing in a line to get on the school bus.
*(Các học sinh đang đứng xếp hàng để lên xe buýt trường.)*

**4.** Mr Binh is taking his daughter to school on his motorbike. She is sitting in front of him.
*(Chú Bình chở con gái đi học bằng xe máy. Cô bé ngồi phía trước.)*

**5.** Michelle is cycling to school and she is waving and shouting to her friends.
*(Michelle đang đạp xe đến trường và cô ấy đang vẫy tay, hét gọi bạn bè.)*
</questions>

---

<!-- chunk: answer_5 -->
<teacher_script pause="0" href="audio/tts_778f802f.webm">
Đã hết thời gian. Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**

| # | Người | An toàn? | Lý do |
|---|-------|----------|-------|
| 1 | Hoang | ✅ An toàn | Đội mũ bảo hiểm khi đạp xe |
| 2 | Mr Long | ❌ Không an toàn | Lái xe nhanh khi trời mưa to, đường trơn |
| 3 | Students | ✅ An toàn | Xếp hàng đợi lên xe buýt |
| 4 | Mr Binh | ❌ Không an toàn | Để con ngồi phía trước, dễ văng ra khi phanh |
| 5 | Michelle | ❌ Không an toàn | Mất tập trung khi đạp xe |
</answer>

<teacher_script pause="0" href="audio/tts_5463894d.webm">
Tình huống 1 về Hoàng - AN TOÀN nha. Hoàng đội mũ bảo hiểm khi đạp xe. Đây là điều rất đúng á. Trong phần Cyclists rule số 2 có viết "Wear helmets", đội mũ bảo hiểm sẽ bảo vệ đầu nếu bị ngã hay va chạm.
</teacher_script>

<teacher_script pause="0" href="audio/tts_aa926e9f.webm">
Tình huống 2 về chú Long - KHÔNG AN TOÀN. Trời mưa to nhưng chú Long vẫn lái xe nhanh. Đây là hành vi rất nguy hiểm hen. Khi trời mưa, đường trơn trượt, nếu lái nhanh dễ mất lái, trượt bánh, gây tai nạn. Phải lái chậm lại mới an toàn nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_2b66e1b8.webm">
Tình huống 3 về các học sinh - AN TOÀN. Các em xếp hàng để lên xe buýt trường. Đây là hành vi đúng, có trật tự, tránh chen lấn và ngã đổ. Xếp hàng còn giúp tài xế dễ quan sát, đảm bảo an toàn cho tất cả mọi người.
</teacher_script>

<teacher_script pause="0" href="audio/tts_3efe6439.webm">
Tình huống 4 về chú Bình - KHÔNG AN TOÀN. Chú Bình chở con gái nhưng để cô bé ngồi phía trước. Đây là hành vi rất nguy hiểm cho trẻ em á. Trẻ em phải ngồi phía sau, có tay vịn, mới được bảo vệ. Ngồi phía trước khi xe phanh gấp, trẻ dễ văng ra ngoài, rất nguy hiểm luôn.
</teacher_script>

<teacher_script pause="0" href="audio/tts_a55d27bc.webm">
Tình huống 5 về Michelle - KHÔNG AN TOÀN. Michelle đang đạp xe nhưng vẫy tay và hét lớn gọi bạn bè. Đây là hành vi mất tập trung khi tham gia giao thông. Trong phần Cyclists rule số 1 có viết "Always keep both hands on the handlebars" - luôn giữ hai tay trên ghi đông. Mà Michelle vẫy tay và la hét thì không nhìn đường, rất dễ gây tai nạn nha.
</teacher_script>

<explanation>
**Giải thích chi tiết:**

**1. Hoang - SAFE ✅**
- Hoang is riding a bike, and he is wearing a helmet.
- Đội mũ bảo hiểm khi đạp xe là điều nên làm.

**2. Mr Long - NOT SAFE ❌**
- It is raining hard, but Mr Long is driving quickly.
- Trời mưa đường trơn, lái nhanh nguy hiểm, dễ trượt bánh.

**3. Students - SAFE ✅**
- The students are standing in a line to get on the school bus.
- Xếp hàng lên xe buýt đúng cách, an toàn.

**4. Mr Binh - NOT SAFE ❌**
- His daughter is sitting in front of him.
- Trẻ em ngồi phía trước xe máy nguy hiểm, dễ văng ra khi phanh gấp.

**5. Michelle - NOT SAFE ❌**
- She is waving and shouting to her friends while cycling.
- Mất tập trung khi lái xe, không nhìn đường, rất nguy hiểm.
</explanation>

<teacher_script pause="30" href="audio/tts_75ebc937.webm">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

<teacher_script pause="0" action="record" href="audio/tts_a8e98ceb.webm">
Các em hãy ghi âm phân tích một tình huống an toàn và một tình huống không an toàn rồi gửi cho thầy nhé.
</teacher_script>

---

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_dbdddc46.webm">
Bài học hôm nay kết thúc tại đây. Các em hãy ghi nhớ các luật an toàn giao thông quan trọng cho người đi bộ, đi xe đạp và hành khách nhé. Bài học tiếp theo là <eng>Skills 2</eng>. Hẹn gặp lại các em!
</teacher_script>
