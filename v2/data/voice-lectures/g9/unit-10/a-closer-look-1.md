# UNIT 10: PLANET EARTH
## A CLOSER LOOK 1 - Vocabulary & Pronunciation

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_52fde63e.webm">
Ok lớp 9, Unit 10 bài <eng>A Closer Look 1</eng> nha. Mở sách trang 106 đi. Bài này mình học từ vựng về chủ đề hành tinh Trái Đất và phát âm về nhịp điệu trong câu.
</teacher_script>

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_f0c9c62f.webm">
Trước tiên mình học từ vựng nha. Bấm nút "Bắt đầu" bên dưới để học từ vựng. Máy sẽ đọc và các em đọc theo. Sau đó làm bài trắc nghiệm từ vựng rồi ghi từ vựng vô tập.
</teacher_script>

<vocabulary>
1. **habitat** : (n) môi trường sống, nơi cư trú /ˈhæbɪtæt/
2. **flora and fauna** : (n) hệ thực vật và động vật /ˈflɔːrə ænd ˈfɔːnə/
3. **grassland** : (n) đồng cỏ /ˈɡrɑːslænd/
4. **nature reserve** : (n) khu bảo tồn thiên nhiên /ˈneɪtʃə rɪˈzɜːv/
5. **food chain** : (n) chuỗi thức ăn /fuːd tʃeɪn/
6. **pole** : (n) cực /pəʊl/
7. **North Pole** : (n) Bắc Cực /nɔːθ pəʊl/
8. **South Pole** : (n) Nam Cực /saʊθ pəʊl/
9. **bamboo forest** : (n) rừng tre /bæmˈbuː ˈfɒrɪst/
10. **climate change** : (n) biến đổi khí hậu /ˈklaɪmət tʃeɪndʒ/
11. **global warming** : (n) sự nóng lên toàn cầu /ˈɡləʊbəl ˈwɔːmɪŋ/
12. **habitat loss** : (n) sự mất môi trường sống /ˈhæbɪtæt lɒs/
13. **ecological balance** : (n) cân bằng sinh thái /ˌiːkəˈlɒdʒɪkəl ˈbæləns/
14. **temperature** : (n) nhiệt độ /ˈtemprətʃə/
15. **face** : (v) đối mặt, đương đầu /feɪs/
16. **lend a hand** : (v) giúp đỡ /lend ə hænd/
17. **hunt** : (v) săn bắt /hʌnt/
18. **preserve** : (v) bảo tồn, gìn giữ /prɪˈzɜːv/
19. **pollute** : (v) gây ô nhiễm /pəˈluːt/
20. **protect** : (v) bảo vệ /prəˈtekt/
21. **provide** : (v) cung cấp /prəˈvaɪd/
22. **pollution** : (n) sự ô nhiễm /pəˈluːʃən/
23. **protection** : (n) sự bảo vệ /prəˈtekʃən/
</vocabulary>

<!-- chunk: exercise-1 -->
### Bai 1 trang 106 - Matching

<teacher_script pause="0" href="audio/tts_bd62e321.webm">
Bài 1 <eng>Write a word or phrase from the box under each picture</eng> nha. Viết một từ hoặc cụm từ trong khung dưới mỗi bức tranh. Các em xem hình trong sách và điền từ phù hợp.
</teacher_script>

<task>
**De:** Write a word or phrase from the box under each picture.

**Dich de:** Viết một từ hoặc cụm từ trong khung dưới mỗi bức tranh.
</task>

<questions type="matching">
| habitats | flora and fauna | grassland | nature reserves | food chain | poles |
|----------|-----------------|-----------|-----------------|------------|-------|

*Từ vựng trong khung:*
- *habitats: môi trường sống*
- *flora and fauna: hệ thực vật và động vật*
- *grassland: đồng cỏ*
- *nature reserves: khu bảo tồn thiên nhiên*
- *food chain: chuỗi thức ăn*
- *poles: các cực*
</questions>

<teacher_script pause="45" href="audio/tts_2622cb66.webm">
Các em làm bài 1 nha. 45 giây.
</teacher_script>

<teacher_script pause="0" href="audio/tts_7242caec.webm">
Ok đáp án bài 1 nè.
</teacher_script>

<answer>
**Dap an:**
1. flora and fauna *(hệ thực vật và động vật)*
2. habitats *(môi trường sống)*
3. poles *(vùng cực)*
4. grassland *(đồng cỏ)*
5. nature reserves *(khu bảo tồn thiên nhiên)*
6. food chain *(chuỗi thức ăn)*
</answer>

<explanation>
- Hình 1 có hình các loài động vật và thực vật khác nhau nên là **flora and fauna** (hệ thực vật và động vật).
- Hình 2 có hình các loài động vật sống trong môi trường tự nhiên nên là **habitats** (môi trường sống).
- Hình 3 có hình vùng băng giá ở cực nên là **poles** (vùng cực).
- Hình 4 có hình đồng cỏ rộng lớn nên là **grassland** (đồng cỏ).
- Hình 5 có hình khu bảo tồn động vật hoang dã nên là **nature reserves** (khu bảo tồn thiên nhiên).
- Hình 6 có hình chuỗi thức ăn trong tự nhiên nên là **food chain** (chuỗi thức ăn).
</explanation>

<!-- chunk: exercise-2 -->
### Bai 2 trang 106 - Fill in the blanks

<teacher_script pause="0" href="audio/tts_a020d412.webm">
Bài 2 <eng>Complete each sentence with a word or phrase from 1</eng> nha. Hoàn thành mỗi câu với từ hoặc cụm từ từ bài 1. Các em dùng từ vựng ở bài 1 để điền vào chỗ trống.
</teacher_script>

<task>
**De:** Complete each sentence with a word or phrase from 1.

**Dich de:** Hoàn thành mỗi câu với từ hoặc cụm từ từ bài 1.
</task>

<questions type="fill_blanks">
**1.** In a _______, some animals eat other animals and become food for a third group of animals.
*Trong một _______, một số động vật ăn thịt động vật khác và trở thành thức ăn cho nhóm động vật thứ ba.*

**2.** A _______ is often wide and its plants are mostly grass and flowers.
*Một _______ thường rộng và thực vật chủ yếu là cỏ và hoa.*

**3.** Areas of land to protect animals and plants are called _______.
*Diện tích đất bảo vệ động vật, thực vật được gọi là _______.*

**4.** The North and South _______ are both extremely cold and icy.
*Cực Bắc và cực Nam _______ đều cực kỳ lạnh và băng giá.*

**5.** Natural _______ for pandas are bamboo forests.
*_______ tự nhiên của gấu trúc là rừng tre.*
</questions>

<teacher_script pause="60" href="audio/tts_77b682b1.webm">
Các em làm bài 2 nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_e4a71e40.webm">
Ok đáp án bài 2 nè.
</teacher_script>

<answer>
**Dap an:** 1. food chain | 2. grassland | 3. nature reserves | 4. poles | 5. habitats
</answer>

<explanation>
**1. food chain** - Trong **chuỗi thức ăn**, một số động vật ăn thịt động vật khác và trở thành thức ăn cho nhóm động vật thứ ba. Đây là mô tả về chuỗi thức ăn trong tự nhiên.

**2. grassland** - Một **đồng cỏ** thường rộng và thực vật chủ yếu là cỏ và hoa. Từ "grass" trong "grassland" cho thấy đây là vùng đất có nhiều cỏ.

**3. nature reserves** - Diện tích đất bảo vệ động vật, thực vật được gọi là **khu bảo tồn thiên nhiên**. Đây là nơi được lập ra để bảo vệ các loài động thực vật.

**4. poles** - Cực Bắc và cực Nam **poles** đều cực kỳ lạnh và băng giá. "North Pole" và "South Pole" là hai cực của Trái Đất.

**5. habitats** - **Môi trường sống** tự nhiên của gấu trúc là rừng tre. Habitat là nơi sinh sống của các loài động vật.
</explanation>

<!-- chunk: exercise-3 -->
### Bai 3 trang 106 - Multiple choice

<teacher_script pause="0" href="audio/tts_7895fe22.webm">
Bài 3 <eng>Choose the correct answer A, B, C, or D</eng> nha. Chọn câu trả lời đúng A, B, C hoặc D. Các em đọc kỹ câu hỏi và chọn từ phù hợp nhất.
</teacher_script>

<task>
**De:** Choose the correct answer A, B, C, or D.

**Dich de:** Chọn câu trả lời đúng A, B, C hoặc D.
</task>

<questions type="multiple_choice">
**1.** When humans use a natural habitat for farming and housing, they cause _______.
- A. climate change
- B. global warming
- C. habitat loss
- D. ecological balance
*Khi con người sử dụng môi trường sống tự nhiên để trồng trọt và làm nhà ở, họ sẽ gây ra _______.*

**2.** When Earth's average temperature increases, we face _______.
- A. ecological balance
- B. global warming
- C. pollution
- D. protection
*Khi nhiệt độ trung bình của Trái đất tăng lên, chúng ta phải đối mặt với _______.*

**3.** Each of us can lend a hand to _______ the natural environment.
- A. hunt
- B. change
- C. preserve
- D. pollute
*Mỗi chúng ta có thể góp một tay để _______ môi trường tự nhiên.*

**4.** We can keep a(n) _______ by stopping hunting and cutting down trees.
- A. ecological balance
- B. habitat loss
- C. climate change
- D. nature reserve
*Chúng ta có thể giữ _______ bằng cách ngừng săn bắn và chặt cây.*

**5.** One way to _______ Mother Earth is by planting more trees.
- A. keep
- B. harm
- C. provide
- D. protect
*Một cách để _______ Đất Mẹ là trồng thêm cây xanh.*
</questions>

<teacher_script pause="60" href="audio/tts_1fcbe2db.webm">
Các em làm bài 3 nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_37893f7e.webm">
Ok đáp án bài 3 nè. Thầy giải thích từng câu luôn nha.
</teacher_script>

<answer>
**Dap an:** 1. C | 2. B | 3. C | 4. A | 5. D
</answer>

<explanation>
**Cau 1. C - habitat loss**
Khi con người sử dụng môi trường sống tự nhiên để trồng trọt và làm nhà ở, họ sẽ gây ra **sự mất môi trường sống**.
- A. climate change (n): biến đổi khí hậu
- B. global warming (n): sự nóng lên toàn cầu
- C. habitat loss (n): mất môi trường sống (DAP AN)
- D. ecological balance (n): cân bằng sinh thái

**Cau 2. B - global warming**
Khi nhiệt độ trung bình của Trái đất tăng lên, chúng ta phải đối mặt với **sự nóng lên toàn cầu**.
- A. ecological balance (n): cân bằng sinh thái
- B. global warming (n): sự nóng lên toàn cầu (DAP AN)
- C. pollution (n): ô nhiễm
- D. protection (n): bảo vệ

**Cau 3. C - preserve**
Mỗi chúng ta có thể góp một tay để **bảo tồn** môi trường tự nhiên. "Lend a hand to" nghĩa là giúp đỡ.
- A. hunt (v): săn bắn
- B. change (v): thay đổi
- C. preserve (v): bảo tồn (DAP AN)
- D. pollute (v): gây ô nhiễm

**Cau 4. A - ecological balance**
Chúng ta có thể giữ **cân bằng sinh thái** bằng cách ngừng săn bắn và chặt cây.
- A. ecological balance (n): cân bằng sinh thái (DAP AN)
- B. habitat loss (n): mất môi trường sống
- C. climate change (n): biến đổi khí hậu
- D. nature reserve (n): khu bảo tồn thiên nhiên

**Cau 5. D - protect**
Một cách để **bảo vệ** Đất Mẹ là trồng thêm cây xanh.
- A. keep (v): giữ
- B. harm (v): gây hại
- C. provide (v): cung cấp
- D. protect (v): bảo vệ (DAP AN)
</explanation>

<!-- chunk: pronunciation-theory -->
### Pronunciation - Rhythm in sentences

<teacher_script pause="0" href="audio/tts_5ba66ac7.webm">
Ok bây giờ mình học phần phát âm nha. Chủ đề là <eng>Rhythm in sentences</eng> - Nhịp điệu trong câu. Các em chú ý nghe lý thuyết nè.
</teacher_script>

<grammar>
**RHYTHM IN SENTENCES - Nhip dieu trong cau**

Trong tiếng Anh, khi nói một câu, chúng ta không nhấn mạnh tất cả các từ như nhau. Một số từ được nhấn mạnh (stressed) và một số từ không được nhấn mạnh (unstressed).

**Tu duoc nhan manh (Content words):**
- Danh từ (nouns): Earth, Sun, water
- Động từ chính (main verbs): eat, drink, study
- Tính từ (adjectives): cold, big, important
- Trạng từ (adverbs): very, extremely, quickly
- Từ nghi vấn (question words): what, where, when
- Từ phủ định (negatives): not, never, don't

**Tu khong duoc nhan manh (Function words):**
- Mạo từ (articles): a, an, the
- Giới từ (prepositions): in, on, at, to, from
- Đại từ (pronouns): I, you, he, she, it
- Trợ động từ (auxiliary verbs): is, are, was, were
- Liên từ (conjunctions): and, but, or

**Luu y:** Nhịp điệu tạo ra bởi sự luân phiên giữa các âm tiết được nhấn và không nhấn. Điều này giúp câu nói nghe tự nhiên và dễ hiểu hơn.
</grammar>

<!-- chunk: exercise-4 -->
### Bai 4 trang 107 - Listening

<teacher_script pause="0" href="audio/tts_33751576.webm">
Bài 4 <eng>Listen to the sentences and pay attention to the bold syllables. Does the speaker place stress on them?</eng> Nghe các câu và chú ý đến các âm tiết in đậm. Người nói có nhấn trọng âm cho chúng không? Các em nghe và để ý các từ in đậm được nhấn mạnh nha.
</teacher_script>

<task>
**De:** Listen to the sentences and pay attention to the bold syllables. Does the speaker place stress on them?

**Dich de:** Nghe các câu và chú ý đến các âm tiết in đậm. Người nói có nhấn trọng âm cho chúng không?
</task>

<audio src="https://img.loigiaihay.com/picture/2024/0516/62.mp3">
**Audio:** Bai 4 - Rhythm in sentences
</audio>

<questions type="listen_tick">
**1.** **Earth** is the **third** planet from the **Sun**.
*Trái đất là hành tinh thứ ba tính từ Mặt trời.*

**2.** **O**ceans, **seas**, rivers, and **lakes** are **wa**ter **bo**dies.
*Đại dương, biển, sông, hồ là các thủy vực.*

**3.** The North and South **poles** are ex**treme**ly **cold** and icy.
*Cực Bắc và Nam rất lạnh và băng giá.*

**4.** Do **moon**quakes **last** up to **half** an **hour**?
*Trận động đất trên Mặt trăng có kéo dài tới nửa giờ không?*

**5.** Pre**ser**ving **na**tural re**sour**ces is very im**port**ant.
*Bảo tồn tài nguyên thiên nhiên là rất quan trọng.*
</questions>

<teacher_script pause="0" href="audio/tts_d08e74c4.webm">
Đúng rồi đó các em. Người nói có nhấn mạnh các âm tiết in đậm. Các em để ý là các từ mang nghĩa chính như danh từ, động từ, tính từ thường được nhấn mạnh. Còn các từ chức năng như mạo từ, giới từ thì không nhấn. Điều này tạo ra nhịp điệu tự nhiên khi nói tiếng Anh nha.
</teacher_script>

<!-- chunk: exercise-5 -->
### Bai 5 trang 107 - Practice

<teacher_script pause="0" href="audio/tts_036ff618.webm">
Bài 5 <eng>Circle the stressed syllables in the sentences. Listen, check, and repeat.</eng> Khoanh tròn những âm tiết được nhấn mạnh trong câu. Nghe, kiểm tra và lặp lại. Các em thử đoán xem từ nào được nhấn mạnh trước nha.
</teacher_script>

<task>
**De:** Circle the stressed syllables in the sentences. Listen, check, and repeat.

**Dich de:** Khoanh tròn những âm tiết được nhấn mạnh trong câu. Nghe, kiểm tra và lặp lại.
</task>

<questions type="speaking">
**1.** We're doing a study on climate change.
*Chúng tôi đang thực hiện một nghiên cứu về biến đổi khí hậu.*

**2.** What is the distance from Earth to Mars?
*Khoảng cách từ Trái Đất đến Sao Hỏa là bao nhiêu?*

**3.** They'll have a discussion on natural habitats.
*Họ sẽ thảo luận về môi trường sống tự nhiên.*

**4.** Plants provide us with food, oxygen, and energy.
*Thực vật cung cấp cho chúng ta thức ăn, oxy và năng lượng.*

**5.** Our school organised various activities on Earth Day.
*Trường chúng tôi tổ chức nhiều hoạt động khác nhau vào Ngày Trái Đất.*
</questions>

<audio src="https://img.loigiaihay.com/picture/2024/0516/63.mp3">
**Audio:** Bai 5 - Circle stressed syllables
</audio>

<teacher_script pause="60" href="audio/tts_9ed04eb1.webm">
Các em khoanh tròn các từ được nhấn mạnh trong 5 câu trên nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_2e99bcf1.webm">
Ok đáp án bài 5 nè. Thầy đọc và các em kiểm tra xem mình khoanh đúng chưa nha.
</teacher_script>

<answer>
**Dap an:**

**1.** We're **DO**ing a **STU**dy on **CLI**mate **CHANGE**.
*Chúng tôi đang thực hiện một nghiên cứu về biến đổi khí hậu.*

**2.** What is the **DIS**tance from **EARTH** to **MARS**?
*Khoảng cách từ Trái Đất đến Sao Hỏa là bao nhiêu?*

**3.** They'll have a **DIS**cus**SION** on **NAT**u**RAL** **HAB**i**TATS**.
*Họ sẽ thảo luận về môi trường sống tự nhiên.*

**4.** Plants pro**VIDE** us with **FOOD**, **OX**ygen, and **EN**er**GY**.
*Thực vật cung cấp cho chúng ta thức ăn, oxy và năng lượng.*

**5.** Our **SCHOOL** or**GAN**ised **VAR**ious ac**TI**vi**TIES** on **EARTH** Day.
*Trường chúng tôi tổ chức nhiều hoạt động khác nhau vào Ngày Trái Đất.*
</answer>

<explanation>
Các em để ý nha:
- **Câu 1:** DOing, STUdy, CLImate, CHANGE - các từ mang nghĩa chính được nhấn mạnh.
- **Câu 2:** DIStance, EARTH, MARS - danh từ riêng và danh từ quan trọng được nhấn.
- **Câu 3:** DIS-cus-SION, NAT-u-RAL, HAB-i-TATS - âm tiết mang trọng âm của từ được nhấn.
- **Câu 4:** proVIDE, FOOD, OXygen, EN-er-GY - động từ và danh từ chính được nhấn.
- **Câu 5:** SCHOOL, orGANised, VARious, ac-TI-vi-TIES, EARTH - các từ nội dung quan trọng.

Nhớ là khi nói, các em nhấn mạnh các từ mang nghĩa chính và nói nhanh hơn các từ chức năng như "a", "the", "on", "with" để tạo nhịp điệu tự nhiên nha.
</explanation>

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_341bda1a.webm">
Ok vậy là xong bài <eng>A Closer Look 1</eng> Unit 10 rồi nha. Hôm nay các em đã học được từ vựng về môi trường và Trái Đất như <eng>habitat, flora and fauna, nature reserves, food chain, ecological balance, global warming</eng>. Phần phát âm thì các em nhớ về nhịp điệu trong câu - nhấn mạnh các từ mang nghĩa chính và nói nhẹ các từ chức năng. Các em về nhà ôn lại từ vựng và luyện đọc các câu cho tự nhiên nha. Hẹn gặp lại các em ở bài sau!
</teacher_script>
