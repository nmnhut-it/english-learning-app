# UNIT 10: PLANET EARTH
## A CLOSER LOOK 2 - Non-defining Relative Clauses

<!-- chunk: intro -->
<teacher_script pause="0">
Ok lớp 9, Unit 10 nha, bài <eng>A Closer Look 2</eng>. Hôm nay mình học về <eng>Non-defining relative clauses</eng>, tức là mệnh đề quan hệ không xác định. Mở sách trang 107 đi.
</teacher_script>

<!-- chunk: grammar-theory -->
<teacher_script pause="0">
Nè, trước khi làm bài tập thì mình xem qua lý thuyết ngữ pháp đã nha.
</teacher_script>

<grammar>
## Non-defining Relative Clauses (Mệnh đề quan hệ không xác định)

### Remember! (Ghi nhớ!)

**1. Công dụng:**
- We use non-defining relative clauses to add extra information.
- *(Chúng ta sử dụng mệnh đề quan hệ không xác định để bổ sung thêm thông tin.)*

**Example:**
- Earth, **which is the third planet from the Sun**, depends much on the Sun for its energy.
- *(Trái Đất, hành tinh thứ ba tính từ Mặt trời, phụ thuộc nhiều vào Mặt trời về năng lượng.)*

**2. Có thể bỏ mà không ảnh hưởng nghĩa:**
- If we remove the non-defining relative clause, the sentence still makes sense.
- *(Nếu bỏ mệnh đề quan hệ không xác định thì câu vẫn có nghĩa.)*

**Example:**
- Earth depends much on the Sun for its energy.
- *(Trái Đất phụ thuộc nhiều vào Mặt trời về năng lượng.)*

**3. Luôn có dấu phẩy:**
- We use comma(s) with non-defining relative clauses.
- *(Chúng ta sử dụng dấu phẩy với mệnh đề quan hệ không xác định.)*

**4. Không được lược bỏ đại từ quan hệ:**
- Relative pronouns cannot be omitted in non-defining relative clauses.
- *(Đại từ quan hệ không được lược bỏ trong mệnh đề quan hệ không xác định.)*

### Đại từ quan hệ:
| Đại từ | Thay thế cho | Ví dụ |
|--------|--------------|-------|
| **who** | người (chủ ngữ) | Peter, **who** is my friend, lives here. |
| **which** | vật/sự việc | Paris, **which** is beautiful, is in France. |
| **whose** | sở hữu (của ai/cái gì) | The boy, **whose** father is a doctor, is smart. |

**Lưu ý:** KHÔNG dùng **that** trong mệnh đề quan hệ không xác định!
</grammar>

<teacher_script pause="0">
Nhớ nè: Mệnh đề quan hệ không xác định thì PHẢI có dấu phẩy, và KHÔNG được dùng <eng>that</eng> nha. Còn đại từ quan hệ thì dùng <eng>who</eng> cho người, <eng>which</eng> cho vật, <eng>whose</eng> cho sở hữu. Ok giờ mình làm bài tập nha.
</teacher_script>

<!-- chunk: bai-1 -->
### Bài 1 trang 107 - Fill in the blanks

<teacher_script pause="0">
Bài 1, <eng>Complete the sentences with correct relative pronouns</eng>, tức là hoàn thành câu với đại từ quan hệ đúng nha. Đọc kỹ từng câu rồi điền <eng>who</eng>, <eng>which</eng> hoặc <eng>whose</eng> vô chỗ trống.
</teacher_script>

<task>
**Đề:** Complete the sentences with correct relative pronouns.

**Dịch đề:** Hoàn thành câu với đại từ quan hệ đúng.
</task>

<questions type="fill_blanks">
**1.** Planet Earth, _______ is also called the Blue Planet, is covered with water and land.
*Hành tinh Trái Đất, còn được gọi là Hành tinh xanh, được bao phủ bởi nước và đất.*

**2.** Peter Molnar, _______ has done many studies on climate change, is one of the best-known Earth scientists.
*Peter Molnar, người đã thực hiện nhiều nghiên cứu về biến đổi khí hậu, là một trong những nhà khoa học Trái đất nổi tiếng nhất.*

**3.** Neil Armstrong, _______ trip to the moon marked a giant leap for mankind, was the first to walk on the moon.
*Neil Armstrong, người có chuyến du hành lên mặt trăng đánh dấu bước nhảy vọt khổng lồ của nhân loại, là người đầu tiên đi bộ trên mặt trăng.*

**4.** Suoi Thau, _______ is in Ha Giang, is one of the most gorgeous grasslands in Viet Nam.
*Suối Thầu ở Hà Giang là một trong những đồng cỏ đẹp nhất Việt Nam.*

**5.** People often do green things to protect Earth on the Earth Day, _______ is celebrated on 22nd April.
*Mọi người thường làm những việc xanh để bảo vệ Trái đất nhân Ngày Trái đất được tổ chức vào ngày 22 tháng 4.*
</questions>

<teacher_script pause="60">
Các em làm bài 1 đi, 1 phút nha.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** 1. which | 2. who | 3. whose | 4. which | 5. which
</answer>

<explanation>
**Giải thích từng câu:**

**Câu 1: which**
- Planet Earth, **which** is also called the Blue Planet, is covered with water and land.
- Ta dùng **which** vì thay thế cho vật "Planet Earth" (Hành tinh Trái Đất).

**Câu 2: who**
- Peter Molnar, **who** has done many studies on climate change, is one of the best-known Earth scientists.
- Ta dùng **who** vì thay thế cho người "Peter Molnar".

**Câu 3: whose**
- Neil Armstrong, **whose** trip to the moon marked a giant leap for mankind, was the first to walk on the moon.
- Ta dùng **whose** để chỉ sự sở hữu: Neil Armstrong's trip = whose trip (chuyến đi của Neil Armstrong).

**Câu 4: which**
- Suoi Thau, **which** is in Ha Giang, is one of the most gorgeous grasslands in Viet Nam.
- Ta dùng **which** vì thay thế cho vật "Suoi Thau" (Suối Thầu).

**Câu 5: which**
- People often do green things to protect Earth on the Earth Day, **which** is celebrated on 22nd April.
- Ta dùng **which** vì thay thế cho vật "the Earth Day" (Ngày Trái Đất).
</explanation>

<teacher_script pause="0">
Nè, câu 1, 4, 5 dùng <eng>which</eng> vì thay cho vật nha. Câu 2 dùng <eng>who</eng> vì <eng>Peter Molnar</eng> là người. Còn câu 3 chú ý nè, dùng <eng>whose</eng> vì nói về chuyến đi CỦA anh ấy, tức là sở hữu á: <eng>whose trip</eng> nghĩa là chuyến đi của anh ấy.
</teacher_script>

<!-- chunk: bai-2 -->
### Bài 2 trang 107 - Identify and tick

<teacher_script pause="0">
Bài 2, <eng>Underline the relative clauses. Tick if the relative clause can be omitted</eng>. Tức là gạch chân mệnh đề quan hệ, rồi đánh dấu tick nếu có thể lược bỏ được nha.
</teacher_script>

<task>
**Đề:** Underline the relative clauses. Tick (✔) if the relative clause can be omitted.

**Dịch đề:** Gạch chân các mệnh đề quan hệ. Đánh dấu (✔) nếu mệnh đề quan hệ có thể được lược bỏ.
</task>

<questions type="true_false">
**1.** The second planet from the Sun is Venus, which is sometimes called the Earth's sister.
*Hành tinh thứ hai tính từ Mặt trời là Sao Kim, đôi khi được gọi là chị em của Trái Đất.*

**2.** The ocean, which is the body of salt water, contains 97% of Earth's water.
*Đại dương là vùng nước mặn, chứa 97% lượng nước trên Trái Đất.*

**3.** Landforms make up the areas which include mountains, hills, plains, and plateaus.
*Địa hình tạo thành các khu vực bao gồm núi, đồi, đồng bằng và cao nguyên.*

**4.** One of the world's most famous Earth scientists is James Hutton, who is a British geologist.
*Một trong những nhà khoa học Trái đất nổi tiếng nhất thế giới là James Hutton, một nhà địa chất người Anh.*

**5.** Arctic Ocean ice and water make up a habitat for polar bears, whose main food is seals.
*Băng và nước ở Bắc Cực tạo nên môi trường sống cho gấu Bắc Cực, thức ăn chính của chúng là hải cẩu.*
</questions>

<teacher_script pause="60">
Các em đọc kỹ từng câu, gạch chân mệnh đề quan hệ, rồi xem bỏ đi thì câu còn nghĩa không nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**
1. ✔ (có thể bỏ) | 2. ✔ (có thể bỏ) | 3. không tick (không thể bỏ) | 4. ✔ (có thể bỏ) | 5. ✔ (có thể bỏ)
</answer>

<explanation>
**Giải thích từng câu:**

**Câu 1: ✔ (có thể bỏ)**
- Mệnh đề quan hệ: **which is sometimes called the Earth's sister**
- Bỏ đi thì câu vẫn có nghĩa: "The second planet from the Sun is Venus." (Hành tinh thứ hai tính từ Mặt trời là Sao Kim.)
- => Có thể lược bỏ.

**Câu 2: ✔ (có thể bỏ)**
- Mệnh đề quan hệ: **which is the body of salt water**
- Bỏ đi thì câu vẫn có nghĩa: "The ocean contains 97% of Earth's water." (Đại dương chứa 97% lượng nước trên Trái Đất.)
- => Có thể lược bỏ.

**Câu 3: KHÔNG tick (không thể bỏ)**
- Mệnh đề quan hệ: **which include mountains, hills, plains, and plateaus**
- Đây là mệnh đề quan hệ XÁC ĐỊNH (không có dấu phẩy), bỏ đi thì câu không có nghĩa hoàn chỉnh.
- => Không thể lược bỏ.

**Câu 4: ✔ (có thể bỏ)**
- Mệnh đề quan hệ: **who is a British geologist**
- Bỏ đi thì câu vẫn có nghĩa: "One of the world's most famous Earth scientists is James Hutton." (Một trong những nhà khoa học Trái đất nổi tiếng nhất thế giới là James Hutton.)
- => Có thể lược bỏ.

**Câu 5: ✔ (có thể bỏ)**
- Mệnh đề quan hệ: **whose main food is seals**
- Bỏ đi thì câu vẫn có nghĩa: "Arctic Ocean ice and water make up a habitat for polar bears." (Băng và nước ở Bắc Cực tạo nên môi trường sống cho gấu Bắc Cực.)
- => Có thể lược bỏ.
</explanation>

<teacher_script pause="0">
Nè, câu 1, 2, 4, 5 đều là mệnh đề quan hệ KHÔNG xác định, có dấu phẩy, bỏ đi thì câu vẫn có nghĩa nên tick hết nha. Riêng câu 3 thì KHÔNG có dấu phẩy, đây là mệnh đề quan hệ XÁC ĐỊNH, bỏ đi thì câu không rõ nghĩa nên không tick.
</teacher_script>

<!-- chunk: bai-3 -->
### Bài 3 trang 108 - Matching

<teacher_script pause="0">
Bài 3, nối câu với mệnh đề quan hệ không xác định phù hợp nha.
</teacher_script>

<task>
**Đề:** Complete each sentence (1-5) with a non-defining relative clause (A - E).

**Dịch đề:** Hoàn thành mỗi câu (1-5) bằng mệnh đề quan hệ không xác định (A - E).
</task>

<questions type="matching">
**Các mệnh đề quan hệ:**
- **A.** which is a Germanic word *(đó là một từ tiếng Đức)*
- **B.** which is also called the Red Planet *(còn được gọi là Hành tinh Đỏ)*
- **C.** whose radius is nearly 4 million miles *(có bán kính gần 4 triệu dặm)*
- **D.** who explained ecological balance to us *(người đã giải thích sự cân bằng sinh thái cho chúng ta)*
- **E.** which has a large collection of animals, birds and reptiles, and unique plants *(nơi có một bộ sưu tập lớn các loài động vật, chim và bò sát cũng như các loài thực vật độc đáo)*

**Các câu cần hoàn thành:**

**1.** Mars, _______, is very much similar to Earth.
*Sao Hỏa, _______, rất giống Trái Đất.*

**2.** Earth, _______, is the fifth largest planet in our solar system.
*Trái Đất, _______, là hành tinh lớn thứ năm trong hệ mặt trời của chúng ta.*

**3.** The name of our planet, _______, simply means "the ground".
*Tên hành tinh của chúng ta, _______, có nghĩa đơn giản là "mặt đất".*

**4.** Cat Ba National Park, _______, is one of the most famous tropical rainforests in Viet Nam.
*Vườn quốc gia Cát Bà, _______, là một trong những khu rừng mưa nhiệt đới nổi tiếng nhất Việt Nam.*

**5.** We were interested in the talk by Mr An, _______.
*Chúng tôi rất thích thú với bài nói chuyện của ông An, _______.*
</questions>

<teacher_script pause="60">
Các em đọc kỹ từng câu, xem mệnh đề nào phù hợp về nghĩa rồi nối nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:** 1 - B | 2 - C | 3 - A | 4 - E | 5 - D
</answer>

<explanation>
**Giải thích từng câu:**

**1 - B:** Mars, **which is also called the Red Planet**, is very much similar to Earth.
*(Sao Hỏa, còn được gọi là Hành tinh Đỏ, rất giống Trái Đất.)*
- Sao Hỏa có biệt danh là Hành tinh Đỏ do màu sắc của nó.

**2 - C:** Earth, **whose radius is nearly 4 million miles**, is the fifth largest planet in our solar system.
*(Trái Đất có bán kính gần 4 triệu dặm, là hành tinh lớn thứ năm trong hệ mặt trời của chúng ta.)*
- Dùng **whose** vì nói về bán kính CỦA Trái Đất (sở hữu).

**3 - A:** The name of our planet, **which is a Germanic word**, simply means "the ground".
*(Tên hành tinh của chúng ta, là một từ tiếng Đức, có nghĩa đơn giản là "mặt đất".)*
- "Earth" là một từ có nguồn gốc từ tiếng Đức.

**4 - E:** Cat Ba National Park, **which has a large collection of animals, birds and reptiles, and unique plants**, is one of the most famous tropical rainforests in Viet Nam.
*(Vườn quốc gia Cát Bà là nơi có bộ sưu tập lớn các loài động vật, chim, bò sát và thực vật độc đáo, là một trong những khu rừng mưa nhiệt đới nổi tiếng nhất Việt Nam.)*
- Cát Bà nổi tiếng với sự đa dạng sinh học.

**5 - D:** We were interested in the talk by Mr An, **who explained ecological balance to us**.
*(Chúng tôi rất thích thú với bài nói chuyện của ông An, người đã giải thích cho chúng tôi về cân bằng sinh thái.)*
- Dùng **who** vì thay thế cho người "Mr An".
</explanation>

<teacher_script pause="0">
Câu 1 chọn B vì Sao Hỏa còn gọi là <eng>Red Planet</eng>. Câu 2 chọn C, dùng <eng>whose</eng> nói về bán kính CỦA Trái Đất. Câu 3 chọn A vì chữ <eng>Earth</eng> có nguồn gốc tiếng Đức. Câu 4 chọn E vì Cát Bà có nhiều động thực vật đa dạng. Câu 5 chọn D, dùng <eng>who</eng> vì ông An là người.
</teacher_script>

<!-- chunk: bai-4 -->
### Bài 4 trang 108 - Combine sentences

<teacher_script pause="0">
Bài 4, kết hợp hai câu thành một câu sử dụng mệnh đề quan hệ không xác định nha.
</teacher_script>

<task>
**Đề:** Combine the two sentences into one, using a non-defining relative clause.

**Dịch đề:** Kết hợp hai câu thành một, sử dụng mệnh đề quan hệ không xác định.
</task>

<questions type="writing">
**1.** The Moon is Earth's only natural satellite. Its surface is dark.
*Mặt trăng là vệ tinh tự nhiên duy nhất của Trái Đất. Bề mặt của nó có màu tối.*

**2.** Moonquakes are much weaker than Earthquakes. Moonquakes can last up to half an hour.
*Động đất trên Mặt trăng yếu hơn nhiều so với Động đất trên Trái đất. Trận động đất trên Mặt trăng có thể kéo dài tới nửa giờ.*

**3.** Mars has mountains and canyons on its surface. It is a rocky planet like Earth.
*Sao Hỏa có núi và hẻm núi trên bề mặt. Đó là một hành tinh đá giống như Trái Đất.*

**4.** Venus is considered the twin sister of Earth. Venus has a similar size and structure as Earth.
*Sao Kim được coi là chị em song sinh của Trái đất. Sao Kim có kích thước và cấu trúc tương tự Trái Đất.*

**5.** We should protect rivers and lakes. Rivers and lakes provide humans with their main sources of fresh water.
*Chúng ta nên bảo vệ sông hồ. Sông và hồ cung cấp cho con người nguồn nước ngọt chính.*
</questions>

<teacher_script pause="90">
Các em viết câu ghép vô vở nha. 1 phút 30 giây.
</teacher_script>

<teacher_script pause="0" href="audio/tts_29c83a10.mp3">
Ok đáp án nè.
</teacher_script>

<answer>
**Đáp án:**

**1.** The Moon, whose surface is dark, is Earth's only natural satellite.
*(Mặt trăng có bề mặt tối, là vệ tinh tự nhiên duy nhất của Trái Đất.)*

**2.** Moonquakes, which can last up to half an hour, are much weaker than Earthquakes.
*(Động đất trên Mặt trăng có thể kéo dài tới nửa giờ, yếu hơn nhiều so với Động đất trên Trái Đất.)*

**3.** Mars, which is a rocky planet like Earth, has mountains and canyons on its surface.
*(Sao Hỏa, một hành tinh đá giống Trái Đất, có núi và hẻm núi trên bề mặt.)*

**4.** Venus, which has a similar size and structure as Earth, is considered the twin sister of Earth.
*(Sao Kim có kích thước và cấu trúc tương tự Trái Đất, được coi là chị em song sinh của Trái Đất.)*

**5.** We should protect rivers and lakes, which provide humans with their main sources of fresh water.
*(Chúng ta nên bảo vệ sông hồ, nơi cung cấp nguồn nước ngọt chính cho con người.)*
</answer>

<explanation>
**Giải thích từng câu:**

**Câu 1:** Dùng **whose** thay cho tính từ sở hữu "its" (của nó).
- its surface = whose surface
- Mệnh đề quan hệ nằm giữa chủ ngữ và vị ngữ, dùng 2 dấu phẩy ngăn cách.

**Câu 2:** Dùng **which** thay cho chủ ngữ chỉ vật "Moonquakes".
- Mệnh đề quan hệ nằm giữa chủ ngữ và vị ngữ, dùng 2 dấu phẩy ngăn cách.

**Câu 3:** Dùng **which** thay cho chủ ngữ chỉ vật "Mars" (Sao Hỏa).
- Mệnh đề quan hệ nằm giữa chủ ngữ và vị ngữ, dùng 2 dấu phẩy ngăn cách.

**Câu 4:** Dùng **which** thay cho chủ ngữ chỉ vật "Venus" (Sao Kim).
- Mệnh đề quan hệ nằm giữa chủ ngữ và vị ngữ, dùng 2 dấu phẩy ngăn cách.

**Câu 5:** Dùng **which** thay cho tân ngữ chỉ vật "rivers and lakes" (sông hồ).
- Mệnh đề quan hệ nằm cuối câu, dùng 1 dấu phẩy ngăn cách.
</explanation>

<teacher_script pause="0">
Nè, câu 1 đặc biệt nha, dùng <eng>whose</eng> vì <eng>its surface</eng> nghĩa là bề mặt CỦA nó, nên đổi thành <eng>whose surface</eng>. Còn câu 2, 3, 4, 5 đều dùng <eng>which</eng> vì thay cho vật. Nhớ phải có dấu phẩy nha!
</teacher_script>

<!-- chunk: bai-5 -->
### Bài 5 trang 108 - Speaking

<teacher_script pause="0">
Bài 5, làm việc theo nhóm, lần lượt nói một địa điểm trong bảng, nhóm kia giải thích hoặc cung cấp thêm thông tin về địa điểm đó bằng mệnh đề quan hệ không xác định nha.
</teacher_script>

<task>
**Đề:** Work in two groups. Take turns to say aloud one of the places in the table. The other group explains or gives more information about it. The team that has the most correct sentences wins.

**Dịch đề:** Làm việc theo hai nhóm. Lần lượt nói to một trong các địa điểm trong bảng. Nhóm còn lại giải thích hoặc cung cấp thêm thông tin về địa điểm đó. Đội nào có nhiều câu đúng nhất sẽ chiến thắng.
</task>

<questions type="speaking">
| **Place** *(Địa điểm)* | **Feature 1** *(Đặc điểm 1)* | **Feature 2** *(Đặc điểm 2)* |
|------------------------|------------------------------|------------------------------|
| The Nile *(Sông Nin)* | longest river *(con sông dài nhất)* | flows into the Mediterranean Sea *(chảy vào biển Địa Trung Hải)* |
| Pacific Ocean *(Thái Bình Dương)* | covers 30% of planet surface *(bao phủ 30% bề mặt hành tinh)* | largest ocean *(đại dương lớn nhất)* |
| Viet Nam *(Việt Nam)* | has the shape of letter S *(có hình chữ S)* | has a long coastline *(có bờ biển dài)* |
| The Sahara *(Sa mạc Sahara)* | largest desert *(sa mạc lớn nhất)* | has almost no rainfall *(hầu như không có mưa)* |
| Mount Fansipan *(Đỉnh Fanxipan)* | highest peak in Viet Nam *(đỉnh cao nhất Việt Nam)* | also called the 'Roof of Indochina' *(còn được mệnh danh là 'Nóc nhà Đông Dương')* |

**Example:**
- **A:** The Nile
- **B:** The Nile, which is the longest river, flows into the Mediterranean Sea.
*(Sông Nile, con sông dài nhất, chảy vào biển Địa Trung Hải.)*
</questions>

<teacher_script pause="60">
Các em nhìn bảng rồi đặt câu theo mẫu nha. 1 phút.
</teacher_script>

<teacher_script pause="0">
Ok, đây là các câu mẫu nha.
</teacher_script>

<answer>
**Câu mẫu:**

**1. The Nile:**
The Nile, which is the longest river, flows into the Mediterranean Sea.
*(Sông Nile, con sông dài nhất, chảy vào biển Địa Trung Hải.)*

**2. Pacific Ocean:**
The Pacific Ocean, which covers 30% of the planet's surface, is the largest ocean.
*(Thái Bình Dương, chiếm 30% bề mặt hành tinh, là đại dương lớn nhất.)*

**3. Viet Nam:**
Viet Nam, which has the shape of letter S, has a long coastline.
*(Việt Nam có hình chữ S, có đường bờ biển dài.)*

**4. The Sahara:**
The Sahara, which is the largest desert, has almost no rainfall.
*(Sahara, sa mạc lớn nhất, hầu như không có lượng mưa.)*

**5. Mount Fansipan:**
Mount Fansipan, which is the highest peak in Viet Nam, is also called the 'Roof of Indochina'.
*(Đỉnh Fanxipan, đỉnh núi cao nhất Việt Nam, còn được gọi là 'Nóc nhà Đông Dương'.)*
</answer>

<teacher_script pause="0">
Nè, các em thấy không, mỗi câu đều có cấu trúc: Địa điểm, <eng>which</eng> + đặc điểm 1, đặc điểm 2. Nhớ phải có dấu phẩy trước và sau mệnh đề quan hệ nha.
</teacher_script>

<!-- chunk: end -->
<teacher_script pause="0">
Ok, xong bài <eng>A Closer Look 2</eng> Unit 10 rồi nha. Hôm nay mình học về mệnh đề quan hệ không xác định. Nhớ nè: LUÔN có dấu phẩy, KHÔNG dùng <eng>that</eng>, và có thể bỏ đi mà câu vẫn có nghĩa. Về nhà ôn lại bài và chuẩn bị bài <eng>Communication</eng> nha. Tạm biệt các em!
</teacher_script>
