...
### Form 1: Suggest + V-ing
Dùng khi đề xuất hành động chung chung hoặc chính mình thực hiện.

| Structure | Example |
|-----------|---------|
| S + suggest + V-ing | I suggest **trying** the new app. |
| S + suggest + V-ing | She suggested **using** a tablet instead. |

### Form 2: Suggest + that + S2 + (should) + V (bare infinitive)
Dùng khi đề xuất người khác làm gì — không chia động từ theo chủ ngữ!

| Structure | Example |
|-----------|---------|
| S + suggest + that + S2 + (should) + V | I suggest (that) he **(should) be** quiet. |
| S + suggest + that + S2 + (should) + V | She suggested (that) we **(should) try** snorkelling. |

### Lưu ý quan trọng:
- Sau *suggest + that*, động từ dùng **dạng nguyên mẫu (base form)** — KHÔNG chia theo chủ ngữ!
- *that* có thể bỏ đi — vẫn đúng ngữ pháp.
- Phủ định: *suggest + **not** + V-ing* / *suggest + that + S + **not** + V*

### Động từ tương tự:
**advise** (khuyên) và **recommend** (đề xuất) — cùng cấu trúc!

| Verb | V-ing form | that-clause |
|------|-----------|-------------|
| suggest | suggest + V-ing | suggest that S + (should) + V |
| recommend | recommend + V-ing | recommend that S + (should) + V |
| advise | advise + V-ing | advise + O + to-V *(khác!)* |
...
<task>
**Đề:** Choose the correct form A, B, C, or D.
**Dịch đề:** Chọn dạng đúng A, B, C hoặc D.
</task>
...
**Dịch đề:** Điền dạng đúng của động từ trong ngoặc vào chỗ trống.
</task>
