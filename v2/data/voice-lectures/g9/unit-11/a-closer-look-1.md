# UNIT 11: ELECTRONIC DEVICES
## A CLOSER LOOK 1 - Vocabulary & Pronunciation

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_0e8e6983.mp3">
Ok lớp 9, Unit 11 bài <eng>A Closer Look 1</eng> nha. Mở sách trang 116 đi. Bài này mình học từ vựng về chủ đề thiết bị điện tử và vật liệu, rồi phần phát âm về trọng âm trong câu ngắn.
</teacher_script>

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_f0c9c62f.mp3">
Trước tiên mình học từ vựng nha. Bấm nút "Bắt đầu" bên dưới để học từ vựng. Máy sẽ đọc và các em đọc theo. Sau đó làm bài trắc nghiệm từ vựng rồi ghi từ vựng vô tập.
</teacher_script>

<vocabulary>
1. **robotic vacuum cleaner** : máy hút bụi tự động /rəʊˈbɒtɪk ˈvækjuːm ˈkliːnə/
2. **e-reader** : thiết bị đọc sách điện tử /iː ˈriːdə/
3. **smartwatch** : đồng hồ thông minh /smɑːt wɒtʃ/
4. **portable music player** : máy nghe nhạc di động /ˈpɔːtəbl ˈmjuːzɪk ˈpleɪə/
5. **3D printer** : máy in 3D /θriː diː ˈprɪntə/
6. **camcorder** : máy quay phim /ˈkæmˌkɔːdə/
7. **rubber** : (n) cao su /ˈrʌbə/
8. **cardboard** : (n) bìa cứng /ˈkɑːdbɔːd/
9. **steel** : (n) thép /stiːl/
10. **plastic** : (n) nhựa /ˈplæstɪk/
11. **copper** : (n) đồng /ˈkɒpə/
12. **soft reddish-brown metal** : kim loại màu nâu đỏ mềm /sɒft ˈredɪʃ braʊn ˈmetl/
13. **electric wire** : dây điện /ɪˈlektrɪk waɪə/
14. **pipe** : (n) ống /paɪp/
15. **conduct electricity** : dẫn điện /kənˈdʌkt ɪlekˈtrɪsəti/
16. **shape** : (v) định hình /ʃeɪp/
17. **construction industry** : ngành công nghiệp xây dựng /kənˈstrʌkʃn ˈɪndəstri/
18. **tyre** : (n) lốp xe /ˈtaɪə/
19. **bridge** : (n) cầu /brɪdʒ/
20. **medical implants** : cấy ghép y tế /ˈmedɪkl ˈɪmplɑːnts/
21. **food packaging** : bao bì thực phẩm /fuːd ˈpækɪdʒɪŋ/
22. **cereal** : (n) ngũ cốc /ˈsɪəriəl/
23. **live-streaming** : phát trực tuyến /laɪv ˈstriːmɪŋ/
24. **live** : (adj, adv) trực tiếp /laɪv/
25. **live** : (v) sống /lɪv/
26. **webinar** : (n) hội thảo trực tuyến /ˈwebɪnɑː/
</vocabulary>

<!-- chunk: exercise-1 -->
### Bai 1 trang 116 - Matching

<teacher_script pause="0" href="audio/tts_c21781fe.mp3">
Bài 1 <eng>Match the electronic devices with the correct pictures</eng> nha. Nối các thiết bị điện tử với tranh ảnh đúng. Sách có hình ảnh, các em đọc mô tả bên dưới rồi chọn thiết bị đúng nha.
</teacher_script>

<task>
**De:** Match the electronic devices with the correct pictures.

**Dich de:** Nối các thiết bị điện tử với tranh ảnh đúng.
</task>

<questions type="matching">
**Thiet bi:**
1. robotic vacuum cleaner
2. e-reader
3. smartwatch
4. portable music player
5. 3D printer
6. camcorder

**Mo ta:**
- a. a watch that can connect to the Internet
- b. a machine that creates solid objects from digital models
- c. a handheld device for reading digital books
- d. a small robot that cleans floors automatically
- e. a small device for listening to music on the go
- f. a device for recording videos

*Dich mo ta:*
- *a. một chiếc đồng hồ có thể kết nối Internet*
- *b. một cỗ máy tạo ra vật thể rắn từ mô hình kỹ thuật số*
- *c. một thiết bị cầm tay để đọc sách điện tử*
- *d. một robot nhỏ tự động lau sàn nhà*
- *e. một thiết bị nhỏ để nghe nhạc khi di chuyển*
- *f. một thiết bị để quay video*
</questions>

<teacher_script pause="45" href="audio/tts_2622cb66.mp3">
Các em làm bài 1 nha. 45 giây.
</teacher_script>

<teacher_script pause="0" href="audio/tts_6a173489.mp3">
Ok đáp án bài 1 nè. Thầy giải thích từng câu luôn nha.
</teacher_script>

<answer>
**Đáp án:** 1.d | 2.c | 3.a | 4.e | 5.b | 6.f
</answer>

<explanation>
**1. d - robotic vacuum cleaner = a small robot that cleans floors automatically**
<eng>Robotic vacuum cleaner</eng> là máy hút bụi tự động - đúng rồi, nó là robot nhỏ tự động lau sàn nhà. Từ "robotic" nghĩa là tự động, "vacuum" là hút bụi, "cleaner" là máy lau dọn.

**2. c - e-reader = a handheld device for reading digital books**
<eng>E-reader</eng> là thiết bị đọc sách điện tử - một thiết bị cầm tay để đọc sách số. "E" là viết tắt của "electronic", "reader" là thiết bị đọc.

**3. a - smartwatch = a watch that can connect to the Internet**
<eng>Smartwatch</eng> là đồng hồ thông minh - đồng hồ có thể kết nối Internet. "Smart" nghĩa là thông minh, nó làm được nhiều thứ hơn đồng hồ thường.

**4. e - portable music player = a small device for listening to music on the go**
<eng>Portable music player</eng> là máy nghe nhạc di động - thiết bị nhỏ để nghe nhạc khi di chuyển. "Portable" nghĩa là di động, mang theo được.

**5. b - 3D printer = a machine that creates solid objects from digital models**
<eng>3D printer</eng> là máy in 3D - máy tạo ra vật thể rắn từ mô hình kỹ thuật số. Máy in 3D in ra đồ vật thật từ bản thiết kế trên máy tính á.

**6. f - camcorder = a device for recording videos**
<eng>Camcorder</eng> là máy quay phim - thiết bị để quay video. "Cam" từ "camera", "corder" từ "recorder".
</explanation>

<!-- chunk: exercise-2 -->
### Bai 2 trang 116 - Fill in the blanks

<teacher_script pause="0" href="audio/tts_978658b4.mp3">
Bài 2 <eng>Complete the sentences with the material words from the box</eng> nha. Hoàn thành các câu với các từ ngữ về vật liệu trong khung. Các em đọc mô tả rồi đoán xem đó là vật liệu gì nha.
</teacher_script>

<task>
**De:** Complete the sentences with the material words from the box.

**Dich de:** Hoàn thành các câu với các từ ngữ về vật liệu trong khung.
</task>

<questions type="fill_blanks">
| copper | plastic | steel | rubber | cardboard |
|--------|---------|-------|--------|-----------|

**1.** We often use _______ which is a soft reddish-brown metal, to make electric wire, pipes, and coins.
*Chúng ta thường dùng _______, là một kim loại màu nâu đỏ mềm, để làm dây điện, ống và tiền xu.*

**2.** Today, _______ is often used to make cases of electric devices because it doesn't conduct electricity and it is light, and easily be shaped into many different forms.
*Ngày nay, _______ thường được dùng để làm vỏ thiết bị điện vì nó không dẫn điện, nhẹ và dễ tạo thành nhiều hình dạng khác nhau.*

**3.** We often use _______ in the construction industry because it is a strong and hard metal.
*Chúng ta thường dùng _______ trong ngành công nghiệp xây dựng vì nó là kim loại cứng và bền.*

**4.** We use _______, which comes from rubber trees, for making tyres, boots, etc.
*Chúng ta dùng _______, lấy từ cây cao su, để làm lốp xe, ủng, v.v.*

**5.** The kids need a piece of _______ to easily cut shapes and make toys.
*Các em nhỏ cần một miếng _______ để dễ dàng cắt hình và làm đồ chơi.*
</questions>

<teacher_script pause="60" href="audio/tts_77b682b1.mp3">
Các em làm bài 2 nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_fe1598eb.mp3">
Ok đáp án bài 2 nè. Thầy giải thích từng câu luôn nha.
</teacher_script>

<answer>
**Đáp án:** 1. copper | 2. plastic | 3. steel | 4. rubber | 5. cardboard
</answer>

<explanation>
**Câu 1. copper** - <eng>Copper</eng> là đồng, một kim loại màu nâu đỏ mềm. Đồng dẫn điện tốt nên được dùng làm dây điện và ống dẫn. Ngoài ra đồng còn được dùng để đúc tiền xu - đó là lý do tiền xu thường có màu nâu đỏ.

**Câu 2. plastic** - <eng>Plastic</eng> là nhựa. Nhựa được dùng làm vỏ (case) của thiết bị điện vì ba lý do: (1) không dẫn điện, tức là an toàn khi tiếp xúc với điện; (2) nhẹ; (3) dễ tạo hình thành nhiều dạng khác nhau. Các em thấy vỏ điện thoại, vỏ laptop đều làm bằng nhựa á!

**Câu 3. steel** - <eng>Steel</eng> là thép, kim loại rất cứng và chắc. Thép được dùng nhiều nhất trong ngành xây dựng vì nó đủ bền để chịu được tải trọng lớn của tòa nhà và cầu đường.

**Câu 4. rubber** - <eng>Rubber</eng> là cao su, lấy từ cây cao su. Cao su dẻo và đàn hồi nên dùng làm lốp xe để bám đường tốt, và làm ủng (boots) để chống nước khi đi mưa.

**Câu 5. cardboard** - <eng>Cardboard</eng> là bìa cứng, giấy dày và cứng. Bìa cứng dễ cắt và tạo hình nên các em nhỏ hay dùng để làm đồ thủ công và đồ chơi. Các em biết làm mô hình từ hộp carton không? Đó chính là bìa cứng nè!
</explanation>

<!-- chunk: exercise-3 -->
### Bai 3 trang 116 - Circle the correct words

<teacher_script pause="0" href="audio/tts_0606d2ac.mp3">
Bài 3 <eng>Circle the correct words or phrases to complete the following sentences</eng> nha. Khoanh tròn từ hoặc cụm từ đúng để hoàn thành câu. Mỗi câu có 2 lựa chọn, các em chọn từ phù hợp nhất.
</teacher_script>

<task>
**De:** Circle the correct words or phrases to complete the following sentences.

**Dich de:** Khoanh tròn những từ hoặc cụm từ đúng để hoàn thành những câu sau.
</task>

<questions type="multiple_choice">
**1.** The most common use for **rubber / iron** is for producing tyres for cars, motorbikes, bicycles, etc.
*Ứng dụng phổ biến nhất của _______ là sản xuất lốp ô tô, xe máy, xe đạp, v.v.*

**2.** The most popular material for the construction of buildings and bridges is **plastic / steel** because it is strong enough to support heavy loads.
*Vật liệu phổ biến nhất để xây dựng tòa nhà và cầu là _______ vì nó đủ bền để chịu tải trọng lớn.*

**3.** Today 3D **printers / photocopiers** allow us to create three-dimensional objects, eg. medical implants, clothes, cars, etc.
*Ngày nay _______ cho phép chúng ta tạo ra vật thể ba chiều, ví dụ: cấy ghép y tế, quần áo, ô tô, v.v.*

**4.** We use **copper / cardboard** to make food packaging such as cereal and pasta boxes, as it is food-safe and not harmful to the environment.
*Chúng ta dùng _______ để làm bao bì thực phẩm như hộp ngũ cốc và mì ống vì nó an toàn cho thực phẩm và không gây hại cho môi trường.*

**5.** They use **camcorders / portable music players** for live-streaming video content such as concerts, live events, and webinars.
*Họ sử dụng _______ để phát trực tiếp nội dung video như buổi hòa nhạc, sự kiện trực tiếp, và hội thảo trên web.*
</questions>

<teacher_script pause="60" href="audio/tts_1fcbe2db.mp3">
Các em làm bài 3 nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_4eeabd35.mp3">
Ok đáp án bài 3 nè. Thầy giải thích từng câu luôn nghen.
</teacher_script>

<answer>
**Đáp án:** 1. rubber | 2. steel | 3. printers | 4. cardboard | 5. camcorders
</answer>

<explanation>
**Câu 1. rubber** - Cao su được dùng phổ biến nhất để sản xuất lốp xe. Iron = sắt, không phù hợp vì lốp xe cần vật liệu đàn hồi, không phải kim loại cứng.

**Câu 2. steel** - Thép là vật liệu phổ biến nhất trong xây dựng tòa nhà và cầu vì nó đủ bền để chịu tải trọng nặng. Plastic = nhựa, quá yếu cho xây dựng.

**Câu 3. printers** - 3D printers = máy in 3D, cho phép tạo ra vật thể 3 chiều như cấy ghép y tế, quần áo, ô tô. Photocopiers = máy photocopy, chỉ sao chép giấy 2D thôi, không tạo được vật thể 3D nha.

**Câu 4. cardboard** - Bìa cứng dùng để làm bao bì thực phẩm vì nó an toàn cho thực phẩm và thân thiện với môi trường. Copper = đồng, là kim loại, không dùng để đóng gói thực phẩm.

**Câu 5. camcorders** - Máy quay phim dùng để phát trực tiếp video. Portable music players = máy nghe nhạc di động, chỉ dùng để nghe nhạc chứ không quay video được nha.
</explanation>

<!-- chunk: pronunciation-theory -->
### Pronunciation - Stress on all words in sentences

<teacher_script pause="0" href="audio/tts_399bb8a0.mp3">
Ok bây giờ mình qua phần phát âm nha. Chủ đề là <eng>Stress on all words in sentences</eng> - Nhấn mạnh vào tất cả các từ trong câu. Đây là kiểu nhấn âm đặc biệt khi câu rất ngắn, chỉ có 2-3 từ thôi á.
</teacher_script>

<grammar>
**STRESS ON ALL WORDS IN SENTENCES**
**Nhấn mạnh vào tất cả các từ trong câu**

Trong những câu rất ngắn (mệnh lệnh, cảm thán, câu 2-3 từ), **tất cả các từ đều được nhấn mạnh**.

**Hai kiểu nhấn âm (stress pattern):**

**1. Kiểu OO (2 từ, mỗi từ 1 âm tiết, đều nhấn mạnh):**
- **'Don't 'talk!** *(Đừng nói chuyện!)*
- **'Look 'out!** *(Coi chừng!)*
- **'I 'see!** *(Tôi hiểu rồi!)*

**2. Kiểu OOo (2-3 từ, từ cuối có 2 âm tiết, nhấn ở âm tiết đầu):**
- **'Don't 'worry!** *(Đừng lo lắng!)* - "worry" nhấn ở "wor"
- **'Be 'quiet!** *(Hãy im lặng!)* - "quiet" nhấn ở "qui"
- **'Say 'sorry!** *(Nói lời xin lỗi!)* - "sorry" nhấn ở "sor"

**Lưu ý:** Trong những câu ngắn mang tính mệnh lệnh hoặc cảm thán, tất cả các từ đều quan trọng nên đều được nhấn. Khác với câu dài nơi chỉ nhấn từ chính (content words).
</grammar>

<teacher_script pause="0" href="audio/tts_f04276f4.mp3">
Nói đơn giản thì khi câu chỉ có 2-3 từ ngắn, mình nhấn hết tất cả các từ luôn á. Ví dụ <eng>Don't talk!</eng> thì cả <eng>Don't</eng> và <eng>talk</eng> đều nhấn mạnh. <eng>Look out!</eng> cũng vậy - nhấn cả 2 từ. Dễ nhớ hen!
</teacher_script>

<!-- chunk: exercise-4 -->
### Bài 4 trang 117 - Listen and repeat

<teacher_script pause="0" href="audio/tts_82d95f14.mp3">
Bài 4 <eng>Listen and repeat the sentences. Pay attention to the stress pattern of each sentence.</eng> Nghe và nhắc lại các câu. Chú ý kiểu nhấn âm của mỗi câu nha.
</teacher_script>

<task>
**Đề:** Listen and repeat the sentences. Pay attention to the stress pattern of each sentence.

**Dịch đề:** Nghe và nhắc lại câu. Hãy chú ý đến kiểu nhấn âm của mỗi câu.
</task>

<audio src="https://img.loigiaihay.com/picture/2024/0516/68.mp3">
**Audio:** Bài 4 - Stress on all words in sentences
</audio>

<questions type="speaking">
**1.** Don't talk!
*Đừng nói chuyện!*

**2.** Don't worry!
*Đừng lo lắng!*

**3.** Keep silent!
*Hãy im lặng!*

**4.** Look out!
*Coi chừng!*

**5.** Remember!
*Hãy nhớ!*
</questions>

<teacher_script pause="45" href="audio/tts_797303ae.mp3">
Các em nghe và nhắc lại từng câu nha. Chú ý nhấn mạnh tất cả các từ trong câu. 45 giây.
</teacher_script>

<teacher_script pause="0" href="audio/tts_52a3f739.mp3">
Ok, các em để ý nha. Tất cả 5 câu đều rất ngắn và mang tính mệnh lệnh. Trong những câu như vậy, mỗi từ đều quan trọng nên mình nhấn mạnh hết:
- **'Don't 'talk!** - Nhấn cả "Don't" và "talk"
- **'Don't 'worry!** - Nhấn "Don't" và "wor" (âm tiết đầu của "worry")
- **'Keep 'silent!** - Nhấn "Keep" và "si" (âm tiết đầu của "silent")
- **'Look 'out!** - Nhấn cả "Look" và "out"
- **Re'member!** - Nhấn ở "mem" (âm tiết thứ hai)
</teacher_script>

<!-- chunk: exercise-5 -->
### Bài 5 trang 117 - Classify stress patterns

<teacher_script pause="0" href="audio/tts_6b9c0a3f.mp3">
Bài 5 <eng>Write each sentence in the suitable box. Then listen, check, and repeat.</eng> Phân loại các câu vào ô phù hợp rồi nghe kiểm tra nha. Có 2 kiểu: <eng>OO</eng> (2 từ đều 1 âm tiết, nhấn đều) và <eng>OOo</eng> (từ cuối có 2 âm tiết).
</teacher_script>

<task>
**Đề:** Write each sentence in the suitable box. Then listen, check, and repeat.

**Dịch đề:** Viết từng câu vào ô thích hợp. Sau đó nghe, kiểm tra và lặp lại.
</task>

<audio src="https://img.loigiaihay.com/picture/2024/0516/69.mp3">
**Audio:** Bài 5 - Stress patterns OO vs OOo
</audio>

<questions type="matching">
**Các câu cần phân loại:**
1. I see! *(Tôi hiểu rồi!)*
2. Be quiet! *(Hãy im lặng!)*
3. Say sorry! *(Nói lời xin lỗi!)*
4. Watch out! *(Coi chừng!)*
5. That long? *(Lâu vậy sao?)*
6. Speak louder! *(Hãy nói to hơn!)*

**Hai ô phân loại:**
- **OO** (2 từ đều 1 âm tiết)
- **OOo** (từ cuối có 2 âm tiết, nhấn ở âm tiết đầu)
</questions>

<teacher_script pause="60" href="audio/tts_8b54d40b.mp3">
Các em phân loại 6 câu vào 2 ô OO và OOo nha. 1 phút.
</teacher_script>

<teacher_script pause="0" href="audio/tts_6afd6e14.mp3">
Ok đáp án bài 5 nè.
</teacher_script>

<answer>
**Bài 5 - Đáp án:**

| **OO** | **OOo** |
|--------|---------|
| 1. **'I 'see!** | 2. **'Be 'quiet!** |
| 4. **'Watch 'out!** | 3. **'Say 'sorry!** |
| 5. **'That 'long?** | 6. **'Speak 'louder!** |
</answer>

<explanation>
**Kiểu OO** (2 từ đều 1 âm tiết, nhấn đều):
- **'I 'see!** - "I" và "see" đều 1 âm tiết, nhấn cả 2.
- **'Watch 'out!** - "Watch" và "out" đều 1 âm tiết, nhấn cả 2.
- **'That 'long?** - "That" và "long" đều 1 âm tiết, nhấn cả 2.

**Kiểu OOo** (từ cuối có 2 âm tiết, nhấn ở âm tiết đầu):
- **'Be 'quiet!** - "Be" 1 âm tiết, "quiet" có 2 âm tiết (qui-et), nhấn ở "qui". Tổng cộng là OOo.
- **'Say 'sorry!** - "Say" 1 âm tiết, "sorry" có 2 âm tiết (sor-ry), nhấn ở "sor". Tổng cộng là OOo.
- **'Speak 'louder!** - "Speak" 1 âm tiết, "louder" có 2 âm tiết (lou-der), nhấn ở "lou". Tổng cộng là OOo.

**Mẹo phân loại:** Nhìn từ cuối cùng - nếu nó chỉ có 1 âm tiết (see, out, long) thì là OO. Nếu từ cuối có 2 âm tiết (quiet, sorry, louder) thì là OOo nha!
</explanation>

<!-- chunk: end -->
<teacher_script pause="0" href="audio/tts_af1a4092.mp3">
Ok vậy là xong bài <eng>A Closer Look 1</eng> Unit 11 rồi nha. Hôm nay các em đã học được từ vựng về thiết bị điện tử như <eng>robotic vacuum cleaner, e-reader, smartwatch, portable music player, 3D printer, camcorder</eng> và các vật liệu như <eng>copper, plastic, steel, rubber, cardboard</eng>. Phần phát âm thì các em nhớ quy tắc nhấn âm trong câu ngắn nha - câu 2-3 từ mang tính mệnh lệnh thì nhấn mạnh tất cả các từ. Phân biệt kiểu OO (2 từ 1 âm tiết) và OOo (từ cuối 2 âm tiết). Các em về nhà ôn lại từ vựng và luyện đọc các câu ngắn cho quen nha. Hẹn gặp lại các em ở bài sau!
</teacher_script>
