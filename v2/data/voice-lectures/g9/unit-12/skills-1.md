# UNIT 12: MY FUTURE CAREER
## SKILLS 1 - Đọc & Nói

<!-- chunk: vocabulary -->
<teacher_script pause="0" href="audio/tts_18614a92.mp3">
Ok lớp 9, bài <eng>Skills 1</eng> Unit 12 nha — hôm nay luyện đọc và nói. Chủ đề là nghề nghiệp trong thế giới đang thay đổi. Mở sách trang 133 đi. Ghi tựa bài, rồi bấm "Bắt đầu" học từ vựng trước nha.
</teacher_script>

<vocabulary>
1. **manicurist** : (n) thợ làm móng /ˈmænɪkjʊərɪst/
2. **receptionist** : (n) nhân viên lễ tân /rɪˈsepʃənɪst/
3. **repetitive** : (adj) lặp đi lặp lại /rɪˈpetətɪv/
4. **hairdresser** : (n) thợ cắt tóc /ˈheəˌdresər/
5. **collaborate** : (v) hợp tác /kəˈlæbəreɪt/
6. **vocational** : (adj) hướng nghiệp, dạy nghề /vəˈkeɪʃənl/
7. **qualification** : (n) bằng cấp, chứng chỉ /ˌkwɒlɪfɪˈkeɪʃən/
8. **inquiring** : (adj) ham tìm hiểu /ɪnˈkwaɪərɪŋ/
9. **assembly worker** : (n) công nhân lắp ráp /əˈsembli ˈwɜːrkər/
10. **replace** : (v) thay thế /rɪˈpleɪs/
</vocabulary>

<teacher_script pause="0" href="audio/tts_f264062a.mp3">
Từ vựng xong rồi nha. Giờ qua bài 1 — nhìn hình và trả lời câu hỏi về các nghề nghiệp.
</teacher_script>

<!-- chunk: exercise-1 -->
<teacher_script pause="45" href="audio/tts_95dce91f.mp3">
Bài 1 trang 133 — nhìn vào các công việc trong hình và trả lời 3 câu hỏi. Làm nhanh trong 1 phút nha.
</teacher_script>

<task>
**Đề:** Look at the jobs below and answer the questions.
**Dịch đề:** Hãy quan sát các công việc dưới đây và trả lời các câu hỏi.
</task>

<questions type="answer_questions">
**1.** Which job is most repetitive?
*Công việc nào lặp đi lặp lại nhiều nhất?*

**2.** Which job relates to beauty?
*Công việc nào liên quan đến sắc đẹp?*

**3.** Which job depends heavily on technology?
*Công việc nào phụ thuộc nhiều vào công nghệ?*
</questions>

<teacher_script pause="0" href="audio/tts_7242caec.mp3">
Ok đáp án bài 1 nè.
</teacher_script>

<answer>
**Bài 1 - Đáp án:**
**1.** A ticket seller job (in picture c) is most repetitive.
**2.** A nail artist (in picture a) relates to beauty.
**3.** Online teacher (in picture b) depends heavily on technology.
</answer>

<explanation>
**Câu 1:** Ticket seller — bán vé là công việc lặp đi lặp lại nhiều nhất vì mỗi lần chỉ làm một thao tác giống nhau.

**Câu 2:** Nail artist — thợ làm móng liên quan đến sắc đẹp (beauty). Manicurist cũng là cách gọi khác nha.

**Câu 3:** Online teacher — giáo viên trực tuyến phụ thuộc nhiều vào công nghệ (máy tính, internet, phần mềm dạy học).
</explanation>

<!-- chunk: reading -->
<teacher_script pause="0" href="audio/tts_421a4e0f.mp3">
Bài 2 trang 133 — đọc bài báo về thế giới việc làm đang thay đổi. Đọc kỹ nha vì có bài tập theo sau.
</teacher_script>

<reading>
When you turn 15, you should start thinking seriously about your career. However, making career choices can be challenging, especially in this changing world.

1. Assembly workers and ticket sellers, whose jobs are repetitive, are being replaced by machines. However, technology has also created new jobs, such as software engineers and online teachers. Besides, beauty jobs like nail artists and hairdressers are becoming popular because people want to take better care of themselves.

2. Computer skills have become a must for many jobs. For example, doctors need to keep digital medical records of their patients. People also collaborate with each other so often that teamwork and communication skills are now increasingly important.

3. Many teenagers are willing to take vocational courses instead of going to university. Training can be both face-to-face and online. Opportunities to learn new skills are open to everyone provided that they have inquiring minds.

In the future, there may be even more changes in the world of work. It's a good idea for you to work well in teams, keep on learning and have good computer skills. By doing so, you can move confidently on your career path.
</reading>

<translation>
**Bản dịch:**

Khi bước sang tuổi 15, bạn nên bắt đầu suy nghĩ nghiêm túc về sự nghiệp của mình. Tuy nhiên, việc lựa chọn nghề nghiệp có thể là một thách thức, đặc biệt là trong thế giới đang thay đổi này.

1. Công nhân lắp ráp và bán vé, những công việc lặp đi lặp lại, đang bị thay thế bởi máy móc. Tuy nhiên, công nghệ cũng tạo ra những việc làm mới, chẳng hạn như kỹ sư phần mềm và giáo viên trực tuyến. Bên cạnh đó, những công việc làm đẹp như làm móng, làm tóc đang trở nên phổ biến vì mọi người muốn chăm sóc bản thân tốt hơn.

2. Kỹ năng máy tính đã trở thành điều bắt buộc đối với nhiều công việc. Ví dụ, bác sĩ cần lưu giữ hồ sơ y tế kỹ thuật số của bệnh nhân. Mọi người cũng cộng tác với nhau thường xuyên nên kỹ năng làm việc nhóm và giao tiếp ngày càng trở nên quan trọng.

3. Nhiều thanh thiếu niên sẵn sàng tham gia các khóa học nghề thay vì học đại học. Đào tạo có thể được thực hiện trực tiếp và trực tuyến. Cơ hội học hỏi các kỹ năng mới luôn mở ra cho tất cả mọi người miễn là họ có đầu óc ham học hỏi.

Trong tương lai, có thể còn có nhiều thay đổi hơn nữa trong thế giới việc làm. Bạn nên làm việc nhóm tốt, không ngừng học hỏi và có kỹ năng máy tính tốt. Làm được như vậy, bạn có thể tự tin bước đi trên con đường sự nghiệp của mình.
</translation>

<teacher_script pause="180" href="audio/tts_41ca9ad9.mp3">
Đọc lại bài và dịch vô vở. 3 phút nha. Chú ý 3 đoạn giữa — mỗi đoạn nói về một khía cạnh khác nhau của thế giới việc làm.
</teacher_script>

<!-- chunk: exercise-2a -->
<teacher_script pause="60" href="audio/tts_31c70004.mp3">
Bài 2a trang 134 — mỗi câu A, B, C phù hợp với đoạn số mấy (1, 2, hoặc 3)? Đánh dấu vào ô đúng. Làm 1 phút nha.
</teacher_script>

<task>
**Đề:** Which number (1-3) does each sentence (A - C) below fit in? Tick (✔) the correct number for each sentence.
**Dịch đề:** Mỗi câu (A - C) dưới đây phù hợp với số nào (1-3)? Hãy đánh dấu (✔) vào số đúng cho mỗi câu.
</task>

<questions type="matching">
A. The skills needed for the jobs are changing, too.
*Các kỹ năng cần thiết cho công việc cũng đang thay đổi.*

B. The preferred type of training is also changing.
*Loại hình đào tạo ưa thích cũng đang thay đổi.*

C. The kinds of jobs people are doing are changing.
*Các loại công việc mọi người đang làm đang thay đổi.*
</questions>

<teacher_script pause="0" href="audio/tts_084aec5f.mp3">
Ok đáp án 2a nè.
</teacher_script>

<answer>
**Bài 2a - Đáp án:** A - 2 | B - 3 | C - 1
</answer>

<explanation>
**A - Đoạn 2:** "The skills needed for the jobs are changing, too." — Đoạn 2 nói về kỹ năng máy tính, teamwork, communication đang trở nên quan trọng hơn → kỹ năng cần thiết đang thay đổi.

**B - Đoạn 3:** "The preferred type of training is also changing." — Đoạn 3 nói "Many teenagers are willing to take vocational courses instead of going to university. Training can be both face-to-face and online." → cách học/đào tạo đang thay đổi.

**C - Đoạn 1:** "The kinds of jobs people are doing are changing." — Đoạn 1 nói các công việc lặp lại bị máy thay thế, công việc mới xuất hiện (kỹ sư phần mềm, giáo viên online, làm đẹp) → loại công việc đang thay đổi.
</explanation>

<!-- chunk: exercise-2b -->
<teacher_script pause="30" href="audio/tts_52112bda.mp3">
Bài 2b — chọn tiêu đề phù hợp nhất cho bài báo. 3 lựa chọn A, B, C. Làm nhanh nha.
</teacher_script>

<task>
**Đề:** Which of the following is the best title for the article?
**Dịch đề:** Tiêu đề nào sau đây là tiêu đề phù hợp nhất cho bài viết?
</task>

<questions type="multiple_choice">
**Chọn tiêu đề phù hợp nhất:**
- A. The changing world of work
- B. How to choose your career path
- C. Making good job decisions

*Tiêu đề nào phù hợp nhất với nội dung bài báo?*
</questions>

<teacher_script pause="0" href="audio/tts_dfae559b.mp3">
Đáp án nè.
</teacher_script>

<answer>
**Bài 2b - Đáp án: A. The changing world of work**
*(Thế giới việc làm đang thay đổi)*
</answer>

<explanation>
**A đúng** vì toàn bộ bài nói về sự thay đổi trong thế giới việc làm: công việc thay đổi (đoạn 1), kỹ năng thay đổi (đoạn 2), cách đào tạo thay đổi (đoạn 3). Đoạn đầu: "making career choices can be challenging, especially in this changing world." Đoạn cuối: "In the future, there may be even more changes in the world of work." → Từ khoá "changing world of work" xuyên suốt bài.

B và C sai vì bài không chỉ nói cách chọn nghề hay đưa ra quyết định — bài tập trung vào sự thay đổi của thế giới việc làm nói chung nha.
</explanation>

<!-- chunk: exercise-3 -->
<teacher_script pause="60" href="audio/tts_5efa5519.mp3">
Bài 3 trang 134 — đọc lại bài và đánh dấu T (Đúng) hoặc F (Sai) cho 5 câu. Làm 1 phút nha.
</teacher_script>

<task>
**Đề:** Read the article again and tick (✔) T (True) or F (False).
**Dịch đề:** Đọc lại bài viết và đánh dấu vào (✔) T (Đúng) hoặc F (Sai).
</task>

<questions type="true_false">
**1.** Teenagers should consider their career paths as soon as they reach 18.
*Thanh thiếu niên nên cân nhắc con đường sự nghiệp của mình ngay khi đủ 18 tuổi.*

**2.** A nail artist is an example of a job created by technology.
*Thợ làm móng là một ví dụ về công việc được tạo ra bởi công nghệ.*

**3.** In many jobs, people need to be able to use computers well.
*Trong nhiều công việc, con người cần có khả năng sử dụng tốt máy tính.*

**4.** People can get job training in more ways than they could in the past.
*Mọi người có thể được đào tạo nghề theo nhiều cách hơn trước đây.*

**5.** There will be fewer changes in the world of work in the future.
*Sẽ có ít thay đổi hơn trong thế giới việc làm trong tương lai.*
</questions>

<teacher_script pause="0" href="audio/tts_9257c365.mp3">
Ok đáp án bài 3 nè.
</teacher_script>

<answer>
**Bài 3 - Đáp án:** 1.F | 2.F | 3.T | 4.T | 5.F
</answer>

<explanation>
**1 — F:** Sai. Bài nói "When you turn **15**" — 15 tuổi, không phải 18 tuổi nha.

**2 — F:** Sai. Nail artist (thợ làm móng) là ví dụ về **beauty job** — công việc làm đẹp đang trở nên phổ biến, KHÔNG phải công việc được tạo ra bởi công nghệ. Công việc do công nghệ tạo ra là software engineer và online teacher nha.

**3 — T:** Đúng. "Computer skills have become a must for many jobs." — kỹ năng máy tính là điều bắt buộc cho nhiều công việc.

**4 — T:** Đúng. "Training can be both face-to-face and online." — có thêm hình thức đào tạo trực tuyến, nhiều cách hơn trước đây.

**5 — F:** Sai. Bài nói ngược lại: "In the future, there may be **even more** changes in the world of work." — sẽ còn nhiều thay đổi HƠN nữa, không phải ít hơn.
</explanation>

<!-- chunk: speaking-4 -->
<teacher_script pause="60" href="audio/tts_96e5a98c.mp3">
Bài 4 trang 134 — hoàn thành hội thoại giữa Ben và Hoa. Có 4 câu trong hộp, điền vào chỗ trống (1)-(4) cho đúng. Làm 1 phút nha.
</teacher_script>

<task>
**Đề:** Complete the conversation with the sentences from the box. Then practise it with a partner.
**Dịch đề:** Hoàn thành đoạn hội thoại với các câu trong khung. Sau đó thực hành nó với một người bạn.
</task>

<questions type="fill_blanks">
**Câu trong hộp:**
- A. You're creative and fashionable. You're good at persuading other people, too.
- B. What skills do you need for the job?
- C. What job do you think you'll do in the future?
- D. Why do you like this job?

**Hội thoại:**

**Ben:** (1) ________

**Hoa:** I want to be a hairdresser.

**Ben:** Interesting! (2) ________

**Hoa:** Well, I'm keen on creating new hairstyles. I can also earn a good living because it's a well-paid job.

**Ben:** (3) ________

**Hoa:** Though there are many skills, I think hair cutting and colouring are the most important.

**Ben:** You'll do that job well. (4) ________

**Hoa:** Thanks. I hope I'll be such a successful hairdresser that I'll have my own hair salon some day.
</questions>

<teacher_script pause="0" href="audio/tts_04f7c05e.mp3">
Đáp án bài 4 nè.
</teacher_script>

<answer>
**Bài 4 - Đáp án:** 1-C | 2-D | 3-B | 4-A
</answer>

<explanation>
**1-C:** Ben hỏi Hoa về nghề tương lai → dùng câu C: "What job do you think you'll do in the future?"

**2-D:** Hoa nói muốn làm thợ cắt tóc, Ben hỏi lý do thích → dùng câu D: "Why do you like this job?"

**3-B:** Ben hỏi về kỹ năng cần có → dùng câu B: "What skills do you need for the job?"

**4-A:** Ben khen Hoa có tính cách phù hợp → dùng câu A: "You're creative and fashionable. You're good at persuading other people, too." — sáng tạo và giỏi thuyết phục rất phù hợp với nghề cắt tóc nha.
</explanation>

<!-- chunk: speaking-5 -->
<teacher_script pause="0" href="audio/tts_9d57ab8f.mp3">
Bài 5 trang 135 — làm việc theo cặp. Thực hành hội thoại tương tự về nghề bạn muốn làm trong tương lai, rồi báo cáo với lớp. Đây là khung báo cáo nha.
</teacher_script>

<task>
**Đề:** Work in pairs. Make similar conversations about the job you want to do in the future. Then report your conversation to the class.
**Dịch đề:** Làm việc theo cặp. Thực hiện những cuộc trò chuyện tương tự về công việc bạn muốn làm trong tương lai. Sau đó báo cáo cuộc trò chuyện của bạn với lớp.
</task>

<questions type="speaking">
**Khung báo cáo:**

My partner is… She/He wants to be a … because … She/He'll need to learn many skills, such as … She/He'll do the job well since ... She/He hopes that one day, she/he'll ...

*(Bạn của tôi là… Cô/Anh ấy muốn trở thành một … bởi vì … Cô/Anh ấy sẽ cần học nhiều kỹ năng, chẳng hạn như … Cô/Anh ấy sẽ làm tốt công việc vì… Cô/Anh ấy hy vọng rằng một ngày nào đó, cô/anh ấy sẽ ...)*

**Gợi ý câu hỏi cho hội thoại:**
1. What job do you think you'll do in the future?
2. Why do you like this job?
3. What skills do you need for the job?
4. What personal qualities do you have for this job?
</questions>

<teacher_script pause="0" href="audio/tts_881e83cd.mp3">
Tốt lắm nha! Vậy là xong bài <eng>Skills 1</eng> Unit 12. Hôm nay đọc bài về thế giới việc làm đang thay đổi, làm bài tập đúng sai, và luyện nói về nghề nghiệp tương lai. Nhớ: thế giới việc làm đang thay đổi — cần kỹ năng máy tính, teamwork, và học suốt đời nha. Ôn bài hen!
</teacher_script>
