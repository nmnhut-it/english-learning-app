# UNIT 3: HEALTHY LIVING FOR TEENS
## COMMUNICATION - Giao tiếp hàng ngày

<!-- chunk: vocabulary -->
<teacher_script pause="0">
Ok lớp 9, bài <eng>Communication — Unit 3</eng> nha. Hôm nay học cách hỏi lại khi không hiểu — <eng>Asking for repetition and checking understanding</eng> — và từ vựng liên quan đến sức khỏe và học tập. Bấm "Bắt đầu" học từ vựng đi nha.
</teacher_script>

<vocabulary>
1. **praise** : (v/n) khen ngợi /preɪz/
2. **encourage** : (v) khuyến khích /ɪnˈkʌrɪdʒ/
3. **happiness** : (n) hạnh phúc /ˈhæpɪnəs/
4. **encouragement** : (n) sự khuyến khích /ɪnˈkʌrɪdʒmənt/
5. **thankful** : (adj) biết ơn /ˈθæŋkfl/
6. **assignment** : (n) bài tập được giao /əˈsaɪnmənt/
7. **reduce** : (v) giảm, giảm bớt /rɪˈdjuːs/
8. **expect** : (v) mong đợi, trông chờ /ɪkˈspekt/
9. **overcome** : (v) vượt qua /ˌəʊvəˈkʌm/
10. **difficulty** : (n) khó khăn /ˈdɪfɪkəlti/
11. **situation** : (n) tình huống /ˌsɪtʃuˈeɪʃn/
12. **learnt** : (v - past form) đã học được /lɜːnt/
</vocabulary>

<teacher_script pause="0">
Ghi xong chưa nha! "Overcome" — vượt qua — hay dùng lắm: *overcome difficulty* = vượt qua khó khăn, *overcome stress* = vượt qua căng thẳng. Giờ học phần ngôn ngữ nha.
</teacher_script>

<!-- chunk: language-asking-repetition -->
<teacher_script pause="0">
Khi nghe ai nói mà không hiểu hoặc không nghe rõ, người Anh sẽ dùng những cụm này để hỏi lại. Ghi vô tập đi nha.
</teacher_script>

<grammar>
## Ngôn ngữ: Asking for Repetition & Checking Understanding

### Khi không nghe rõ / không hiểu:
| Cụm từ | Nghĩa | Mức độ trang trọng |
|---|---|---|
| **Sorry?** | Xin lỗi bạn nói gì? | Thường ngày |
| **I beg your pardon?** | Xin lỗi, bạn nói gì ạ? | Trang trọng hơn |
| **Could you say that again, please?** | Bạn có thể nói lại không? | Lịch sự |
| **Would you mind repeating that?** | Bạn có phiền nhắc lại không? | Rất lịch sự |

### Khi muốn xác nhận mình hiểu đúng:
| Cụm từ | Nghĩa |
|---|---|
| **Do you mean...?** | Ý bạn là...? |
| **So you're saying that...?** | Vậy ý bạn là...? |

### Khi đã hiểu rồi:
| Cụm từ | Nghĩa |
|---|---|
| **Got it!** | Hiểu rồi! |
| **I see.** | Tôi hiểu rồi. |
| **That makes sense.** | Có lý đó! |
</grammar>

<teacher_script pause="0">
Mẹo nhỏ: trong lớp học, dùng "Could you say that again, please?" là an toàn và lịch sự nhất nha. "Sorry?" thì hơi ngắn, còn "I beg your pardon?" thì nghe formal lắm, dùng khi gặp người lớn hay trong môi trường nghiêm túc. Bây giờ vô hội thoại nha!
</teacher_script>

<!-- chunk: dialogue -->
<teacher_script pause="0">
Hội thoại giữa An và Phong — hai bạn đang nói chuyện về stress học tập. Chú ý các câu hỏi lại nha.
</teacher_script>

<dialogue>
| English | Vietnamese |
|---------|------------|
| **An:** I've been really stressed this week. I don't know what to do. | **An:** Tuần này tao căng thẳng lắm. Tao không biết phải làm gì nữa. |
| **Phong:** Sorry? Did you say stressed? | **Phong:** Sao? Mày nói căng thẳng hả? |
| **An:** Yes. I have three assignments due on Friday and I can't manage. | **An:** Ừ. Tao có ba bài tập nộp thứ Sáu mà xoay không nổi. |
| **Phong:** I see. Would you mind telling me more? | **Phong:** Tao hiểu rồi. Mày có phiền kể thêm cho tao nghe không? |
| **An:** I just feel like there's too much to do at once. I can't concentrate on anything. | **An:** Tao cứ cảm giác có quá nhiều thứ cần làm cùng lúc. Tao không tập trung được vào cái gì hết. |
| **Phong:** Have you tried the Pomodoro technique? | **Phong:** Mày đã thử kỹ thuật Pomodoro chưa? |
| **An:** Could you repeat that? Po-mo-do-ro? | **An:** Mày có thể nhắc lại không? Po-mo-do-ro? |
| **Phong:** Yes! You study for 25 minutes, then take a 5-minute break. It reduces distractions and helps you focus. | **Phong:** Ừ! Mày học 25 phút rồi nghỉ 5 phút. Cách này giảm sự xao lãng và giúp mày tập trung hơn. |
| **An:** Do you mean I should repeat this all day? | **An:** Ý mày là tao nên làm như vậy cả ngày hả? |
| **Phong:** Exactly! Until all your assignments are done. | **Phong:** Đúng rồi! Cho đến khi làm xong hết tất cả bài tập. |
| **An:** Got it! I'll try that tonight. Thank you, Phong! | **An:** Hiểu rồi! Tối nay tao sẽ thử. Cảm ơn mày nha, Phong! |
</dialogue>

<teacher_script pause="180">
Dịch hội thoại vô vở nha. Gạch chân các câu hỏi lại và xác nhận: "Sorry?", "Could you repeat that?", "Do you mean...?", "Got it!". Ba phút.
</teacher_script>

<!-- chunk: exercise-2-checking-understanding -->
<teacher_script pause="60">
<eng>Exercise 2</eng> — chọn câu phản hồi phù hợp cho từng tình huống nha. 60 giây.
</teacher_script>

<task>
**Đề:** Choose the most appropriate response for each situation.
**Dịch đề:** Chọn phản hồi phù hợp nhất cho mỗi tình huống.
</task>

<questions type="multiple_choice">
**a.** Your teacher explains a grammar point very quickly and you don't follow.
*(Thầy/cô giải thích ngữ pháp rất nhanh và em không theo kịp.)*
- A. "Sorry? I didn't catch that. Could you say that again, please?"
- B. "That makes sense!"
- C. "Got it!"

**b.** Your friend gives you directions to a new place and you're not sure you understood correctly.
*(Bạn chỉ đường đến chỗ mới và em không chắc mình hiểu đúng.)*
- A. "I beg your pardon?"
- B. "Do you mean I should turn left at the traffic lights?"
- C. "Would you mind repeating that?"

**c.** Your classmate explains a maths solution step by step and finishes. You understood everything.
*(Bạn cùng lớp giải thích bài toán từng bước và vừa xong. Em hiểu hết rồi.)*
- A. "Sorry?"
- B. "Could you say that again?"
- C. "Got it! That makes sense now, thank you!"
</questions>

<teacher_script pause="0">
Đáp án và giải thích nè!
</teacher_script>

<answer>
**Đáp án:** a.A | b.B | c.C
</answer>

<explanation>
**a. A — "Could you say that again, please?"** — Em không hiểu → phải hỏi lại. "Sorry, I didn't catch that" thêm lý do giải thích, lịch sự và rõ ràng. Không dùng "Got it!" khi chưa hiểu nha.

**b. B — "Do you mean I should turn left...?"** — Em nghe rồi nhưng muốn *xác nhận* lại hiểu đúng không → dùng "Do you mean...?" Không cần hỏi lại từ đầu vì em đã nghe rồi.

**c. C — "Got it! That makes sense!"** — Em hiểu hết rồi → thể hiện đã nắm bài. "Got it!" = hiểu rồi, "That makes sense!" = có lý đó. Không hỏi lại khi đã hiểu hết rồi nha.
</explanation>

<!-- chunk: exercise-3-fill-blanks -->
<teacher_script pause="60">
<eng>Exercise 3</eng> — điền từ đúng vào chỗ trống nha. Chọn: *happy, delay, happiness, overcome*. 60 giây.
</teacher_script>

<task>
**Đề:** Fill in the blanks with the correct words: *happy, delay, happiness, overcome*.
**Dịch đề:** Điền từ đúng vào chỗ trống.
</task>

<questions type="fill_blanks">
**1.** Staying ________ is one of the keys to good mental health.

**2.** Don't ________ solving your problems — face them now!

**3.** Music brings me ________ when I feel sad.

**4.** If you ________ one difficulty, you will feel stronger.
</questions>

<teacher_script pause="0">
Đáp án nè!
</teacher_script>

<answer>
**Đáp án:**
1. happy
2. delay
3. happiness
4. overcome
</answer>

<explanation>
**1. happy** — "vui vẻ, hạnh phúc" — tính từ (*adjective*). Sau *"Staying"* (linking verb) cần dùng tính từ: *Staying **happy***.

**2. delay** — "trì hoãn". Sau *"Don't"* dùng động từ nguyên thể: *Don't **delay** solving your problems*.

**3. happiness** — "hạnh phúc" — danh từ (*noun*). Câu cần danh từ làm tân ngữ: *Music brings me **happiness**.*

**4. overcome** — "vượt qua". Câu điều kiện loại 1: sau *"If you"* + hiện tại đơn: *If you **overcome** one difficulty...* Chú ý: "overcome" bất quy tắc: overcome – overcame – overcome.
</explanation>

<teacher_script pause="0">
Xong bài <eng>Communication Unit 3</eng> rồi nha! Hôm nay học cách hỏi lại khi không hiểu, xác nhận hiểu đúng, và từ vựng về tình cảm, sức khỏe tinh thần. Kỹ năng hỏi lại này quan trọng lắm trong giao tiếp thực tế — đừng ngại hỏi lại khi không hiểu nha, người ta sẽ vui mà giải thích thêm cho. Giỏi lắm!
</teacher_script>
