# UNIT 7: NATURAL WONDERS OF THE WORLD

## GETTING STARTED - The Natural Wonders Contest

<!-- chunk: intro -->
<teacher_script pause="0" href="audio/tts_fd00461e.mp3">
Chào các em. Chào mừng các em đến với bài học hôm nay. Mời các em mở sách giáo khoa.
</teacher_script>

<teacher_script pause="0" href="audio/tts_c0841176.mp3">
Chúng ta cùng học từ vựng mới. Các em hãy nghe và lặp lại theo mẫu nhé.
</teacher_script>

---

<!-- chunk: vocabulary -->
<vocabulary>
1. **wonder** : (n) kỳ quan /ˈwʌndə/
2. **natural wonder** : (n) kỳ quan thiên nhiên /ˈnætʃrəl ˈwʌndə/
3. **corridor** : (n) hành lang /ˈkɒrɪdɔː/
4. **contest** : (n) cuộc thi /ˈkɒntest/
5. **congratulations** : (n) lời chúc mừng /kənˌɡrætʃuˈleɪʃnz/
6. **explore** : (v) khám phá /ɪkˈsplɔː/
7. **be crazy about** : cực kỳ mê /bi ˈkreɪzi əˈbaʊt/
8. **Mount Everest** : Đỉnh Everest /maʊnt ˈevərɪst/
9. **the Great Barrier Reef** : Rạn san hô Great Barrier /ðə ɡreɪt ˈbæriə riːf/
10. **the Dead Sea** : Biển Chết /ðə ded siː/
11. **landscape** : (n) phong cảnh /ˈlændskeɪp/
12. **snow-covered** : (adj) phủ tuyết /snəʊ ˈkʌvəd/
13. **peak** : (n) đỉnh (núi) /piːk/
14. **charming** : (adj) quyến rũ, duyên dáng /ˈtʃɑːmɪŋ/
15. **admire** : (v) ngưỡng mộ /ədˈmaɪə/
16. **can't help + Ving** : không thể không (làm gì) /kɑːnt help/
17. **mention** : (v) đề cập, nhắc đến /ˈmenʃn/
18. **risk** : (n) rủi ro /rɪsk/
19. **destroy** : (v) phá hủy /dɪˈstrɔɪ/
20. **preserve** : (v) bảo tồn /prɪˈzɜːv/
21. **sustainable development** : phát triển bền vững /səˈsteɪnəbl dɪˈveləpmənt/
22. **host** : (n) người dẫn chương trình /həʊst/
23. **the Grand Canyon** : Hẻm núi Grand /ðə ɡrænd ˈkænjən/
24. **Ha Long Bay** : Vịnh Hạ Long /hɑː lɒŋ beɪ/
25. **support** : (v) ủng hộ /səˈpɔːt/
26. **complex** : (n) quần thể /ˈkɒmpleks/
27. **environmentalist** : (n) nhà môi trường học /ɪnˌvaɪrənˈmentəlɪst/
28. **measure** : (n) biện pháp /ˈmeʒə/
29. **goal** : (n) mục tiêu /ɡəʊl/
30. **location** : (n) vị trí /ləʊˈkeɪʃn/
31. **unique** : (adj) độc đáo, duy nhất /juːˈniːk/
32. **discover** : (v) khám phá, phát hiện /dɪˈskʌvə/
33. **contribute to** : đóng góp cho /kənˈtrɪbjuːt tuː/
</vocabulary>


<teacher_script pause="0" action="record" href="audio/tts_508f13d8.mp3">
Các em hãy ghi âm phần luyện tập của mình và gửi cho thầy nhé.
</teacher_script>

---

### Bài 1 trang 72 - Listen and read

<teacher_script pause="0" href="audio/tts_c08c1d7b.mp3">
Bây giờ các em sẽ nghe một đoạn băng. Hãy chú ý lắng nghe để tìm thông tin trả lời câu hỏi.
</teacher_script>

<audio src="https://img.loigiaihay.com/picture/2024/0515/41.mp3">
**Audio:** Listen and read
</audio>

<task>
**Đề:** Listen and read.
**Dịch đề:** Nghe và đọc.
**Yêu cầu:** Nghe audio và đọc theo đoạn hội thoại giữa Lan và Tom.
**Hướng dẫn:**
- Bước 1: Nghe audio 1 lần để hiểu nội dung chung
- Bước 2: Đọc theo từng câu, chú ý phát âm
- Bước 3: Đọc lại và gạch chân từ mới
</task>

<dialogue>
**Lan:** Hi, Tom. I heard that you won second prize in the Natural Wonders Contest. Congratulations!
**Tom:** Thank you, Lan. I like exploring the wonders of nature. I'm crazy about them.
**Lan:** What did you talk about in the contest?
**Tom:** I talked about some natural wonders like Mount Everest, the Great Barrier Reef, and the Dead Sea.
**Lan:** Great! I'm interested in them, too.
**Tom:** You know, our planet has so many beautiful landscapes, from snow-covered peaks to charming rivers, lakes, and other natural places that we can't help admiring. So that's what I talked about.
**Lan:** Oh, yes... And what else did you mention?
**Tom:** Well, I mentioned the risks of us, humans, destroying natural wonders, and I suggested ways to preserve them and support sustainable development.
**Lan:** Sounds interesting. Do you want to visit any of these places?
**Tom:** The host also asked me if I wanted to visit those places, and I answered that I wanted to see two of them: the Grand Canyon and Ha Long Bay.
**Lan:** I want to visit them, too.
</dialogue>

<translation>
**Lan:** Chào Tom. Tôi nghe nói bạn đã giành giải nhì trong Cuộc thi Kỳ quan Thiên nhiên. Chúc mừng!
**Tom:** Cảm ơn, Lan. Tôi thích khám phá những kỳ quan của thiên nhiên. Tôi mê chúng lắm.
**Lan:** Bạn đã nói về gì trong cuộc thi?
**Tom:** Tôi đã nói về một số kỳ quan thiên nhiên như Núi Everest, Rạn san hô Great Barrier, và Biển Chết.
**Lan:** Tuyệt vời! Tôi cũng quan tâm đến chúng.
**Tom:** Bạn biết đấy, hành tinh của chúng ta có rất nhiều phong cảnh đẹp, từ những đỉnh núi phủ tuyết đến những dòng sông quyến rũ, hồ, và những nơi tự nhiên khác mà chúng ta không thể không ngưỡng mộ. Đó là những gì tôi đã nói về.
**Lan:** Ồ vâng... Và bạn đã đề cập gì khác nữa?
**Tom:** Ừm, tôi đã nhắc đến rủi ro của chúng ta, con người, phá hủy những kỳ quan thiên nhiên, và tôi đã đề xuất cách để bảo tồn chúng và hỗ trợ phát triển bền vững.
**Lan:** Nghe thú vị đấy. Bạn có muốn đến thăm bất kỳ nơi nào trong số những địa điểm này không?
**Tom:** Người dẫn chương trình cũng hỏi tôi nếu tôi muốn đến thăm những nơi đó, và tôi trả lời rằng tôi muốn nhìn thấy hai trong số chúng: Grand Canyon và Vịnh Hạ Long.
**Lan:** Tôi cũng muốn đến thăm chúng.
</translation>

<teacher_script pause="240" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<teacher_script pause="60" href="audio/tts_75ebc937.mp3">
Các em hãy sửa những lỗi sai vào bài làm của mình nếu có nhé.
</teacher_script>

<teacher_script pause="0" action="record" href="audio/tts_508f13d8.mp3">
Các em hãy ghi âm phần luyện tập của mình và gửi cho thầy nhé.
</teacher_script>

---

### Bài 2 trang 73 - True or False

<teacher_script pause="60" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<task>
**Đề:** Decide if the following statements are true (T) or false (F).
**Dịch đề:** Xác định các câu sau đúng (T) hay sai (F).
**Yêu cầu:** Đọc lại hội thoại, xác định thông tin đúng hay sai.
**Hướng dẫn:**
- Bước 1: Đọc câu phát biểu
- Bước 2: Tìm thông tin trong hội thoại
- Bước 3: Đánh dấu T (True) hoặc F (False)
</task>

<questions type="true_false">
**1.** Tom took part in the Natural Wonders Contest. _____

*Tom tham gia Cuộc thi Kỳ quan Thiên nhiên.*

**2.** Lan is not keen on natural wonders. _____

*Lan không quan tâm đến kỳ quan thiên nhiên.*

**3.** Tom suggested ways to preserve natural wonders. _____

*Tom gợi ý những cách bảo tồn kỳ quan thiên nhiên.*

**4.** Tom asked the host if he wanted to go to Ha Long Bay. _____

*Tom hỏi người dẫn chương trình xem anh ấy có muốn đi Vịnh Hạ Long không.*
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_d5fd6a66.mp3">
Đã hết thời gian. Sau đây là đáp án gợi ý. Các em hãy so sánh với bài làm của mình nhé.
</teacher_script>

<answer>
**Bài 2 - Đáp án:** 1.T | 2.F | 3.T | 4.F
</answer>

<teacher_script pause="0" href="audio/tts_005e7f0d.webm">
Câu 1 là <eng>True</eng>. Tom <eng>won second prize</eng>, nghĩa là Tom tham gia cuộc thi.
</teacher_script>

<teacher_script pause="0" href="audio/tts_f0a9f384.webm">
Câu 2 là <eng>False</eng>. Lan nói <eng>I'm interested in them, too</eng>, nghĩa là Lan cũng quan tâm. Chữ <eng>not</eng> trong đề làm câu sai.
</teacher_script>

<teacher_script pause="0" href="audio/tts_0d40e40c.webm">
Câu 3 là <eng>True</eng>. Tom nói <eng>I suggested ways to preserve them</eng>. Đúng y chang trong bài.
</teacher_script>

<teacher_script pause="0" href="audio/tts_81fe3e9f.webm">
Câu 4 là <eng>False</eng>. Cẩn thận câu này. <eng>The host asked me if I wanted to visit those places</eng> - người dẫn chương trình hỏi Tom, không phải Tom hỏi. Đề đổi ngược chủ ngữ, nhiều bạn hay sai chỗ này.
</teacher_script>

<explanation>
**Giải thích:**
1. T - Tom won second prize in the Natural Wonders Contest, nghĩa là Tom tham gia cuộc thi.
2. F - Lan nói "I'm interested in them, too", nghĩa là Lan cũng quan tâm.
3. T - Tom nói "I suggested ways to preserve them".
4. F - Ngược lại, "The host asked me if I wanted to visit those places" - người dẫn chương trình hỏi Tom, không phải Tom hỏi.
</explanation>

---

### Bài 3 trang 73 - Fill in the blanks

<teacher_script pause="60" href="audio/tts_d5b1d729.webm">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<task>
**Đề:** Fill in the blanks with the words from the conversation.
**Dịch đề:** Điền từ vào chỗ trống bằng từ trong hội thoại.
**Yêu cầu:** Đọc câu, tìm từ phù hợp trong hội thoại để điền.
**Hướng dẫn:**
- Bước 1: Đọc câu và hiểu nghĩa
- Bước 2: Nghĩ từ phù hợp
- Bước 3: Kiểm tra lại trong hội thoại
</task>

<questions type="fill_blanks">
**1.** A _______ is everything you can see in a large area of land, especially in the countryside.
**2.** It was risky for him to climb the peak, but we _______ admiring his courage.
**3.** The best way to _______ the Trang An Landscape Complex is by boat.
**4.** Environmentalists strongly _______ measures to save these heritage trees.
**5.** Sustainable _______ goals will guide us to create a more sustainable future for all.
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_d5fd6a66.mp3">
Đã hết thời gian. Sau đây là đáp án gợi ý. Các em hãy so sánh với bài làm của mình nhé.
</teacher_script>

<answer>
**Bài 3 - Đáp án:** 1. landscape | 2. couldn't help | 3. explore | 4. support | 5. development
</answer>

<teacher_script pause="0" href="audio/tts_37684104.webm">
Câu 1 là <eng>landscape</eng> - phong cảnh. <eng>Everything you see when you look across a large area</eng> - đó là định nghĩa của <eng>landscape</eng>.
</teacher_script>

<teacher_script pause="0" href="audio/tts_a8cd7ba0.webm">
Câu 2 là <eng>couldn't help</eng>. Cấu trúc <eng>can't help + Ving</eng> = không thể không làm gì. Trong bài là <eng>can't help admiring</eng>. Cấu trúc này quan trọng nha, nhớ kỹ.
</teacher_script>

<teacher_script pause="0" href="audio/tts_db6d275f.webm">
Câu 3 là <eng>explore</eng> - khám phá. <eng>The best way to explore</eng> = cách tốt nhất để khám phá.
</teacher_script>

<teacher_script pause="0" href="audio/tts_5e363ca8.webm">
Câu 4 là <eng>support</eng> - ủng hộ. <eng>Environmentalists support measures</eng> = nhà môi trường ủng hộ biện pháp.
</teacher_script>

<teacher_script pause="0" href="audio/tts_305d4f73.webm">
Câu 5 là <eng>development</eng>. <eng>Sustainable development</eng> = phát triển bền vững. Cụm từ này rất quan trọng, hay ra thi luôn.
</teacher_script>

<explanation>
**Giải thích:**
1. landscape - phong cảnh (everything you see when you look across a large area)
2. couldn't help - không thể không (can't help + Ving = không thể không làm gì)
3. explore - khám phá (I like exploring the wonders of nature)
4. support - ủng hộ (support sustainable development)
5. development - phát triển (sustainable development = phát triển bền vững)
</explanation>

---

### Bài 4 trang 73 - Complete the sentences

<teacher_script pause="60" href="audio/tts_d5b1d729.mp3">
Các em hãy đọc kỹ yêu cầu và hoàn thành bài tập vào vở hoặc sách giáo khoa.
</teacher_script>

<task>
**Đề:** Complete the sentences with the correct form of the words given.
**Dịch đề:** Hoàn thành câu với dạng đúng của từ cho sẵn.
**Yêu cầu:** Chia động từ hoặc biến đổi từ cho phù hợp với câu.
**Hướng dẫn:**
- Bước 1: Đọc câu và xác định thì
- Bước 2: Chia từ cho đúng dạng
- Bước 3: Kiểm tra ngữ pháp
</task>

<questions type="fill_blanks">
**Word bank:** *crazy / landscape / explore / suggest / contributes*

**1.** Charles is _______ about football. He follows all the World Cup matches.
**2.** When you go to Mui Ne, you can see the unique _______ of a desert.
**3.** The children were very eager to _______ the surrounding countryside.
**4.** Can you _______ ways of boosting our local tourism?
**5.** Tourism _______ millions of dollars to our country's economy.
</questions>

<teacher_script pause="0" type="answer" href="audio/tts_d5fd6a66.mp3">
Đã hết thời gian. Sau đây là đáp án gợi ý. Các em hãy so sánh với bài làm của mình nhé.
</teacher_script>

<answer>
**Bài 4 - Đáp án:** 1. crazy | 2. landscape | 3. explore | 4. suggest | 5. contributes
</answer>

<teacher_script pause="0" href="audio/tts_b11bfd2d.webm">
Câu 1: <eng>crazy</eng>. <eng>Be crazy about</eng> = mê, cuồng. Không đổi dạng vì đây là tính từ.
</teacher_script>

<teacher_script pause="0" href="audio/tts_e7010cf0.webm">
Câu 2: <eng>landscape</eng> - phong cảnh. Danh từ, không đổi dạng.
</teacher_script>

<teacher_script pause="0" href="audio/tts_099a70a3.webm">
Câu 3: <eng>explore</eng>. Cấu trúc <eng>eager to + V-inf</eng>, nên giữ nguyên <eng>explore</eng>.
</teacher_script>

<teacher_script pause="0" href="audio/tts_8889c144.webm">
Câu 4: <eng>suggest</eng>. <eng>Can you suggest...?</eng> Động từ nguyên mẫu sau <eng>can</eng>, không chia.
</teacher_script>

<teacher_script pause="0" href="audio/tts_0946c3f3.webm">
Câu 5: <eng>contributes</eng>. <eng>Tourism</eng> là danh từ số ít, nên động từ phải thêm <eng>s</eng>. Thì hiện tại đơn, diễn tả sự thật. Cẩn thận câu này, nhiều bạn quên thêm <eng>s</eng>.
</teacher_script>

<explanation>
**Giải thích:**
1. crazy - be crazy about = mê, cuồng (không đổi dạng)
2. landscape - phong cảnh (danh từ, không đổi)
3. explore - eager to + V-inf (khám phá)
4. suggest - Can you suggest...? (đề xuất, động từ nguyên mẫu sau can)
5. contributes - Tourism (số ít) + V-s/es (thì hiện tại đơn)
</explanation>

---

<teacher_script pause="0" href="audio/tts_ba688992.mp3">
Bài học hôm nay kết thúc tại đây. Các em nhớ ôn bài và chuẩn bị cho bài học tiếp theo. Tạm biệt các em.
</teacher_script>
