# Unit 8: New Ways To Learn - Writing

## Writing about the benefits of blended learning
*(Viết về lợi ích của việc học kết hợp)*

### New words

1. blended learning: (n.phr) học kết hợp /ˈblendɪd ˈlɜːnɪŋ/
2. control: (n, v) kiểm soát /kənˈtrəʊl/
3. access: (n, v) truy cập /ˈækses/
4. lesson materials: (n.phr) tài liệu bài học /ˈlesn məˈtɪəriəlz/
5. communication skills: (n.phr) kỹ năng giao tiếp /kəˌmjuːnɪˈkeɪʃn skɪlz/
6. teamwork skills: (n.phr) kỹ năng làm việc nhóm /ˈtiːmwɜːk skɪlz/
7. engaged: (adj) tham gia, gắn kết /ɪnˈɡeɪdʒd/
8. benefit: (n) lợi ích /ˈbenɪfɪt/
9. suitable: (adj) phù hợp /ˈsuːtəbl/
10. provide: (v) cung cấp /prəˈvaɪd/

---

### Bài 1 (trang 92)

**1. Work in pairs. Discuss and tick the benefits of blended learning**

*(Làm việc theo cặp. Thảo luận và đánh dấu những lợi ích của việc học kết hợp)*

| Benefits |
|----------|
| ☐ 1. It helps students have more control over their own learning. |
| ☐ 2. It provides easier access to lesson materials. |
| ☐ 3. It may not prepare students well for exams. |
| ☐ 4. Students can develop better communication and teamwork skills. |
| ☐ 5. It is not suitable for students who don't have Internet connection or computers. |

#### Lời giải chi tiết:

**Đáp án:** 1, 2, 4 ✓

#### Giải thích:

- Câu 3 và 5 là **nhược điểm** (disadvantages), không phải lợi ích.

#### Hướng dẫn dịch:

1. Nó giúp học sinh kiểm soát tốt hơn việc học của mình. ✓
2. Tiếp cận tài liệu bài học dễ dàng hơn. ✓
3. Nó có thể không chuẩn bị tốt cho học sinh cho các bài kiểm tra. ✗
4. Học sinh có thể phát triển các kỹ năng giao tiếp và làm việc nhóm tốt hơn. ✓
5. Nó không phù hợp cho học sinh không có kết nối Internet hoặc máy tính. ✗

---

### Bài 2 (trang 92)

**2. Work in groups. Use the benefits in 1 and your own ideas to complete the following outline**

*(Làm việc nhóm. Sử dụng những lợi ích trong 1 và ý tưởng của riêng bạn để hoàn thành dàn ý sau)*

#### Lời giải chi tiết:

| Part | Content |
|------|---------|
| **Topic sentence** | Blended learning is a way of studying a subject, which combines learning over the Internet and face-to-face learning, and brings several benefits to us. |
| **First benefit** | First, blended learning helps us have more control over our own learning. |
| **Explanation 1** | We are able to get access to many online materials and video lectures at any time. This also allows us to watch and review lessons that we have not understood at home. |
| **Second benefit** | Second, we can develop better communication and teamwork skills. |
| **Explanation 2** | We can use different ways to communicate in class. Since a lot of the work is assigned for homework, we have more time for pair work and group work in the classroom. |
| **Concluding sentence** | I think we should have more blended learning classes at school because they keep us engaged and helps us learn useful skills. |

#### Hướng dẫn dịch:

| Phần | Nội dung |
|------|----------|
| **Câu chủ đề** | Học kết hợp là một cách học một môn học, kết hợp học qua Internet và học trực tiếp, và mang lại nhiều lợi ích cho chúng ta. |
| **Lợi ích thứ nhất** | Đầu tiên, học tập kết hợp giúp chúng ta kiểm soát nhiều hơn việc học của chính mình. |
| **Giải thích 1** | Chúng ta có thể truy cập vào nhiều tài liệu trực tuyến và video bài giảng bất cứ lúc nào. Điều này cũng cho phép chúng ta xem và xem lại các bài học mà chúng ta chưa hiểu ở nhà. |
| **Lợi ích thứ hai** | Thứ hai, chúng ta có thể phát triển kỹ năng giao tiếp và làm việc nhóm tốt hơn. |
| **Giải thích 2** | Chúng ta có thể sử dụng nhiều cách khác nhau để giao tiếp trong lớp. Vì có rất nhiều bài tập được giao cho bài tập về nhà, chúng ta có nhiều thời gian hơn để làm việc theo cặp và làm việc nhóm trong lớp học. |
| **Câu kết luận** | Tôi nghĩ chúng ta nên có nhiều lớp học kết hợp hơn ở trường vì chúng giúp chúng ta tham gia và giúp chúng ta học các kỹ năng hữu ích. |

---

### Bài 3 (trang 92)

**3. Use the outline in 2 and expressions below to write a paragraph (120 - 150 words) about the benefits of blended learning**

*(Sử dụng dàn ý trong 2 và cách diễn đạt dưới đây để viết một đoạn văn (120 - 150 từ) về lợi ích của việc học kết hợp)*

#### Useful Expressions:

| Function | Expressions |
|----------|-------------|
| **Introducing the topic** | ... is a way of studying... |
| **Listing benefits** | Firstly / First / First of all, ... Secondly / Second, ... |
| **Giving explanations** | This means... / This allows... |
| **Concluding** | In conclusion, ... / To sum up, ... |

#### Lời giải mẫu:

**Bài viết:**

Blended learning is a way of studying a subject, which combines learning over the Internet and face-to-face learning, and brings several benefits to us. **Firstly**, blended learning helps us have more control over our own learning. We are able to get access to many online materials and video lectures at any time. This also allows us to watch and review lessons that we have not understood at home. If we are absent from school, we will not miss any lessons. **Secondly**, we can develop better communication and teamwork skills in a blended learning class. Since a lot of the work is assigned for homework, we have more time for pair work and group work in the classroom. Unlike traditional learning, blended learning provides us with more ways of communication including text messages, emails or social media. **In conclusion**, I think we should have more blended learning classes at school because they keep us engaged and helps us learn useful skills.

*(147 words)*

#### Hướng dẫn dịch:

Học kết hợp là một cách học một môn học, kết hợp học qua Internet và học trực tiếp, và mang lại nhiều lợi ích cho chúng ta. **Thứ nhất**, học tập kết hợp giúp chúng ta kiểm soát nhiều hơn việc học của chính mình. Chúng ta có thể truy cập vào nhiều tài liệu trực tuyến và video bài giảng bất cứ lúc nào. Điều này cũng cho phép chúng ta xem và xem lại các bài học mà chúng ta chưa hiểu ở nhà. Nếu nghỉ học, chúng ta sẽ không bỏ sót buổi học nào. **Thứ hai**, chúng ta có thể phát triển các kỹ năng giao tiếp và làm việc nhóm tốt hơn trong một lớp học kết hợp. Vì có rất nhiều bài tập được giao cho bài tập về nhà, chúng ta có nhiều thời gian hơn để làm việc theo cặp và làm việc nhóm trong lớp học. Không giống như học tập truyền thống, học tập kết hợp cung cấp cho chúng ta nhiều cách giao tiếp hơn bao gồm tin nhắn văn bản, email hoặc phương tiện truyền thông xã hội. **Tóm lại**, tôi nghĩ chúng ta nên có nhiều lớp học kết hợp hơn ở trường vì chúng giúp chúng ta tham gia và giúp chúng ta học các kỹ năng hữu ích.

---

## Useful Writing Expressions

### Introducing the topic (Giới thiệu chủ đề)

| English | Vietnamese |
|---------|------------|
| ... is a way of studying... | ... là một cách học... |
| ... combines... and... | ... kết hợp... và... |
| ... brings several benefits to... | ... mang lại nhiều lợi ích cho... |

### Listing benefits (Liệt kê lợi ích)

| English | Vietnamese |
|---------|------------|
| Firstly / First / First of all | Thứ nhất / Đầu tiên / Trước hết |
| Secondly / Second | Thứ hai |
| Thirdly / Third | Thứ ba |
| Finally / Lastly | Cuối cùng |
| In addition / Moreover / Furthermore | Ngoài ra / Hơn nữa |

### Giving explanations (Đưa ra giải thích)

| English | Vietnamese |
|---------|------------|
| This means... | Điều này có nghĩa là... |
| This allows... | Điều này cho phép... |
| This helps... | Điều này giúp... |
| For example / For instance | Ví dụ |

### Concluding (Kết luận)

| English | Vietnamese |
|---------|------------|
| In conclusion | Tóm lại |
| To sum up | Tóm tắt lại |
| In short | Nói ngắn gọn |
| All in all | Nhìn chung |

---

## Vocabulary (Từ vựng)

1. blended learning: (n.phr) học kết hợp /ˈblendɪd ˈlɜːnɪŋ/
2. control: (n, v) kiểm soát /kənˈtrəʊl/
3. access: (n, v) truy cập /ˈækses/
4. lesson materials: (n.phr) tài liệu bài học /ˈlesn məˈtɪəriəlz/
5. video lecture: (n.phr) bài giảng video /ˈvɪdiəʊ ˈlektʃə/
6. communication skills: (n.phr) kỹ năng giao tiếp /kəˌmjuːnɪˈkeɪʃn skɪlz/
7. teamwork skills: (n.phr) kỹ năng làm việc nhóm /ˈtiːmwɜːk skɪlz/
8. pair work: (n.phr) làm việc theo cặp /peə wɜːk/
9. group work: (n.phr) làm việc nhóm /ɡruːp wɜːk/
10. engaged: (adj) tham gia, gắn kết /ɪnˈɡeɪdʒd/
11. social media: (n.phr) mạng xã hội /ˈsəʊʃl ˈmiːdiə/
12. benefit: (n) lợi ích /ˈbenɪfɪt/
13. unlike: (prep) không giống như /ˌʌnˈlaɪk/
14. text message: (n.phr) tin nhắn văn bản /tekst ˈmesɪdʒ/
