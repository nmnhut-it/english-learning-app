# UNIT 5: NATURAL WONDERS OF VIET NAM

## GETTING STARTED   
**Vocabulary** 

1. **Geography Club** : câu lạc bộ địa lý /dʒiˈɒɡrəfi klʌb/
2. **but** : (conj) nhưng /bʌt/
3. **remember** : (v) nhớ, ghi nhớ /rɪˈmembə/
4. **must** : (v) phải, cần phải /mʌst/
5. **always** : (adv) luôn luôn /ˈɔːlweɪz/
6. **on time** : đúng giờ /ɒn taɪm/
7. **sure** : (adj) chắc chắn /ʃɔː/
8. **natural wonder** : kỳ quan thiên nhiên /ˈnætʃrəl ˈwʌndə/
9. **first** : (adj) đầu tiên, thứ nhất (adv) trước hết /fɜːst/
10. **talk** : (v) nói chuyện /tɔːk/
11. **look** : (v) nhìn, trông có vẻ /lʊk/
12. **amazing** : (adj) tuyệt vời, kinh ngạc /əˈmeɪzɪŋ/
13. **island** : (n) hòn đảo /ˈaɪlənd/
14. **scenery** : (n) phong cảnh /ˈsiːnəri/
15. **large** : (adj) to lớn, rộng lớn /lɑːdʒ/
16. **show** : (v) cho thấy, trình bày /ʃəʊ/
17. **how about** : thế còn, còn về /haʊ əˈbaʊt/

(SGK trang 48\)  
**Alice:** Chào, chào mừng tới câu lạc bộ Địa Lý của chúng tôi   
(Gõ cửa)   
**Alice:** Mời vào, Elena. Chúng tôi chỉ đang bắt đầu bây giờ. Nhưng hãy nhớ bạn phải luôn đúng giờ.   
**Elena:** Chắc chắn. Xin lỗi.   
**Alice:** Hôm nay tôi dự định nói về một vài kì quan thiên nhiên của Việt Nam.   
**Nick:** Tuyệt vời\! Trong bức tranh thứ nhất là cái gì?   
**Alice**: Nó là Gành Đá Dĩa ở Phú Yên  
**Elena:** Wow\! Nó trông thật đáng ngạc nhiên   
**Nick:** Bức tranh số 2 là vịnh Hạ Long phải không?   
**Alice:** Đúng. Bạn biết gì về nó?   
**Nick:** Nó có nhiều hòn đảo.   
**Alice:** Vâng. Phong cảnh thì tuyệt vời. Bức hình này cho thấy đảo Tuần Châu, một hòn đảo lớn  
**Tommy:** Còn bức tranh 3 thì sao?

**\* Vocabulary:** 

1. **accept** : (v) chấp nhận, đồng ý /əkˈsept/
2. **appointment** : (n) cuộc hẹn, sự bổ nhiệm /əˈpɔɪntmənt/
3. **famous** : (adj) nổi tiếng /ˈfeɪməs/
4. **island** : (n) hòn đảo /ˈaɪlənd/
5. **listen** : (v) nghe, lắng nghe /ˈlɪsən/
6. **must** : (v) phải, cần phải /mʌst/
7. **natural** : (adj) tự nhiên, thuộc về thiên nhiên /ˈnætʃrəl/
8. **scenery** : (n) phong cảnh, cảnh quan /ˈsiːnəri/
9. **start** : (v) bắt đầu /stɑːt/
10. **talk** : (v) nói chuyện /tɔːk/
11. **there** : (adv) ở đó, tại đó /ðeə(r)/
12. **welcome** : (adj) được chào đón; (v) chào đón /ˈwelkəm/
13. **wonder** : (n) điều kỳ diệu; (v) tự hỏi, thắc mắc /ˈwʌndə(r)/
14. **wonderful** : (adj) tuyệt vời, kỳ diệu /ˈwʌndəfl/
15. **cave** : (n) hang động, hang đá /keɪv/
16. **charm** : (n) sự quyến rũ, sức hấp dẫn; (v) mê hoặc /tʃɑːm/
17. **desert** : (n) sa mạc; (v) bỏ rơi, từ bỏ /ˈdezət/
18. **forest** : (n) rừng /ˈfɒrɪst/
19. **in** : (prep) trong, ở trong /ɪn/
20. **make** : (v) làm, chế tạo (make-made-made) /meɪk/
21. **man** : (n) người đàn ông, con người /mæn/
22. **mountain** : (n) núi /ˈmaʊntɪn/
23. **national** : (adj) thuộc quốc gia, dân tộc /ˈnæʃnəl/
24. **rock** : (n) đá, núi đá; (v) lắc lư, đung đưa /rɒk/
25. **shape** : (n) hình dạng; (v) định hình /ʃeɪp/
26. **size** : (n) kích thước, cỡ /saɪz/
27. **thousand** : (n) một nghìn /ˈθaʊznd/
    **Bài 2 trang 49**  
1. Người ta không tạo ra Gành Đá Dĩa. Chúng là đá tự nhiên → natural   
2. Vịnh Hạ Long có hàn nghìn hòn đảo lớn và nhỏ → islands   
3. Vịnh Hạ Long thì cuốn hút và phong cảnh thì tuyệt vời → scenery   
4. Gành Đá Dĩa có những cột đá đáng ngạc nhiên trong những hình dạng và kích thước khác nhau  → amazing  
5. Có nhiều kì quan thiên nhiên và nhân tạo ở Việt Nam → wonders 

**Bài 3 trang 49** 

1. **desert** : sa mạc
2. **island** : đảo
3. **cave** : hang động
4. **river** : sông
5. **waterfall** : thác nước
6. **mountain** : núi
7. **forest** : rừng
8. **beach** : bãi biển

**Bài 4 trang 49**  
1.a   
2\. a  
3\. b  
4\. b  
5\. b   
**A CLOSER LOOK 1**   
**Vocabulary**

1. **backpack** : (n) ba lô /ˈbækpæk/
2. **finish** : (v) hoàn thành, kết thúc /ˈfɪnɪʃ/
3. **holiday** : (n) kỳ nghỉ, ngày lễ /ˈhɒlɪdeɪ/
4. **hurt** : (v) làm đau, gây tổn thương (hurt-hurt-hurt) /hɜːt/
5. **item** : (n) món đồ, khoản mục /ˈaɪtəm/
6. **overnight** : (adv) qua đêm; (adj) xảy ra qua đêm /ˌəʊvəˈnaɪt/
7. **pack** : (v) đóng gói, xếp đồ; (n) gói, túi /pæk/
8. **plaster** : (n) cao, thạch cao; (v) dán, trát /ˈplɑːstə(r)/
9. **scissors** : (n) cái kéo (luôn ở dạng số nhiều) /ˈsɪzəz/
10. **sleep** : (v) ngủ (sleep-slept-slept) /sliːp/
11. **some** : (det) một số, một vài /sʌm/
12. **thing** : (n) vật, đồ vật /θɪŋ/
13. **useful** : (adj) hữu ích /ˈjuːsfl/
14. **usefulness** : (n) sự hữu ích /ˈjuːsfʊlnəs/

**Bài 1 trang 50**

1. **plaster** : băng keo cá nhân
2. **suncream** : kem chống nắng
3. **sleeping bag** : túi ngủ
4. **scissors** : kéo
5. **backpack** : ba lô
6. **compass** : la bàn

**Bài 2 trang 50** 

1. Chúng tôi bị lạc. Vui lòng cho tôi cái la bàn → compass   
2. Nó thì nắng hôm nay. Tôi cần đặt lên (bôi) một ít suncream   
3. Một túi ngủ thì rất hữu ích khi bạn đi cắm trại qua đêm → sleeping bag   
4. Tôi đã hoàn thành đóng gói. Tất cả những thứ của tôi thì ở trong ba lô của tôi → backpack  
5. Chân của tôi đau. Tôi cần phải đặt một băng keo cá nhân trên chân của tôi → plaster

**Pronuncation /t/ and /d/**  
/t/:

- Âm vô thanh (không rung dây thanh)  
- Đặt lưỡi chạm vào phía sau răng trên  
- Thả lưỡi ra tạo tiếng nổ nhẹ

Ví dụ: time, tea, top

/d/:

- Âm hữu thanh (có rung dây thanh)  
- Vị trí lưỡi giống /t/  
- Có rung dây thanh khi phát âm

Ví dụ: dog, day, do

**A CLOSER LOOK 2**   
**Danh từ đếm được và không đếm được** 

- Danh từ đếm được là những danh từ có thể đếm được trực tiếp bằng con số   
- Danh từ không đếm được là những danh không trực tiếp đếm được bằng con số   
+ Chất lỏng   
+ Những vật chất nhỏ, nhiều, cần cân đo, đong, đếm   
+ Những khái niệm trừu tượng 

Ví dụ: 

- Đếm được: người, xe, bút, thước   
  person, car, pen, ruler.   
- Không đếm được: air, coffee, dust, hair

**Bài 1 trang 51:** Xác định từ gạch dưới là đếm được (C) hay không đếm được (U)

1. C   
2. U  
3. U  
4. C   
5. U 

**Bài 2 trang 52**

**Ngữ pháp: some, any, much, many, few, a few, little, a little** 

some: một vài, một ít → thường dùng trong câu khẳng định  
any: bất cứ … nào  → dùng trong câu hỏi hoặc nghi vấn  
much: nhiều → thường dùng trong câu phủ định hoặc nghi vấn, nhưng không bắt buộc  
**much đi với danh từ không đếm được**  
many: nhiều  → thường dùng trong câu phủ định hoặc nghi vấn, nhưng không bắt buộc   
**much đi với danh từ đếm được số nhiều**  
a lot of \= lots of: nhiều   
a few: ít   
few: gần như không có   
→ few, a few đi với danh từ đếm được  
little: ít  
a little: gần như không có   
→ little, a little đi với với danh từ không đếm được 

- dùng trong câu nào   
- đi kèm với danh từ đếm được, k đếm được

| Từ | Câu | Đi kèm |
| :---- | :---- | :---- |
| some | khẳng định, lời mời | cả hai  |
| any | phủ định, nghi vấn | cả hai |
| much: nhiều little: ít | ***thường**  xuất hiện trong câu phủ định, nghi vấn*  | không đếm được |
| many: nhiều few: ít | ***thường**  xuất hiện trong câu phủ định, nghi vấn*  |  đếm được  |
| a lot of \= lots of: nhiều | ***thường** xuất hiện trong câu khẳng định* | cả hai  |

**Bài 2 trang 52** 

1. A  
2. B  
3. A   
4. B   
5. A 

**Bài 3 trang 52** 

1. many   
2. any   
3. much   
4. some   
5. a 

**Must và mustn’t: phải và không được phép**   
I must keep silent in class.   
I mustn’t talk when the teacher is talking.

**Chủ từ \+ must \+ V0: Ai đó phải làm gì**   
**Chủ từ \+ must not \= mustn’t \+ V0: Ai đó không được phép làm gì**   
**Bài 4 trang 52:** Điền must hoặc mustn’t 

1. must   
2. mustn’t   
3. must   
4. mustn’t   
5. must 

**Bài 5 trang 52: Viết câu: Ai đó phải làm gì hoặc không được làm gì trong lớp học**   
3\. We must do homework before class.   
4\. We mustn’t drink milk tea during lessons.   
5\. We mustn’t eat during lessons. 

6. We mustn’t listen to music during class   
7. We mustn’t play football in the classroom

**COMMUNICATION**   
**Rủ rê & chấp nhận lời rủ rê**   
Let’s play football\!  → Let’s \+ V0\!: Chúng ta hãy … đi  
How about swimming?  → What about/How about \+ Ving? Còn việc …. thì sao?   
Do you feel like playing football with me? → Bạn có có nhã hứng …. ?   
**Trả lời** 

- Sure  
- Great.   
- That’s a good idea.   
- I’d love to. 

**Từ chối:**

- I’d love to, but \+ lí do   
  Ví dụ: I’d love to but I’m busy this afternoon   
- I’m afraid I can’t. \<lí do\> 

**Bài tập: rủ bạn Cường đi bơi lúc 9h chủ nhật (200 điểm)**  

A: Cường, let’s go swimming this Sunday.   
Cường: That’s fine. What time can we meet?  
A: How about 9 o’clock?   
Cường: Sure. I’ll meet you at that time.   
**Lưu ý:**  
Let’s \+ V0  
How about \+ Ving  
**Bài 3 trang 53** 

1. **add** : (v) thêm vào, cộng thêm /æd/
2. **all** : (det) tất cả, toàn bộ /ɔːl/
3. **attraction** : (n) sự hấp dẫn, điểm thu hút /əˈtrækʃn/
4. **complete** : (v) hoàn thành; (adj) hoàn chỉnh /kəmˈpliːt/
5. **enjoy** : (v) thích thú, tận hưởng /ɪnˈdʒɔɪ/
6. **kite** : (n) diều /kaɪt/
7. **land** : (n) đất, đất liền; (v) hạ cánh /lænd/
8. **landscape** : (n) phong cảnh, cảnh quan /ˈlændskeɪp/
9. **large** : (adj) to, lớn /lɑːdʒ/
10. **late** : (adj) muộn, trễ /leɪt/
11. **place** : (n) địa điểm, nơi chốn; (v) đặt /pleɪs/
12. **popular** : (adj) phổ biến, được ưa chuộng /ˈpɒpjələ(r)/
13. **rain** : (n) mưa; (v) mưa /reɪn/
14. **ride** : (v) cưỡi, đi xe (ride-rode-ridden) /raɪd/
15. **slope** : (n) sườn dốc, độ dốc; (v) dốc /sləʊp/
16. **speak** : (v) nói (speak-spoke-spoken) /spiːk/
17. **tell** : (v) kể, nói (tell-told-told) /tel/
18. **umbrella** : (n) ô, dù /ʌmˈbrelə/
19. **visitor** : (n) khách, người thăm /ˈvɪzɪtə(r)/

**Bài dịch: Bài 3 trang 53**  
Hướng dẫn du lịch toàn cầu 

Dãy Hi-ma-lay-a là một dãy núi. Nó thì rất đặc biệt. Nó có núi cao nhất thế giới \- đỉnh Everest.   
Khi thăm đỉnh Hi-ma-lay-a, hãy nhớ theo những luật sau đây: 

- Bạn phải hỏi trước khi bạn thăm khu vực   
- Bạn không được phép đi du lịch một mình. Luôn luôn đi theo nhóm   
- Bạn không được xả rác   
- Bạn chỉ được mang những thứ cần thiết   
- Bạn cũng phải mang quần áo thích hợp. Đừng mang quần short hoặc áo thun. 

**Bài 4**   
**must:** food, warm clothes, matches (que diêm), backpack  
**mustn’t**: T-shirts, shorts, skirts, handbag   
**Bài 5**   
A: I’d like to go to the Himalayas next month  
B: OK. I think you must bring a waterproof coat. It’s cold and rainy there\!  
A: Yes. Anything else?   
B: You must bring warm clothes like scarves (khăn choàng) and wool jackets (áo khoác lông cừu). It is very cold in the mountains. And you mustn’t wear shorts in that weather.  
**SKILLS 1**  
**Reading**   
**Vocabulary** 

1. activity (activities) \- (n) hoạt động, các hoạt động giải trí /æk'tiviti/  
2. attraction \- (n) điểm du lịch hấp dẫn /ə'trækʃn/  
3. bike \- (n) xe đạp /baik/  
4. cave (caves) \- (n) hang động /keɪv/  
5. colour (colours) \- (n) màu sắc /'kʌlə/  
6. **fly \- (v) thả, cho bay (trong ngữ cảnh** : thả diều) /flaɪ/
7. here \- (trạng từ) ở đây /hɪə/  
8. kite (kites) \- (n) diều, cánh diều /kaɪt/  
9. landscape (landscapes) \- (n) phong cảnh, cảnh quan /'lændskeɪp/  
10. morning \- (n) buổi sáng /ˈmɔːnɪŋ/  
11. popular \- (tính từ) nổi tiếng, được ưa chuộng /'pɒpjʊlə/  
12. ride \- (v) đi, chạy (xe đạp) /raɪd/  
13. slope (slopes) \- (n) sườn dốc, triền cát /sləʊp/  
14. **wear \- (v) mặc, đeo (trong ngữ cảnh** : bôi, thoa kem chống nắng) /weə/

Vịnh Hạ Long thì ở Quảng Ninh. Nó có nhiều hòn đảo và hang động. Tuần Châu, với nhiều bãi biển đẹp của nó, là một điểm thu hút khách du lịch phổ biến ở Vịnh Hạ Long . Ở đó bạn có thể thưởng thức hải sản tuyệt vời. Và bạn có thể tham gia vào những hoạt động thú vị. Vịnh Hạ Long là kì quan thiên nhiên đẹp nhất của Việt Nam.

Mũi Né thì phổ biến (nổi tiếng) vì những phong cảnh đáng ngạc nhiên của nó. Cát có những màu khác nhau: trắng, vàng, đỏ …. Nó thì giống như một sa mạc ở đây. Bạn có thể cưỡi xe đạp xuống các sườn dốc. Bạn cũng có quyền thả diều, hoặc có một buổi dã ngoại bên bờ biển. Thời gian tốt nhất để viếng thăm những Đồi Cát Mũi Né là sáng sớm hoặc chiều muộn. Hãy nhớ thoa kem chống nắng và mang theo nước.   
**Bài 2 trang 54** 

1. Vịnh Hạ Long thì nổi tiềng vì … đẹp của nó → islands   
2. Vịnh Hạ Long thì là ….. số một ở Việt Nam →  wonder   
3. Một …. là một diện tích đất lớn với rất ít nước → desert  
4. …… mang dù, vì nó thường mưa ở đó → remember   
5. Nhiều người …. đảo Lý Sơn vào mùa hè → visit

**Bài 3 trang 54** 

1. Vịnh Hạ Long thì ở đâu?  → It is in Quang Ninh  
2. Bạn có thể làm gì ở Vịnh Hạ Long? →We can enjoy great seafood and join in exciting activities.   
3. Có một sa mạc ở Mũi Né không? → No. It is just like a desert.   
4. Bạn có thể có một buổi dã ngoại ở đâu ở Mũi Né? → We can have a picnic by the beach.   
5. Thời gian tốt nhất để thăm Đồi cát Mũi Né là gì? → It is early morning and late afternoon.

**Speaking**   
**Bài 4 trang 54**   
Vịnh Hạ Long và Mũi Né có gì?

**Hạ Long Bay:**  interesting islands, beautiful caves, great seafood.  
**Mũi Né:** colorful sand, Sand Dunes, kite-flying, ….   
Ha Long Bay has a lot of interesting islands. It also has beautiful caves. We can go sailing and enjoy great seafood there.   
Mui Ne is popular for its amazing landscapes. It has sand of different colours. We can fly kites and have a picnic there. We can also visit the Sand Dunes. 

**Bài 5 trang 54** 

You must wear suncream   
You mustn’t litter.   
You must bring swimsuits   
You mustn’t have a picnic on the Sand Dunes. 

**SKILLS 2**   
**Listening**   
**Vocabulary** 

1. **green forest** : khu rừng xanh /ˌgriːn ˈfɒrɪst/
2. **resort** : (n) khu nghỉ dưỡng /rɪˈzɔːt/
3. **hotel** : (n) khách sạn /həʊˈtel/
4. **bar** : (n) quán bar /bɑː/
5. **international airport** : sân bay quốc tế /ˌɪntəˈnæʃənəl ˈeəpɔːt/
6. **fishing village** : làng chài /ˈfɪʃɪŋ ˈvɪlɪdʒ/
7. **pagoda** : (n) chùa /pəˈgəʊdə/
8. **temple** : (n) đền, miếu /ˈtempl/
9. **national park** : công viên quốc gia /ˌnæʃənəl ˈpɑːk/
10. **sailing** : (n) môn đua thuyền buồm, (v) đi thuyền buồm /ˈseɪlɪŋ/
11. **water sport** : môn thể thao dưới nước /ˈwɔːtə spɔːt/
12. **market** : (n) chợ, thị trường /ˈmɑːkɪt/

1\. T  
(Phú Quốc là một hòn đảo rất đẹp ở Việt Nam. \=\> Đúng)  
2\. F  
(Ở Phú Quốc không có rừng xanh. \=\> Sai)  
3\. T  
(Khách du lịch có thể đến thăm làng chài và vườn quốc gia ở đó. \=\> Đúng)  
4\. F  
(Bạn không được chơi các môn thể thao dưới nước ở Phú Quốc.  \=\> Sai)  
5\. T  
(Mọi người bán những thứ thú vị ở chợ. \=\> Đúng)  
**Writing**   
**Bài 3 trang 55**

1. **Tên của điểm thu hút** : Đà Lạt
2. **Nó ở đâu** : Lâm Đồng province
3. **Đi tới bằng cách nào** : coach or air plane
4. **Có gì đặc biệt về nó** : cool weather, fresh air, beautiful landscapes, large forests, many waterfalls, a lot of flowers.
5. **Bạn có thể làm gì ở đó** : take photo, go camping, picnic, go sightseeing.

**Bài 4 trang 55**   
I am writing about Đà Lạt. It is in Lâm Đồng Province in Central Highland. We can go there by coach or by plane. It is famous for its cool weather and fresh air. It is also known as ‘The City of Flowers’. The landscape is beautiful with large forests and many waterfalls and a lot of flowers. Tourists can go sightseeing, take photos or go camping or have a picnic by the forests. 

**LOOKING BACK**   
**Bài 1 trang 56**

1. waterfall   
2. cave   
3. desert   
4. river   
5. beach   
6. island

**Bài 2 trang 56** 

1. đỉnh Fansipan   
2. Ban Gioc waterfall   
3. Ha Long Bay   
4. Cuc Phuong Forest   
5. The Sahara Desert   
6. Con Dao island

**Bài 3 trang 56** 

1\. scissors (cái kéo)  
2\. sleeping bag  (túi ngủ)  
3\. compass (la bàn)  
4\. backpack  (ba lô)  
5\. plaster (băng cá nhân)   
**Bài 4 trang 56**   
\- there is \+ danh từ số ít/ danh từ không đếm được  
\- there are \+ danh từ số nhiều  
\- many \+ danh từ số nhiều  
\- much \+ danh từ không đếm được   
1\. is → are  2\. are → is  3\. instrument → instruments   
4\. are → is 5\. luggages → luggage.   
**Bài 5 trang 56** 

1. must   
2.  must   
3. mustn’t   
4. must   
5. mustn’t