# UNIT 11: TRAVELLING IN THE FUTURE - DU HÀNH TRONG TƯƠNG LAI

## GETTING STARTED - BẮT ĐẦU

### 📚 Vocabulary - Từ vựng
- **traffic jam** : (n) kẹt xe /ˈtræfɪk dʒæm/
- **will** : (modal) sẽ /wɪl/
- **how** : (adv) bằng cách nào, như thế nào /haʊ/
- **it's a pity that...** : thật đáng tiếc rằng... /ɪts ə ˈpɪti ðæt/
- **hyperloop** : (n) hyperloop (hệ thống vận chuyển tốc độ cao trong ống chân không) /ˈhaɪpəluːp/
- **amazing** : (adj) tuyệt vời, kinh ngạc /əˈmeɪzɪŋ/
- **a system of tubes** : hệ thống ống /ə ˈsɪstəm əv tjuːbz/
- **from one place to another** : từ nơi này đến nơi khác /frɒm wʌn pleɪs tə əˈnʌðə/
- **in just minutes** : chỉ trong vài phút /ɪn dʒʌst ˈmɪnɪts/
- **even** : (adv) thậm chí, ngay cả /ˈiːvən/
- **event** : (n) sự kiện, biến cố /ɪˈvent/
- **teleporter** : (n) máy dịch chuyển tức thời, cổng dịch chuyển /ˈtelɪˌpɔːtə/
- **different mode of travel** : phương thức di chuyển khác nhau /ˈdɪfrənt məʊd əv ˈtrævəl/
- **soon** : (adv) sớm, chẳng bao lâu nữa /suːn/
- **campsite** : (n) khu cắm trại /ˈkæmpsaɪt/
- **mode** : (n) phương thức, cách thức /məʊd/
- **eco-friendly** : (adj) thân thiện với môi trường /ˌiːkəʊ ˈfrendli/
- **totally** : (adv) hoàn toàn /ˈtəʊtəli/
- **safe** : (adj) an toàn /seɪf/
- **fumes** : (n) khói thải /fjuːmz/
- **worry about** : (v) lo lắng về /ˈwʌri əˈbaʊt/
- **bad weather** : thời tiết xấu /bæd ˈweðə/

### 💬 Content - Nội dung
**Minh**: Oh, another traffic jam.
Ôi, lại một cái kẹt xe nữa.

**Ann**: How will we get to the campsite on time now?
Làm sao chúng ta sẽ đến địa điểm cắm trại đúng giờ bây giờ?

**Minh**: I don't know. It's a pity that we don't have a hyperloop now!
Tôi không biết. Thật đáng tiếc rằng chúng ta không có hyperloop bây giờ!

**Ann**: What's a hyperloop?
Hyperloop là gì?

**Minh**: It's a system of tubes. You can go from one place to another in just minutes.
Nó là một hệ thống ống. Bạn có thể đi từ nơi này đến nơi khác chỉ trong vài phút.

**Ann**: Amazing! Even faster than a flying car!
Tuyệt vời! Thậm chí nhanh hơn cả xe bay!

**Minh**: Yes, and there are no traffic jams.
Đúng vậy, và không có kẹt xe.

**Ann**: Will it pollute the environment?
Liệu nó có gây ô nhiễm môi trường không?

**Minh**: No. There will be no fumes, and it's totally safe.
Không. Sẽ không có khói thải, và nó hoàn toàn an toàn.

**Ann**: Like a teleporter?
Giống như máy dịch chuyển?

**Minh**: A teleporter is also fast, safe and eco-friendly. But it's a different mode of travel.
Máy dịch chuyển cũng nhanh, an toàn và thân thiện với môi trường. Nhưng nó là một phương thức di chuyển khác.

**Ann**: So when we travel in hyperloops, we won't have to worry about bad weather, right?
Vậy khi chúng ta di chuyển trong hyperloop, chúng ta sẽ không phải lo lắng về thời tiết xấu, đúng không?

**Minh**: Right.
Đúng vậy.

**Ann**: Sounds great! I hope we'll have them soon.
Nghe thật tuyệt! Tôi hy vọng chúng ta sẽ có chúng sớm.

### ✍️ Exercises - Bài tập

**Bài 2**: What are Ann and Minh talking about? (Ann và Minh đang nói về điều gì?)
A. Travelling by teleporter (Du lịch bằng máy dịch chuyển)
B. Future modes of travel (Các phương thức di chuyển trong tương lai)
C. Being in a traffic jam (Bị kẹt xe)

**Answer:** B

**Bài 3**: Read the conversation again and tick (✓) T (True) or F (False) (Đọc lại đoạn hội thoại và đánh dấu T (Đúng) hoặc F (Sai))
1. Ann and Minh are in a traffic jam now. (Ann và Minh đang bị kẹt xe bây giờ.)
2. Ann and Minh are going to school. (Ann và Minh đang đi học.)
3. Ann knows a lot about hyperloops. (Ann biết rất nhiều về hyperloop.)
4. Minh seems to know about different modes of travel. (Minh có vẻ biết về các phương thức di chuyển khác nhau.)
5. They hope there will be hyperloops soon. (Họ hy vọng sẽ sớm có hyperloop.)

**Answers:** 1-T, 2-F, 3-F, 4-T, 5-T

**Bài 4**: Find the words and phrases that describe the means of transport in the conversation and write them in the correct columns. (Tìm các từ và cụm từ mô tả phương tiện giao thông trong đoạn hội thoại và viết chúng vào đúng cột.)

| hyperloop | teleporter |
|-----------|-----------|
| no fumes (không khói thải) | fast (nhanh) |
| safe (an toàn) | safe (an toàn) |
| fast (nhanh) | eco-friendly (thân thiện với môi trường) |

**Bài 5**: QUIZ What vehicle is this? Work in pairs. Discuss to find out the vehicles in these sentences. (Đố vui: Đây là phương tiện gì? Làm việc theo cặp. Thảo luận để tìm ra phương tiện trong các câu này.)

1. It has two wheels and a pedal. (Nó có hai bánh xe và một bàn đạp) → **bicycle** (xe đạp)
2. It has four wheels. It runs on petrol or electricity and can carry up to eight passengers. (Nó có bốn bánh xe. Nó chạy bằng xăng hoặc điện và có thể chở tối đa tám hành khách) → **car** (ô tô)
3. It carries many passengers and runs on rails. (Nó chở nhiều hành khách và chạy trên đường ray) → **train** (tàu hỏa)
4. It sails on the sea. It needs wind to sail. (Nó chèo trên biển. Nó cần gió để chèo) → **boat** (thuyền buồm)
5. It travels in space. It can even take people to the moon. (Nó di chuyển trong không gian. Nó thậm chí có thể đưa con người tới mặt trăng) → **spaceship** (tàu vũ trụ)

## A CLOSER LOOK 1 - TÌM HIỂU THÊM 1

### 📚 Vocabulary - Từ vựng
- **flying car** : (n) xe hơi bay /ˈflaɪɪŋ kɑː/
- **bamboo-copter** : (n) chong chóng tre /ˌbæmˈbuː ˈkɒptə/
- **skyTran** : (n) hệ thống vận chuyển trên không skyTran /ˈskaɪtræn/
- **solar-powered ship** : (n) tàu chạy bằng năng lượng mặt trời /ˈsəʊlə ˈpaʊəd ʃɪp/
- **eco-friendly** : (adj) thân thiện với môi trường /ˌiːkəʊ ˈfrendli/
- **simple** : (adj) đơn giản /ˈsɪmpl/
- **fly away** : (v) bay đi /flaɪ əˈweɪ/
- **instead of** : (prep) thay vì /ɪnˈsted ɒv/
- **airplane** : (n) máy bay /ˈeəpleɪn/
- **cause pollution** : gây ô nhiễm /kɔːz pəˈluːʃn/
- **driverless car** : (n) xe hơi không người lái /ˈdraɪvələs kɑː/
- **ride** : (v) cưỡi, đi /raɪd/
- **drive** : (v) lái xe /draɪv/
- **fly** : (v) bay /flaɪ/
- **sail** : (v) chèo thuyền /seɪl/
- **run on** : (v) chạy bằng /rʌn ɒn/
- **space** : (n) không gian /speɪs/
- **autopilot** : (n) chế độ lái tự động /ˈɔːtəʊpaɪlət/

### ✍️ Exercises - Bài tập

**Bài 1**: Write the words or phrases under the correct pictures. (Viết từ hoặc cụm từ dưới hình ảnh đúng.)
1. solar-powered ship (tàu chạy bằng năng lượng mặt trời)
2. flying car (ô tô bay)
3. bamboo-copter (chong chóng tre)
4. hyperloop (tàu cao tốc)
5. skyTran (tàu trên cao)

**Answers:** 1-d, 2-c, 3-a, 4-b, 5-e

**Bài 2**: Write words or phrases from 1 in the correct columns. (Viết các từ hoặc cụm từ từ bài 1 vào đúng cột.)

| ride | drive | fly | sail |
|------|-------|-----|------|
| hyperloop | a car | bamboo-copter | solar-powered ship |
| skyTran | | a plane | |
| a horse | | flying car | |
| a bike | | | |

**Bài 3**: Complete the sentences, using the words or phrases from the box. (Hoàn thành câu, sử dụng các từ hoặc cụm từ trong hộp.)

**bamboo-copter   fly   ride   hyperloop   eco-friendly**

1. Travelling by **bamboo-copter** is simple. You just put it on and fly away.
2. People will soon **fly** in flying cars instead of driving normal cars.
3. It will be fun to **ride** a skyTran to work.
4. Travelling by **hyperloop** will be faster than by airplane.
5. Solar-powered ships are **eco-friendly**. They will not cause pollution.

### 🗣️ Pronunciation - Phát âm

**Nhấn trọng âm trong câu (Sentence stress)**

Trong tiếng Anh, một câu có cả từ được nhấn trọng âm và từ không nhấn trọng âm:

- **Từ thường được nhấn trọng âm**: danh từ, động từ chính, tính từ, trạng từ, và trợ động từ phủ định.
  Ví dụ: We **don't like** travelling.

- **Từ thường không nhấn trọng âm**: giới từ, đại từ, mạo từ, và trợ động từ.
  Ví dụ: I will **go** by **car**.

**Bài 4**: Listen to the sentences and repeat. Pay attention to the bold words. (Nghe các câu và lặp lại. Chú ý đến các từ in đậm.)

1. We'll **have** a **new** motorway **soon**.
2. **flying** cars will be **popular**.
3. **How** will **people travel** in **twenty years**?
4. We **won't have** **driverless** buses.
5. **Will** we **have** **hyperloops** in the **future**?

**Bài 5**: Listen to the sentences and repeat. How many stresses are there in each sentence? (Nghe các câu và lặp lại. Có bao nhiêu trọng âm trong mỗi câu?)

1. That's an **eco-friendly** **car**. (2 stresses)
2. A **teleporter** is **super fast**. (3 stresses)
3. It'll be a **green** **mode** of **transport**. (3 stresses)
4. **You** can **avoid** **traffic jams**. (3 stresses)
5. They're **travelling** in a **driverless** **car**. (3 stresses)

## A CLOSER LOOK 2 - TÌM HIỂU THÊM 2

### 📚 Vocabulary - Từ vựng
- **charge** : (v) sạc /tʃɑːdʒ/
- **battery** : (n) pin /ˈbætəri/
- **popular** : (adj) phổ biến, được nhiều người ưa thích /ˈpɒpjələr/
- **planet** : (n) hành tinh /ˈplænɪt/
- **holiday** : (n) kỳ nghỉ /ˈhɒlɪdeɪ/
- **parking place** : (n) chỗ đậu xe /ˈpɑːkɪŋ pleɪs/
- **disappear** : (v) biến mất /ˌdɪsəˈpɪə/
- **appear** : (v) xuất hiện /əˈpɪə/
- **certainly** : (adv) chắc chắn /ˈsɜːtnli/
- **travel** : (v) di chuyển, đi /ˈtrævəl/

### 📖 Grammar - Ngữ pháp

#### **Thì tương lai đơn (Future Simple)**

**Cấu trúc:**
- **Khẳng định:** S + will + V (nguyên mẫu)
- **Phủ định:** S + will not (won't) + V (nguyên mẫu)
- **Nghi vấn:** Will + S + V (nguyên mẫu)?

**Cách sử dụng:**
- Diễn tả hành động sẽ xảy ra trong tương lai
- Diễn tả quyết định đột xuất
- Đưa ra lời hứa, lời đề nghị, lời cảnh báo
- Dự đoán về tương lai

**Ví dụ:**
- We **will travel** by hyperloop in the future. (Chúng ta sẽ di chuyển bằng hyperloop trong tương lai.)
- Cars **won't use** petrol. (Ô tô sẽ không sử dụng xăng.)
- **Will** people **travel** to Mars? (Mọi người sẽ du lịch đến sao Hỏa không?)

#### **Đại từ sở hữu (Possessive Pronouns)**

| Tính từ sở hữu | Đại từ sở hữu | Ví dụ |
|----------------|---------------|--------|
| my | mine | This is **my** car. = The car is **mine**. |
| your | yours | Is that **your** bag? = Is that bag **yours**? |
| his | his | **His** phone is new. = The new phone is **his**. |
| her | hers | **Her** sister is tall. = The tall sister is **hers**. |
| its | its | The dog wagged **its** tail. |
| our | ours | **Our** house is big. = The big house is **ours**. |
| their | theirs | **Their** car is fast. = The fast car is **theirs**. |

### ✍️ Exercises - Bài tập

**Bài 1**: Complete the sentences with will or won't. (Hoàn thành câu với will hoặc won't.)
1. Petrol-powered cars are not eco-friendly, so they **won't** be popular.
2. We **will** travel by teleporter soon because it is faster.
3. Rich people **will** choose to travel to other planets for holidays.
4. People **won't** take holidays in crowded places as before.
5. Bullet trains **will** soon become a popular means of transport in Viet Nam because they are green and safe.

**Bài 2**: Rearrange the words and phrases to make sentences. (Sắp xếp lại các từ và cụm từ để tạo thành câu.)
1. We will have driverless cars in 2030.
2. Cities won't allow cars in 2050.
3. Will electric cars be popular?
4. We will use solar-powered buses soon.
5. Will we have enough parking places in ten years?

**Bài 3**: Use the correct form of the verbs in brackets to complete the conversation. (Sử dụng dạng đúng của động từ trong ngoặc để hoàn thành hội thoại.)

Alice: Do you think travelling by teleporter (1. be) **will be** possible soon?
Tom: Yes. I hear it (2. take) **will take** very little time to travel from one country to another.
Alice: Great! We (3. not have) **won't have** to wait at the airport.
Tom: (4. we/have) **Will we have** to learn to drive it?
Alice: No. We (5. not drive) **won't drive** it. It makes us disappear then appear in another place in just seconds.
Tom: How wonderful! There (6. not be) **won't be** any traffic jams.

**Bài 4**: Work in pairs. Look at the pictures and tell your partner if you will / won't travel by these means of transport. Give reason(s). (Làm việc theo cặp. Nhìn vào hình ảnh và nói với bạn của bạn liệu bạn sẽ / không đi lại bằng các phương tiện giao thông này. Đưa ra lý do.)

Example answers:
- I will travel by flying car because it is very fast.
- I won't travel by car in another 20 years because it pollutes the air.
- I will travel by hyperloop because it is very fast and has no fumes.
- I won't travel by petrol-powered cars because they are not eco-friendly.

**Bài 5**: Replace the underlined phrases with possessive pronouns. (Thay thế các cụm từ được gạch chân bằng đại từ sở hữu.)
1. My motorbike runs on electricity. *Their motorbikes* don't. → **theirs**
2. Her dream is to travel to Mars. *My dream* is to live on the Moon. → **Mine**
3. We don't like travelling by bus. *His travel* is by train. → **His**
4. *Your bus* was late. *Ours* isn't. → **Ours**
5. *Our car* is a driverless one. *Their car* isn't. → **theirs**

## COMMUNICATION - GIAO TIẾP

### 📚 Vocabulary - Từ vựng
- **Do you think we will...?** : Bạn có nghĩ chúng ta sẽ...? /duː juː θɪŋk wiː wɪl/
- **We certainly will** : Chúng ta chắc chắn sẽ /wiː ˈsɜːtnli wɪl/
- **It probably won't** : Có lẽ nó sẽ không /ɪt ˈprɒbəbli wəʊnt/
- **be able to** : (v) có thể, có khả năng /biː ˈeɪbl tuː/
- **wheel** : (n) bánh xe /wiːl/
- **self-balancing** : (adj) tự cân bằng /ˌself ˈbælənsɪŋ/
- **means of transport** : phương tiện di chuyển /miːnz əv ˈtrænspɔːt/
- **turn on the switch** : bật công tắc /tɜːn ɒn ðə swɪtʃ/
- **fall off** : (v) ngã khỏi /fɔːl ɒf/
- **dangerous** : (adj) nguy hiểm /ˈdeɪndʒərəs/
- **across oceans** : băng qua đại dương /əˈkrɒs ˈəʊʃnz/
- **wing** : (n) cánh /wɪŋ/
- **electricity** : (n) điện /ɪˌlekˈtrɪsəti/
- **use** : (v) sử dụng /juːz/

### 🗣️ Activities - Hoạt động

**Bài 1**: Listen and read the conversation. Pay attention to the highlighted sentences. (Nghe và đọc hội thoại. Chú ý các câu được tô đậm.)

Ann: Do you think we will travel by flying car in the future?
Bạn có nghĩ chúng ta sẽ di chuyển bằng ô tô bay trong tương lai không?

Tom: We certainly will.
Chúng ta chắc chắn sẽ.

Ann: Will it be able to fly across oceans?
Liệu nó có thể bay qua các đại dương không?

Tom: It probably won't.
Chắc sẽ không đâu.

**Bài 2**: Work in pairs. Make similar conversations about future travelling. (Làm việc theo cặp. Thực hiện các cuộc hội thoại tương tự về du lịch trong tương lai.)

Example:
1. Nam: Do you think we will travel long distances by hyperloop in the future?
   Phong: We certainly will.
   Nam: Will it be able to run by wind energy?
   Phong: It probably won't.

2. Minh: Do you think we will travel to other planets for holidays?
   Tuan: We certainly will.
   Minh: Will we be able to travel there by SkyTran?
   Tuan: It probably won't.

**Bài 3**: Complete the conversation between Mark and Lan with the questions and answers from the box. (Hoàn thành cuộc hội thoại giữa Mark và Lan với các câu hỏi và câu trả lời trong hộp.)

A - How many wheels will a walkcar have?
B - I think they will go by walkcar.
C - It is self-balancing.
D - What will it run on?

**Answer order:** B - A - D - C

**Bài 4**: Work in pairs. Choose one means of transport below and make a similar conversation as in 3. (Làm việc theo cặp. Chọn một phương tiện giao thông dưới đây và thực hiện một cuộc hội thoại tương tự như trong bài 3.)

Example dialogue:
John: So what means of transport will students use to go to school?
Linh: I think they will go by bamboo-copter.
John: Bamboo-copter? How many wings will a bamboo-copter have?
Linh: Three.
John: How will it work?
Linh: You put it on and turn on the switch and fly away.
John: What will it run on?
Linh: Electricity.
John: Won't users fall off it? It sounds dangerous.
Linh: No, they won't. It is self-balancing. It is fun and safe to use it.
John: I can't wait to get one.

## SKILLS 1 - KỸ NĂNG 1

### 📚 Vocabulary - Từ vựng
- **car company** : (n) công ty xe hơi /kɑː ˈkʌmpəni/
- **make electric cars** : sản xuất xe điện /meɪk ɪˈlektrɪk kɑːz/
- **introduce** : (v) giới thiệu /ˌɪntrəˈdjuːs/
- **model** : (n) mẫu, kiểu mẫu /ˈmɒdl/
- **the first model** : mẫu đầu tiên /ðə fɜːst ˈmɒdl/
- **not very expensive** : không quá đắt /nɒt ˈveri ɪkˈspensɪv/
- **four seats** : bốn chỗ ngồi /fɔː siːts/
- **travel** : (v) đi, di chuyển /ˈtrævl/
- **over 350 km per hour** : trên 350 km mỗi giờ /ˈəʊvə θriː ˈhʌndrəd ænd ˈfɪfti ˌkɪˈlɒmɪtəz pɜːr ˈaʊə/
- **charge** : (v) sạc /tʃɑːdʒ/
- **battery** : (n) pin /ˈbætri/
- **gaming screen** : màn hình chơi game /ˈɡeɪmɪŋ skriːn/
- **the largest and safest electric car** : xe điện lớn nhất và an toàn nhất /ðə ˈlɑːdʒɪst ænd ˈseɪfɪst ɪˈlektrɪk kɑː/
- **carry seven passengers** : chở bảy hành khách /ˈkæri ˈsevn ˈpæsɪndʒəz/
- **economical** : (adj) tiết kiệm /ˌiːkəˈnɒmɪkl/
- **autopilot function** : chức năng tự lái /ˈɔːtəʊpaɪlət ˈfʌŋkʃn/
- **driverless** : (adj) không người lái /ˈdraɪvələs/
- **passenger** : (n) hành khách /ˈpæsɪndʒə/
- **even sleep** : thậm chí ngủ /ˈiːvn sliːp/

### 📖 Reading - Đọc

**Roadrunner Electric Cars**

Roadrunner is a car company that makes electric cars. The company introduced the first model in 2015 and they named it Speed. Speed soon became a success. Speed is safe, fast, comfortable, and not very expensive. It has four seats. The car can travel over 350 kilometres per hour. You only have to charge the battery every 700 kilometres. It even has a gaming screen inside.

Roadrunner là một công ty sản xuất ô tô điện. Công ty đã giới thiệu mẫu đầu tiên vào năm 2015 và họ đặt tên cho nó là Speed. Speed sớm thành công. Speed an toàn, nhanh chóng, thoải mái và không tốn kém nhiều. Nó có bốn chỗ ngồi. Xe có thể di chuyển trên 350 km/h. Bạn chỉ phải sạc pin sau mỗi 700 km. Nó thậm chí còn có một màn hình chơi game bên trong.

The company will introduce a new model next year called Safety. This model will become the largest and safest electric car you can buy. It will be able to carry seven passengers. The new model will also be more economical.

Công ty sẽ giới thiệu một mẫu mới vào năm tới có tên là Safety. Mẫu xe này sẽ trở thành chiếc xe điện lớn nhất và an toàn nhất mà bạn có thể mua. Nó sẽ có thể chở bảy hành khách. Mẫu mới cũng sẽ tiết kiệm hơn.

Both models have an autopilot function, so they are driverless. Passengers can read, play games, or even sleep while they travel.

Cả hai mẫu đều có chức năng lái tự động, vì vậy chúng không cần người lái. Hành khách có thể đọc sách, chơi trò chơi hoặc thậm chí ngủ trong khi di chuyển.

### ✍️ Exercises - Bài tập

**Bài 2**: Read the passage and answer the question. What is its main idea? (Đọc đoạn văn và trả lời câu hỏi. Ý chính của nó là gì?)
A. Roadrunner is a success.
B. Roadrunner is a car company.
C. Roadrunner's cars have an autopilot function.

**Answer:** B

**Bài 3**: Read the passage again and complete the fact file with no more than TWO words or a number. (Đọc lại đoạn văn và hoàn thành tệp dữ kiện với không quá HAI từ hoặc một số.)

ROADRUNNER'S CARS
1. Company's first model: **2015**
2. Safety: **largest** and **safest** model
3. Both models have: **autopilot function**
4. Both models run on: **electricity**

### 🗣️ Speaking - Nói

**Bài 4**: Work in pairs. Look at the fact file in 3 then ask and answer questions about it. (Làm việc theo cặp. Nhìn vào tệp thông tin trong bài 3 rồi hỏi và trả lời các câu hỏi về nó.)

Example:
A: What is the name of the company?
B: Roadrunner.
A: When did the company introduce its first model?
B: In 2015.
A: What will Safety be?
B: It will be the largest and safest electric car.
A: What will both models have?
B: They will have a gaming screen and an autopilot function.
A: What will both models run on?
B: Electricity.
A: What can passengers do?
B: Read, sleep, play games.

**Bài 5**: Work in groups. Discuss why Roadrunner's cars will or won't become popular in the near future. Report your reasons to the class. (Làm việc nhóm. Thảo luận tại sao xe của Roadrunner sẽ hoặc không trở nên phổ biến trong tương lai gần. Báo cáo lý do của bạn với lớp.)

Example answers:
- We think Roadrunner's cars will become popular in the near future because they are eco-friendly.
- We think Roadrunner's cars won't become popular in the near future because they might be dependent on batteries.

## SKILLS 2 - KỸ NĂNG 2

### 📚 Vocabulary - Từ vựng
- **comfortable** : (adj) thoải mái /ˈkʌmfətəbl/
- **economical** : (adj) tiết kiệm, kinh tế /ˌiːkəˈnɒmɪkl/
- **run on petrol** : chạy bằng xăng /rʌn ɒn ˈpetrəl/
- **bullet train** : (n) tàu cao tốc /ˈbʊlɪt treɪn/
- **popular** : (adj) phổ biến /ˈpɒpjələr/
- **use up much space** : chiếm nhiều diện tích /juːz ʌp mʌtʃ speɪs/
- **avoid traffic accident** : tránh tai nạn giao thông /əˈvɔɪd ˈtræfɪk ˈæksɪdənt/
- **reasonable prices** : giá hợp lý /ˈriːznəbl ˈpraɪsɪz/
- **huge amount** : lượng lớn /hjuːdʒ əˈmaʊnt/
- **reduce** : (v) giảm /rɪˈdjuːs/
- **automobile** : (n) ô tô /ˈɔːtəməbiːl/
- **advantages** : (n) ưu điểm /ədˈvɑːntɪdʒɪz/

### 👂 Listening - Nghe

**Listening Transcript:**
Mr Ha: Now, let's turn to future means of transport. How do you think people will travel in 2050, Tom?
Tom: I think people will use bullet trains. Bullet trains will be faster and safer than cars and they can help avoid traffic accidents.
Lan: I think skyTrans will also be popular, too. They will not use up much space and will be safe because they run on autopilot.
Tom: But skyTrans may be too expensive for students to use. Bamboo-copters will be cheaper and easier for them to use. They will just put on their bamboo-copters and fly to school.
Mr Ha: Sounds interesting. How about travelling on sea?
Lan: I think people will use solar-powered ships. They will be eco-friendly and comfortable.

### ✍️ Exercises - Bài tập

**Bài 2**: Listen to a talk between Mr Ha and his students. How many means of transport are they talking about? Circle the correct answer. (Nghe cuộc nói chuyện giữa thầy Hà và học sinh. Họ đang nói về bao nhiêu phương tiện giao thông?)
A. 2     B. 3     C. 4

**Answer:** C

**Bài 3**: Listen to the talk again and complete each sentence with ONE word. (Nghe lại cuộc nói chuyện và hoàn thành mỗi câu với MỘT từ.)
1. Bullet trains will be faster, safer and riders can avoid traffic **accidents**.
2. SkyTrans will use little space and run on **autopilot**.
3. SkyTrans may be too **expensive** for students.
4. **Bamboo-copters** will be cheaper and easier to use.
5. Solar-powered ships will be **eco-friendly** and comfortable.

**Bài 4**: Work in pairs. Choose one future means of transport in 3 and tick (✓) the words and phrases that describe its advantages. Can you add more words and phrases? (Làm việc theo cặp. Chọn một phương tiện giao thông tương lai trong bài 3 và đánh dấu các từ và cụm từ mô tả ưu điểm của nó.)

Example for bullet trains:
✓ fast
✓ safe
✓ eco-friendly
✓ comfortable
✓ avoid traffic jams

### ✍️ Writing - Viết

**Bài 5**: Now write a paragraph of about 70 words about the advantages of the means of transport you've chosen. (Bây giờ viết một đoạn văn khoảng 70 từ về ưu điểm của phương tiện giao thông bạn đã chọn.)

**Model answer:**
I think people will like travelling by bullet trains in the future because it has many advantages. First, travelling by bullet trains is very eco-friendly. It helps reducing the huge amount of transport and traffic jams. Second, travelling bullet trains are much faster than driving a car. Finally, using bullet trains is safe. Tens of thousands of people die each year in automobile accidents, but trains are one of the safest forms of transportation we currently have.

Tôi nghĩ mọi người sẽ thích đi du lịch bằng tàu cao tốc trong tương lai vì nó có nhiều ưu điểm. Đầu tiên, di chuyển bằng tàu cao tốc rất thân thiện với môi trường. Nó giúp giảm lượng lớn phương tiện giao thông và kẹt xe. Thứ hai, đi tàu cao tốc nhanh hơn nhiều so với lái xe ô tô. Cuối cùng, sử dụng tàu cao tốc là an toàn. Hàng chục nghìn người chết mỗi năm trong các vụ tai nạn ô tô, nhưng tàu hỏa là một trong những hình thức vận chuyển an toàn nhất mà chúng ta hiện có.

## LOOKING BACK - ÔN TẬP

### 📚 Vocabulary Review - Ôn tập từ vựng

**Complete Unit Vocabulary List:**
- bamboo-copter, bullet train, driverless car, eco-friendly, electric car
- flying car, hyperloop, means of transport, run on, safe
- skyTran, solar-powered ship, teleporter, traffic jam, will/won't

### ✍️ Exercises - Bài tập

**Bài 1**: Write three adjectives or phrases to describe each picture. (Viết ba tính từ hoặc cụm từ để mô tả mỗi bức tranh.)
1. hyperloop: fast, green, carrying many passengers
2. flying car: fast, convenient, autopilot function
3. monowheel/self-balanced bike: electric, green, convenient

**Bài 2**: Complete the sentences with the words and phrases from the box. (Hoàn thành câu với các từ và cụm từ từ hộp.)
1. An **eco-friendly** way to travel is by bicycle.
2. A car can **run on** petrol or electricity.
3. A **bamboo-copter** can carry only one or two people.
4. A **driverless** car can drive itself.
5. **Bullet trains** are faster and greener than normal trains.

**Bài 3**: Find ONE mistake in each sentence and correct it. (Tìm MỘT lỗi trong mỗi câu và sửa lại.)
1. My motorbike is an electric one. **Her** is not. → **Hers** is not.
2. **Our** can carry seven passengers. → **Ours** can carry seven passengers.
3. **My** is an electric one. → **Mine** is an electric one.
4. This is my one. **Its** is not fast. → **It** is not fast.
5. Your flying car is red. **Your** is green. → **Mine** is green.

**Bài 4**: Read the passage and put the verbs in brackets in the correct future form. (Đọc đoạn văn và đặt động từ trong ngoặc ở dạng tương lai đúng.)

Welcome to our company. We are Elongated. We make electric cars. Our new electric car (1. be) **will be** on the market soon. It (2. have) **will have** four seats. The car (3. be) **will be** safe, comfortable, and fast. It (4. not have) **won't have** a steering wheel. It will have an autopilot function. In the future, you (5. not need) **won't need** to drive your car anymore. Just sit back and relax. The car (6. take) **will take** you wherever you want to go. It (7. be) **will be** your smartest choice.